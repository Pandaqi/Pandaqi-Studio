...
export default class BoardDisplay
{
    ...
    convertToRealPositions(points:Vector2[])
    {
        const arr = [];
    ...
    convertToRealPosition(p:Vector2)
    {
    ...
    draw(board:BoardState)
    {
        ...
        const rect = new Rectangle().fromTopLeft(new Vector2(), canvSize);
    ...
    drawOutline(paths:Path[], op:LayoutOperation)
    {
        ...
        {
            const pathReal = this.convertToRealPositions(path.toPathArray());
            const pathObj = new Path(pathReal);
    ...
    drawContinentScores(b:BoardState)
    {
        ...
        const pos = new Vector2(radius+margin);
        ...
        for(const continent of b.continents.get())
        {
            ...
            const textPos = new Vector2(pos.x + 2*radius, pos.y);
