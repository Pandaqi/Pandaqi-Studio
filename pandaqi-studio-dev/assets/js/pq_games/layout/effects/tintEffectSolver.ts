...
const getTintCSSFilters = (inputColor:Color, baseColor = Color.WHITE) => 
{
    ...
    const color = new ColorRGB(rgb[0], rgb[1], rgb[2]);
    const fromColor = new ColorRGB(rgb2[0], rgb2[1], rgb2[2]);
    const solver = new TintSolver(color, fromColor);
...
class TintSolver 
{
    ...
    solve() {
      ...
      return {
        ...
        loss: result.loss,
        filters: this.filterList(result.values),
    ...
    filterList(filters)
    {
        ...
        return [
            // I always use white tint spritesheets, so use this to turn it black first
            `brightness(0)`,
            `saturate(100%)`,

            // then this is from the solver to reach that exact color
            `invert(${fmt(0)}%)`,
            `sepia(${fmt(1)}%)`,
            `saturate(${fmt(2)}%)`,
