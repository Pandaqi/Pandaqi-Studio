...
export default class FlowInput extends InputGroup
{
    ...
    calc(cont:LayoutNode) : FlowOutput
    {
    ...
    allHaveSize(list:LayoutNode[]) : boolean
    {
...
class FlowLine
{
    ...
    constructor(params:Record<string,any> = {})
    {
        this.LayoutNode = params.LayoutNode;
        this.list = [];
    ...
    getElementSize(elem:LayoutNode)
    {
    ...
    getAvailableSpace()
    {
        ...
        spaceUsedFlow += this.LayoutNode.flowOutput.gap * (this.count() - 1);
    ...
    getAvailableSpaceElement(elem:LayoutNode)
    {
    ...
    alignFlowAxis()
    {
        ...
        const flowInput = this.LayoutNode.flowInput;
        let spaceEdge = 0;
        let spaceBetween = 0;
    ...
    getGapFlow()
    {
        return this.flowVec.clone().scaleFactor(this.LayoutNode.flowOutput.gap);
    }

    getGapStack()
    {
        return this.stackVec.clone().scaleFactor(this.LayoutNode.flowOutput.gap);
    }

    updateCurrentPosition(child:LayoutNode, extraSpace:number = 0) : Point
    {
        ...
        if(this.LayoutNode.flowInput.alignFlow == AlignValue.END) { return betweenPos; }
        return oldPos;
    }
    ...
    resetCurrentPosition()
    {
        ...
        if(this.LayoutNode.flowInput.alignFlow == AlignValue.END)
        {
            this.curPos.move(this.flowVec.clone().abs().scale(this.maxSize))
        }

        if(this.LayoutNode.flowInput.alignStack == AlignValue.END)
        {
            this.curPos.move(this.stackVec.clone().abs().scale(this.maxSize))
        }
    }

    add(child:LayoutNode)
    {
    ...
    getDimensions(list = this.list) : Dims
    {
        const dims = new Dims();
        for(const cont of list)
        {
            dims.takeIntoAccount(cont.getDimensions());
        }
        return dims
    }
