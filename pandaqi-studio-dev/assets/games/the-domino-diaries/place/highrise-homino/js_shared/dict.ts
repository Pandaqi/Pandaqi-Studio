...
const OBJECTS:Record<string, GeneralData> =
{
    ...
    floor_counter: { frame: 2, prob: 3, label: "Floor Icon", desc: "<b>How many</b> you own is used to <b>sort players</b> by floor.", sets: ["base", "roomService", "happyHousing", "walletWatchers"] },
    sofa: { frame: 3, label: "Sofa", sets: ["base", "roomService"] },
    bed: { frame: 4, label: "Bed", sets: ["base", "roomService"] },
    toilet: { frame: 5, label: "Toilet", sets: ["base", "happyHousing"] },
    table: { frame: 6, label: "Table", sets: ["base", "happyHousing"] },
    chair: { frame: 7, label: "Chair", sets: ["base", "walletWatchers"] },
    plant: { frame: 8, label: "Plant", sets: ["base", "happyHousing"] },
    pool: { frame: 9, label: "Pool", desc: "During Attract, 1 Tenant of a <b>connected player</b> with <b>fewer pools</b> moves to you.", sets: ["roomService", "happyHousing"] },
    bar: { frame: 10, label: "Bar", desc: "Worth <b>+5</b> if you have <b>Water</b> at the end of the game.", sets: ["usefulUtilities"] },
    computer: { frame: 11, label: "Computer", desc: "All adjacent icons, rooms and floors <b>count double, if</b> you currently have <b>Wi-Fi</b>.", sets: ["usefulUtilities"] },
    wardrobe: { frame: 12, label: "Wardrobe", desc: "Worth <b>-5</b> points. You may <b>attract 2 Tenants</b> each round.", sets: ["roomService"] },
    piano: { frame: 13, label: "Piano", desc: "Only counts if <b>inside a Room</b>.", sets: ["roomService", "happyHousing"] },
    hometrainer: { frame: 14, label: "Hometrainer", desc: "Adds <b>+2</b> to every Tenant that shows my icon.", sets: ["walletWatchers"] },
    desk: { frame: 15, label: "Desk", desc: "Adds <b>+1</b> for every Room, <b>if</b> inside a Room.", sets: ["walletWatchers", "roomService"] },
    generator_electricity: { frame: 16, label: "Power Generator", desc: "Gives you <b>Power</b> if you have access to it.", sets: ["usefulUtilities"], nonWishable: true },
    generator_water: { frame: 17, label: "Water Generator", desc: "Gives you <b>Water</b> if you have access to it.", sets: ["usefulUtilities"], nonWishable: true },
    generator_wifi: { frame: 18, label: "Wi-Fi Generator", desc: "Gives you <b>Wi-Fi</b> if you have access to it.", sets: ["usefulUtilities"], nonWishable: true },
...
const WISHES:Record<string, GeneralData> =
{
    ...
    floor_type: { frame: 2, label: "Floor Type", desc: "You need at least this many of the floor type shown.", range: new Bounds(1,5), sets: ["walletWatchers"], subKey: true },
    num_tenants: { frame: 3, label: "Tenant Count", desc: "You need at least this many tenants.", range: new Bounds(1,4), sets: ["happyHousing"] },
    num_rooms: { frame: 4, label: "Room Count", desc: "You need at least this many rooms.", range: new Bounds(1,4), sets: ["roomService"] },
    utilities: { frame: 5, label: "Utility", desc: "You need this utility.", range: new Bounds(1,1), prob: 4.5, sets: ["usefulUtilities"], subKey: true, scoreVal: 2.5 },
    num_connections: { frame: 6, label: "Connection Count", desc: "You need to be connected to at least this many players.", range: new Bounds(1,3), sets: ["happyHousing"] },
    tenants: { frame: 7, label: "Tenant Type", desc: "You need to have this type of tenant already.", range: new Bounds(1,2), sets: ["usefulUtilities"], subKey: true },
    diversity_object: { frame: 8, label: "Object Diversity", desc: "You need to have at least this many unique objects.", range: new Bounds(2,6), sets: ["happyHousing"] },
    diversity_tenant: { frame: 9, label: "Tenant Diversity", desc: "You need to have at least this many unique tenants.", range: new Bounds(3,5), sets: ["walletWatchers"] },
...
const MISSIONS:Record<string,MissionData> = 
{
    ...
    all_utilities: { type: MissionType.GOAL, descTask: "Have access to <b>all 3 utilities</b> (electricity, water, wifi).", descReward: "<b>Power:</b> You have access to electricity, no matter what floor you're on.", set: "usefulUtilities"  },
    water_utility: { type: MissionType.GOAL, descTask: "<b>All</b> players have access to <b>Water</b> this round.", descReward: "<b>Power:</b> everyone has access to water, always.", set: "usefulUtilities" },
    wifi_utilitity: { type: MissionType.GOAL, descTask: "Have access to <b>Wi-Fi</b>, and be the <b>only player</b> to do so.", descReward: "<b>Power:</b> If you're the highest floor, this domino provides Wi-Fi to all players for free.", set: "usefulUtilities" },
    num_rooms: { type: MissionType.GOAL, descTask: "Have at least <b>5 rooms</b>.", descReward: "This domino is considered one room on its own, of size 2 and any floor type you want.", set: "roomService" },
    ...
    all_expensive: { type: MissionType.GOAL, descTask: "Have a map with a <b>price</b> of at least <b>5</b>.", descReward: "<b>One-Time:</b> Attract any Tenant you want (from deck) that has the Wallet property.", set: "walletWatchers" },
    many_wander: { type: MissionType.GOAL, descTask: "Have at least <b>3 tenants leave</b> (the same player) in a single round.", descReward: "<b>Power:</b> no Tenant will ever leave you again, for whatever reason.", set: "happyHousing" },
    ...
    window_reward: { type: MissionType.GOAL, descTask: "Have (at least) <b>twice as many windows</b> as doors.", set: "walletWatchers" },
    ...
    identical_rooms: { type: MissionType.GOAL, descTask: "All players have an <b>identical number of Rooms</b>, and at least 1.", set: "roomService" },
    ...
    room_requirement: { type: MissionType.THREAT, descTask: "The next time someone attracts a Tenant, they must have <b>at least 1 Room</b>.", descReward: "All players lose 1 Tenant.", set: "roomService" },
    no_room_requirement: { type: MissionType.THREAT, descTask: "One player must <b>not</b> have a <b>single Room</b>.", set: "roomService" },
    room_window_requirement: { type: MissionType.THREAT, descTask: "The next time someone attracts a Tenant, they must have <b>at least 1 Room</b> with a <b>window</b>.", set: "roomService" },
    ...
    price_requirement: { type: MissionType.THREAT, descTask: "<b>No</b> map may have a <b>higher price than 4</b>.", set: "walletWatchers" },
    wander_requirement: { type: MissionType.THREAT, descTask: "<b>No</b> tenant may <b>leave</b> this round.", set: "happyHousing" },
    utility_requirement: { type: MissionType.THREAT, descTask: "One player must have <b>no</b> connection to any <b>utilities</b> at all.", set: "usefulUtilities" },
    ...
    property_requirement: { type: MissionType.THREAT, descTask: "<b>No</b> player may have <b>all Tenant properties</b> in the game (spread across all their tenants).", set: "happyHousing" },
    ...
    window_requirement: { type: MissionType.THREAT, descTask: "<b>All</b> players must have <b>at least as many windows</b> as doors.", set: "walletWatchers" },
