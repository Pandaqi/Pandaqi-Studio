...
export default class BoardRiver
{
    ...
    flow(start, grid, bounds, crossMap) 
    {
    ...
    createRiver(start, grid, bounds, crossMap)
    {
        var list = [start];
        var lastCell = start;
    ...
    edgesAreDifferent(lastCell, start)
    {
    ...
    edgesAreOpposite(lastCell, start)
    {
        if(!lastCell.onEdge() || !start.onEdge()) { return false; }
        var a = lastCell.getEdgeName();
        var b = start.getEdgeName();
    ...
    filterAlreadyVisited(list, listMatch)
    {
        const arr = [];
        for(const cell of list)
        {
