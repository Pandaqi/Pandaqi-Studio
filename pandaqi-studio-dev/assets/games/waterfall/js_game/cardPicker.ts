...
export default class CardPicker
{
    ...
    generateRecipeCards()
    {
        ...
        for(let i = 1; i < pickers.length; i++)
        {
            ...
            {
                card.addRecipe(this.createMultiFoodRecipe(pickers, i));
            }
        }
        ...
        for(const card of arr)
        {
            ...
            {
                card.addRecipe(this.createMultiFoodRecipe(pickers, 0));
            }
        }
    ...
    createMultiFoodRecipe(pickers:BalancedFrequencyPickerWithMargin[], maxTier:number, customProb:number = -1)
    {
        const recipe = [pickers[maxTier].pickNext()];
        if(!CONFIG.allowMultiFoodRecipes) { recipe; }

        let prob = CONFIG.generation.multiFoodRecipeProbPerTier[maxTier];
        if(customProb >= 0) { prob = customProb; }

        const shouldCreateMulti = Math.random() <= prob;
