...
const CONFIG =
{
    debugSpecificSet: "random", // @DEBUGGING (should be null)
    debugSimulate: true, // @DEBUGGING (should be false)
...
async function generate()
{
    ...
    // > cards on the table
    const numCards = rangeInteger(4,6);
    o.addParagraph("These cards were played. (Imagine they're in a circle. The first card is the one played by the king.)");
    const round = new Round().generate(numCards);
