...
const CONTRACT_SPECIAL:GeneralDict =
{
    ...
    actions_max: { desc: "You fail if more than %numactions% actions appear", diff: 1 },
    special_soul_reroll: { desc: "1 Soul may reroll once (for free)", diff: -1 },
    special_soul_backout: { desc: "1 Soul is allowed to leave the challenge before it's done", sets: ["lost"], diff: -1 },
    insta_fail_action: { desc: "If 2 Souls roll the same action, you fail", prob: 0.5, diff: 2 },
    insta_fail_number: { desc: "If 2 Souls roll the same number, you fail", prob: 0.5, sets: ["lost"], diff: 3 },
    insta_win_number: { desc: "If 2 Souls roll the same number, yousucceed", prob: 0.33, sets: ["lost"], diff: -3 },
    insta_win_action: { desc: "If 2 Souls roll the same action, you succeed", prob: 0.33, diff: -2 },
...
const CARD_ACTIONS:GeneralDict =
{
    ...
    ignore: { label: "Ignore", desc: "<b>Ignore</b> one number already rolled ( = it's 0 and no action).", prob: 1.5 },
    ...
    no_duplicates: { label: "No Duplicates", desc: "If anybody rolls the <b>same number</b> as me, I'm <b>ignored</b> ( = 0 and no action).", sets: ["devilish"], prob: 2 },
    attendance_bonus: { label: "Attendance", desc: "If <b>all players joined</b>, I'm any number <b>between 1 and 10</b> (you choose).", sets: ["devilish"] },
    die_manipulation: { label: "Die Manipulator", desc: "Pick another Soul. <b>Add or Remove</b> 1 card from their die.", sets: ["devilish"] },
    random_roll: { label: "Reroll Random", desc: "<b>Draw</b> 1 card from deck. Pretend you rolled that; keep it afterward.", sets: ["devilish"] },
    ...
    reroll_mega: { label: "Mega Reroll", desc: "Other Souls may reroll their result, but <b>at most 2 times</b> (for the whole group).", sets: ["devilish"], prob: 0.5 },
    diebreak: { label: "Diebreak", desc: "Other Souls may use only a <b>part</b> of their die, but at least 2 cards.", sets: ["devilish"], prob: 0.5 }
}
