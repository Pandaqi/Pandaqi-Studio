...
const BoardGeneration = new Phaser.Class({
    ...
    preload: function() {
		...
		var base = 'gamesites/one-pizza-the-puzzle/';

		this.load.spritesheet('roadmarks', base + 'roadmarks.png?c=1', { frameWidth: 200, frameHeight: 200 }) 
		this.load.spritesheet('ingredients', base + 'ingredients.png?c=6', { frameWidth: 200, frameHeight: 200 }) // big ingredient icons, used on buildings/entrances
		this.load.spritesheet('crust', base + 'crust.png?c=3', { frameWidth: 200, frameHeight: 200 }) // pizza crust + smaller icons used when combining pizza

		this.load.spritesheet('general_icons', base + 'general_icons.png?c=8', { frameWidth: 200, frameHeight: 200 }) // the icons for any special buildings or elements on the map

		this.load.spritesheet('decorations', base + 'decorations.png?c=4', { frameWidth: 200, frameHeight: 200 }) // natural decorations (such as fountain, roundabout, hedge, ...)

		this.load.spritesheet('traffic_signs', base + 'traffic_signs.png?c=1', { frameWidth: 200, frameHeight: 200 }) // traffic signs (from the Expansion)

		this.load.spritesheet('shapes', base + 'shapes.png?c=3', { frameWidth: 200, frameHeight: 200 }); // movement shapes to pick from (when moving a Pizza Courier)

		this.load.spritesheet('special_buildings', base + 'special_buildings.png?c=3', { frameWidth: 200, frameHeight: 200 }) // special building icons (used when Preposterous Places is enabled)
    
		this.load.image('unique_shapes_hint', base + 'unique_shapes_hint.png'); // a hint to remind players about picking unique shapes for each courier
    },
    ...
    create: function(config) {
    	...
    	this.cfg.resX = 10 + Math.floor(gameConfig.numPlayers*1.5);
    	...
    	this.cfg.buildingMinSize = Math.round(gameConfig.numPlayers*0.33); // when growing buildings, minimum size required (at 4 players = 2)
    	this.cfg.buildingMaxSize = Math.round(gameConfig.numPlayers); // when growing buildings, maximum size allowed (at 4 players = 6)
    	this.cfg.buildingGrowthChance = 0.6 + 0.033 * gameConfig.numPlayers; // between min-max, probability of continuing growth (at 4 players = 0.8)

    	this.cfg.maxEntrancesPerBuilding = 2 + Math.round(gameConfig.numPlayers*0.2) // when placing extra entrances (after dead-ends), average cutoff used for ignoring buildings that already have X entrances

    	// TO DO: Debugging = hard cutoff values for all player counts => seems to work fine?
    	this.cfg.buildingMinSize = 2;
    	this.cfg.buildingMaxSize = 5;
    	this.cfg.buildingGrowthChance = 0.6;
    	this.cfg.maxEntrancesPerBuilding = 1 + Math.round(gameConfig.numPlayers*0.2);
    	...
    	const variation = gameConfig.boardVariation;
    	...
    	this.cfg.policeFactor = Math.min( (variation + 1.0)/5.0, 1.0) * Math.max( (gameConfig.numPlayers / 4.0), 1.0); // if the board has more straight roads, we need LESS police, because they cover everything in a straight line 
    	...
    	if(gameConfig.expansions["Preposterous Places"]) { this.cfg.roundaboutProbability = 0.75; }
    	...
    	this.cfg.numTrafficSigns = gameConfig.numPlayers*5;
    ...
    generateBoard: function() {
    	...
    	if(gameConfig.expansions["Pizza Police"]) {
    ...
    reserveShapeSquares: function() {
    	var CONFIG_RNG = new Math.seedrandom(gameConfig.seed + "-config")
    ...
    determineShapes: function() {
    	var RNG = new Math.seedrandom(gameConfig.seed + '-shapes');
    	...
    	// TO DO: Also include AT MOST one exotic shape?
    	if(numShapes <= 2) {
    ...
    performCrossingDiagonalVariation: function() {
    ...
    addRoadCell: function(pos, dir, originCrossing) {
    ...
    removeRoadCell: function(pos) {
    	...
    	// TO DO: also remove ourselves from the curRoadSet
    ...
    getValidExtensionDir: function(pos) {
    ...
    extendRoad: function(pos, dir) {
    	...
    	while(canContinue) {
    		...
    		// TO DO: Make this CLEANER => put into its own function?
    ...
    prepareLists: function() {
    	...
    	var seed = gameConfig.seed;
    	this.RAND_POINT_RNG = new Math.seedrandom(seed + "-randPoints");
    	this.BUILDING_GROW_RNG = new Math.seedrandom(seed + "-buildingGrowth");
    	this.BUILDING_TYPE_RNG = new Math.seedrandom(seed + '-buildingType');
    	this.SHUFFLE_RNG = new Math.seedrandom(seed + "-shuffle");

    	this.ALLEY_RNG = new Math.seedrandom(seed + "-alleys");

    	this.ROAD_EXTEND_RNG = new Math.seedrandom(seed + "-roadExtensions");
    	this.VARIATION_RNG = new Math.seedrandom(seed + "-roadVariations");

    	this.RANDOM_DRAW_RNG = new Math.seedrandom(seed + "-randomDraws");
    	this.RANDOM_SQUARE_RNG = new Math.seedrandom(seed + "-randomSpecialties");

    	this.TRAFFIC_SIGN_RNG = new Math.seedrandom(seed + "-trafficSigns");
    ...
    generateBordersToPos: function(pos, buildingIndex) {
    	var nbs = [[1,0], [0,1], [-1,0], [0,-1]]
    	var borders = [];
    ...
    convertDirToIndex: function(dir) {
    	if(dir[0] == 1 && dir[1] == 0) {
    		return 0;
    ...
    convertIndexToDir: function(num) {
    ...
    generateValidNeighbours: function(pos) {
    	var nbs = [[1,0], [0,1], [-1,0], [0,-1]]
    	var validNeighbours = [];
    ...
    shuffle: function(a) {
	    ...
	    for (i = a.length - 1; i > 0; i--) {
	        j = Math.floor(this.SHUFFLE_RNG() * (i + 1));
	        x = a[i];
	        a[i] = a[j];
	        a[j] = x;
	    }
    ...
    getOrientationFromNeighbours: function(pos) {
    ...
    solveUnconnectedAreas: function() {
    ...
    createAlleys: function() {
    	for(var i = this.buildings.length - 1; i >= 0; i--) {
    		...
    		// TO DO: If I calculate this, I really don't need the "streetConnection" parameter on buildings
    ...
    calculateDirFromSurroundings: function(pos) {
    	var nbs = [[1,0], [0,1], [-1,0], [0,-1]];
    	for(var i = 0; i < 4; i++) {
    ...
    generateEmptyBuilding: function(index = -1) {
    	return {
			'tiles': [],
			'borders': [],
			'index': index,
    ...
    determineBuildingBorders: function() {
    ...
    determineDecorationBorders: function() {
    	var specialPlacesEnabled = gameConfig.expansions["Preposterous Places"]
    ...
    obstacleWithCenter: function(pos) {
    	for(var i = 0; i < this.obstacles.length; i++) {
    ...
    removeUglyBoxes: function() {
		var dirVectors = [[1,0], [2,1], [1,2], [0,1]];
		var boxNbs = [[0,0], [1,0], [1,1], [0,1]];
		var solutionTypes = ['hedge', 'roundabout'];

		var RNG = new Math.seedrandom(gameConfig.seed + '-uglyBoxes');
		var specialPlacesEnabled = gameConfig.expansions["Preposterous Places"] // TO DO: I repeat this variable all over the code, maybe just save it once in this.cfg?
    	var totalBuildingProbability = this.getTotal(SPECIAL_BUILDINGS);
    	var roundabouts = [];
		...
		if(specialPlacesEnabled) { 
	    	...
	    	for(var i = 0; i < decorationsToConvert; i++) {
	    		var type = this.getRandom(SPECIAL_BUILDINGS, totalBuildingProbability)
	    	...
	    	// TO DO: I use this twice in almost identical code => removeUglyBoxes() and determineDecorationBorders()
	    	...
	    	// TO DO: keep a counter on cells with the current SIZE of their plaza?
	    	// TO DO: Go through all cells one more time. Any plazas with a size that is too small, are converted back into something else
		}
    	
    },

    mergeUglyBox: function(pos, RNG) {
		var boxNbs = [[0,0], [1,0], [1,1], [0,1]];
		var success = false;
    ...
    buildingAtSide: function(pos, num) {
    ...
    sideClosed: function(pos, num) {
    	return (this.map[pos[0]][pos[1]].closedSides[num]);
    },

    isRoundabout: function(pos) {
    ...
    connectedWithRoad: function(pos) {
    	var nbs = [[1,0], [0,1], [-1,0], [0,-1]]
    ...
    getNeighbourBuildingTile: function(pos, maxEntrances = -1) {
    	var nbs = [[1,0], [0,1], [-1,0], [0,-1]]
    ...
    getNeighbourBuilding: function(type = 'connected', pos, buildingIndex) {
    	var nbs = [[1,0], [0,1], [-1,0], [0,-1]]
    ...
    reserveBuildings: function() {
    	...
    	const numBuildingsToReserve = gameConfig.numPlayers;
    	for(var i = 1; i < (numBuildingsToReserve + 1); i++) {
    		buildingsCopy[i].type = 'reserved'
    	}
    },

    placeSubway: function(pos) {
    ...
    createSubways: function() {
    	var SUBWAY_RNG = new Math.seedrandom(gameConfig.seed + '-subways')
    ...
    generateIngredientLists: function() {
    	...
    	var listMax = 28 * gameConfig.numPlayers;
    ...
    placeEntrances: function() {
    	...
    	var RNG = new Math.seedrandom(gameConfig.seed + "-entrances");
    	...
    	numNonEmptyBuildings -= (gameConfig.numPlayers + 1 + 1)
    ...
    placeSingleEntrance: function(pos, obj, maxEntrances = -1) {
    ...
    getSquarePositionOnCell: function(pos, ind, center = true) {
    ...
    distanceToSameEntrance: function(pos, ing) {
    ...
    convertBuildingsWithoutEntrance: function() {
    	var specialPlacesEnabled = gameConfig.expansions["Preposterous Places"]
    	var totalBuildingProbability = this.getTotal(SPECIAL_BUILDINGS);
    	...
    	for(var i = 0; i < this.buildings.length; i++) {
    		...
    		if(specialPlacesEnabled) {
    			var randSpecialType = this.getRandom(SPECIAL_BUILDINGS, totalBuildingProbability);
			...
			// if no merge is possible, just set the building to 'reserved' ( = empty)
    			...
    			// TO DO: Make this a general function, because we also call the exact same thing multiple times during other algorithms
    ...
    distanceToClosestPolice: function(posA) {
    	var dist = Infinity;
    ...
    getFreeSquareIndex: function(pos) {
    	return this.getRandomFalseValue(this.map[pos[0]][pos[1]].filledSquares)
    },

    // TO DO: This is almost a copy of the getOrientationFromNeighbour function, make general to avoid repeating myself?
    getEmptySide: function(pos) {
    	var nbs = [[1,0], [0,1], [-1,0], [0,-1]]
    	var tempArr = [];
    ...
    getRandomFalseValue: function(arr) {
    	var tempArr = [];
    ...
    distanceToClosestTrafficSign: function(posA) {
    	var dist = Infinity;
    ...
    getRandomTrafficPoint: function(type) {
    	...
    	do {
    		...
    		if(!invalidPoint) {
	    		if(isGate) {
	    			...
	    			// TO DO: Be more precise with checking this SIDE/EDGE overlaps with a DECORATION
    ...
    placeTrafficSigns: function() {
    	const numSigns = this.cfg.numTrafficSigns;

    	this.trafficSigns = [];

    	var totalTrafficSignProbability = this.getTotal(TRAFFIC_SIGNS);

    	for(var i = 0; i < numSigns; i++) {
    		var randType = this.getRandom(TRAFFIC_SIGNS, totalTrafficSignProbability);
    ...
    swapSpecialIngredients: function() {
    	var RNG = new Math.seedrandom(gameConfig.seed + "-specialIngredients");
    	...
    	var totalIngredientProbability = this.getTotal(SPECIAL_INGREDIENTS);
    	...
    	for(var i = 0; i < numSpecialIngredientsWanted; i++) {
    		...
    		if(this.specialIngredientsIncluded.length < 7) {
	    		do {
	    			type = this.getRandom(SPECIAL_INGREDIENTS, totalIngredientProbability)
    ...
    placeSpecialBuildings: function() {
    	// TO DO: Find all decorations. Replace small groups (1-3) with random buildings, large groups with PLAZAS
    	//  => This means the "border detection"/"group detection" must happen before this ... but also again afterward?

    	// TO DO: Add some more decorations + functional sprites to the road squares that do cool stuff
    },
    ...
    visualizeGame: function() {
    	...
    	for(var x = 0; x < this.cfg.resX; x += 0.5) {
    		var line = new Phaser.Geom.Line(x * cs, 0, x * cs, this.cfg.resY * cs);
		    gridGraphics.strokeLineShape(line);
    	}

    	for(var y = 0; y < this.cfg.resY; y += 0.5) {
    		var line = new Phaser.Geom.Line(0, y * cs, this.cfg.resX * cs, y * cs);
		    gridGraphics.strokeLineShape(line);
    	}
    	...
    	for(var x = 0; x < this.cfg.resX; x++) {
    		for(var y = 0; y < this.cfg.resY; y++) {
    			var obj = this.map[x][y];
    			var rect = new Phaser.Geom.Rectangle(x * cs, y * cs, cs, cs)

    			var color = 0xEEEEEE

    			// TO DO/DEBUGGING: check where crossings start
    			/*if(obj.subType == 'crossing') {
    				color = 0x999999;
    			}*/
    			...
    			if(obj.type == 'road') {
    				...
    				if(buildingObj.type == 'ingredient' || buildingObj.type == 'special') {
    					if(!gameConfig.inkFriendly || (x == buildingObj.centerCellData.cell[0] && y == buildingObj.centerCellData.cell[1])) {
    				...
    				if(!gameConfig.inkFriendly) {
	    				...
	    				var shadowRect = Phaser.Geom.Rectangle.Clone(rect);
    	...
    	for(var a = 0; a < this.buildings.length; a++) {
	    	for(var i = 0; i < this.buildings[a].borders.length; i++) {
	    		...
	    		// API: new Phaser.Geom.Line(x1, y1, x2, y2)
	    		var line = new Phaser.Geom.Line(b[0][0] * cs, b[0][1] * cs, b[1][0] * cs, b[1][1] * cs);
	    ...
	    var txt = this.add.text((randNeighbour[0] + 0.5)*cs, (randNeighbour[1] + 0.5)*cs, '' + gameConfig.seed, seedTextCfg);
	    txt.setOrigin(0.5, 0.5)
	    txt.depth = 15;
		...
		var rect = new Phaser.Geom.Rectangle(rectStart[0]*cs, rectStart[1]*cs, this.cfg.shapeRectSize[0]*cs, this.cfg.shapeRectSize[1]*cs);
    ...
    visualizeEntrance: function(obj, roadGraphics) {
    	const cs = this.cfg.cellSize;
    	const specialPlacesEnabled = gameConfig.expansions["Preposterous Places"];

    	var entData = obj.entrance
		var rectEntrance = new Phaser.Geom.Rectangle(entData.pos[0] * cs, entData.pos[1] * cs, 0.5*cs, 0.5*cs)
    ...
    visualizeTrafficSigns: function() {
    	const cs = this.cfg.cellSize;
    	...
    	for(var i = 0; i < this.trafficSigns.length; i++) {
    		...
    		if(dictObj.gate) {
				...
				if(obj.type == 'Line Gate') {
					...
					if(this.TRAFFIC_SIGN_RNG() <= gameConfig.numPlayers*0.025) { string = 3; }
    ...
    visualizeDecorations: function(buildingGraphics, overlayGraphics) {
    	const cs = this.cfg.cellSize;

    	var DECORATION_RNG = new Math.seedrandom(gameConfig.seed + "-decorations");
	    ...
	    for(var i = 0; i < this.obstacles.length; i++) {
	    	...
	    	if(o.type == 'hedge') {
	    		var h = o.line;
		    	var line = new Phaser.Geom.Line(h[0] * cs, h[1] * cs, h[2] * cs, h[3] * cs);
				...
				var rect = new Phaser.Geom.Rectangle((o.center[0] - 0.5) * cs, (o.center[1]-0.5) * cs, cs, cs);
				...
				for(var b = 0; b < o.borders.length; b++) {
					...
					var line = new Phaser.Geom.Line(border[0]*cs, border[1]*cs, border[2]*cs, border[3]*cs);
    ...
    visualizeOrders: function() {
	    ...
	    var DRAW_RNG = new Math.seedrandom(gameConfig.seed + '-drawStuff')
	    for(var i = 0; i < this.buildings.length; i++) {
	    	var b = this.buildings[i];
    ...
    pickCenterCellAndNeighbour: function(b) {
    ...
    getAllBuildingCellsAround: function(pos, ind) {
    	var nbs = [[1,0], [0,1], [-1,0], [0,-1]]
    ...
    getRandom: function(list, total) {
		const rand = this.RANDOM_DRAW_RNG();
...
function generateGameConfig() {
    gameConfig.boardVariation = parseInt(document.getElementById('setting-boardVariation').value) || 0
    gameConfig.expansions = 
        {
            'Pizza Police': document.getElementById('setting-pizzaPolice').checked || false,
            'Treacherous Traffic': document.getElementById('setting-treacherousTraffic').checked || false,
            'Ingenious Ingredients': document.getElementById('setting-ingeniousIngredients').checked || false,
            'Preposterous Places': document.getElementById('setting-preposterousPlaces').checked || false,
        }

    gameConfig.gameTitle = "One Pizza The Puzzle"
}