...
*/
var gamePhase = 'setup';
var subGamePhase = 'register';

var gameConfig, numPlayers, numTasksPerRound, numQuestionsPerTask, numRounds, firstGame;

// some configuration/globally needed variables
var taskList, oldTaskList;
var questionList;

var task;
var curRound = -1;
var curTask = -1, curQuestion = -1;
var curQuestionHintIndex = -1;

var players = [];
var moleIndex = -1;
var curTestTakerIndex = -1;
var playersThatTookTest = 0;

var calculatingTestResults = false;
var testStatistics = {};

var previousTaskWasForbidden = false;

var timerInterval = null;
var timerValue = 0;

var gokkerResults = null;
...
function prepareTaskList() {
	...
	var taskTypes = ['simple', 'money', 'help'];
	var leftoverTasks = [];
	...
	// TO DO: This assumes 3 tasks per round => make variable somehow?
...
function inputButtonPressed() {
	...
	if(gamePhase == 'setup') {
		if(subGamePhase == 'register') {
			...
			var playerObj = { name: val, mole: false };
			var resultText = 'Jij bent een kandidaat!'
	...
	// during the test, the input button starts a new test for the given player
		...
		if(subGamePhase == 'inbetween') {
			...
			var p = players[curTestTakerIndex]
			if(p.takenTest) {
			...
			var lastRound = (curRound == numRounds - 1)
...
function displayNextTask() {
	...
	var description = task.desc.replace(/\#/gi, '<span class="numPlayersIndicator">#</span>');
	...
	if(task.params.timePerPlayer != undefined || task.params.totalTime != undefined) {
		...
		document.getElementById('timerValue').setAttribute('data-originalTimerValue', timerValue);
...
function toggleRuleReminder(btn) {
	var reminderText = '<p>Elke opdracht volgt deze structuur:</p><ol><li>Pak startinzet uit bank + iedere speler mag eigen geld inzetten</li><li>Meeste geld ingezet? Je mag nieuwe PM nomineren</li><li>Leider kiest welke speler begint.</li><li>Tijdens je beurt, trek je één kaart, en speelt zoveel kaarten (open) als je wilt.</li><li>Speel één ronde (met de klok mee), check dan resultaat</li></ol>'
...
function getRandomMoneyValue() {
	var moneyValues = [10, 20, 50, 100];
	var randNumBriefjes = Math.floor(Math.random() * 3) + 3;

	var total = 0;
	for(var m = 0; m < randNumBriefjes; m++) {
...
function displayRandomOffer() {
	// TO DO: Think of more (varied/interesting/challenging) offers
	// 		  (Maybe also some things that don't require taking a lot of money out of the shared money jar)

	var offerTypes = ['moneyToJokers', 'moneyToFreePass', 'moneyToCards', 'moneyToMisc'];
	var randType = offerTypes[Math.floor(Math.random() * offerTypes.length)]

	var msg = '<strong>Aanbieding! </strong>';

	switch(randType) {
		case 'moneyToJokers':
			var geldBedrag = getRandomMoneyValue();
			var reward = Math.floor(Math.random() * 2) + 2
		...
		case 'moneyToMisc':
			...
			var geldBedrag = getRandomMoneyValue();
			var rewardType = possibleRewards[Math.floor(Math.random() * possibleRewards.length)];
			var reward = Math.floor(Math.random() * 3) + 1;
...
function displayNextQuestion() {
	...
	var q = questionList[curQuestion];
	
	var connectedTask = q.connectedTask;
	var question = q.question;
	var options = q.answers.slice(); // copy of array, instead of reference => why? because mole can edit option list
	...
	var title = 'Vraag over <em>' + connectedTask + '</em>';
	if(connectedTask == null) {
		title = 'De grote vraag ...';
	}

	var curPlayerIsMole = (curTestTakerIndex == moleIndex);
	var lastRound = (curRound == numRounds - 1)
	var lastQuestion = (curQuestion == questionList.length - 1);
	...
	for(var j = 0; j < options.length; j++) {
		...
		number.addEventListener('click', function(ev) {
			var number = ev.currentTarget.getAttribute('data-num');
...
function displayMoleHint(mole = true, fixedHint = null, fixedIndex = -1) {
	...
	var hintTypes = ['Categories', 'Task', 'Money', 'Treasurer', 'Leader', 'TaskType', 'Snitch'];
	if(!mole) {
		hintTypes = ['Categories', 'Money', 'Treasurer', 'Leader'];
	}

	var randomHintType = hintTypes[Math.floor(Math.random() * hintTypes.length)];
	if(fixedHint != null) {
		randomHintType = fixedHint
	}

	var hintText = '<strong>Hint! </strong>';

	var randTaskIndex = Math.floor(Math.random() * numTasksPerRound);
	if(fixedIndex >= 0) {
	...
	switch(randomHintType) {
		...
		case 'Treasurer':
			var numTreasurerTasks = 0;
			for(var i = 0; i < numTasksPerRound; i++) {
		...
		case 'Snitch':
			var correctMoleVotersHistory = [];
			var correctMoleVotersNow = [];

			var testLength = questionList.length
			for(var i = 0; i < players.length; i++) {
			...
			if(correctMoleVotersNow.length <= 0 && correctMoleVotersHistory.length <= 0) {
				...
				var randomMoleVoter = correctMoleVotersNow[Math.floor(Math.random() * correctMoleVotersNow.length)]
				hintText += 'Oh nee! De speler <em>' + randomMoleVoter + '</em> heeft jou als trol aangewezen bij deze test';
			} else {
				var randomMoleVoter = correctMoleVotersHistory[Math.floor(Math.random() * correctMoleVotersHistory.length)]
		...
		case 'KnownAnswers':
			...
			var lastQuestion = (randTaskIndex == (questionList.length - 1))
			var splitText = [];
			for(var i = 0; i < players.length; i++) {
				if(players[i].takenTest) {
					var answer = players[i].testAnswers[randTaskIndex]
...
function createQuestionList() {
	...
	for(var i = 0; i < numTasksPerRound; i++) {
		...
		// (or only add as many as the current settings allow => TO DO: Maybe shuffle this? Now it always adds the same on reduced settings?)
		var numQ = Math.min(numQuestionsPerTask, questions.length)
	...
	var q = new Q("Wie is de Trol?", answers);
	questionList.push(q);
}
...
function getPlayerWithWorstTestResult() {
	...
	var correctAnswers = players[moleIndex].testAnswers;
	...
	var selfDestruct = (correctAnswers[correctAnswers.length - 1] == players.length);
	if(selfDestruct) {
	...
	var destroyFreePass = false;
	var numPlayersWithFreePass = 0;
	for(var i = 0; i < players.length; i++) {
	...
	var lastRound = (curRound == numRounds - 1)
	var checkingDir = lastRound ? -1 : 1;

	// then grade the test from all other players
	var maxPossibleTestScore = questionList.length
	var worstPlayer = null, worstScore = checkingDir*Infinity, worstTime = checkingDir*Infinity;
	for(var i = 0; i < players.length; i++) {
		if(i == moleIndex) { continue; }
		...
		var p = players[i];
		var score = 0;
		for(var a = 0; a < p.testAnswers.length; a++) {
			if(p.testAnswers[a] == correctAnswers[a]) {
				score++;

				var lastQuestion = (a == p.testAnswers.length - 1);
				if(lastQuestion) {
	...
	if(gameConfig.expansions.specialeKrachten) {
		testStatistics.resultsWithPlayerAttached.sort(function(a,b) { return a.score < b.score });

		for(var i = 0; i < testStatistics.resultsWithPlayerAttached.length; i++) {
			var playerIndex = testStatistics.resultsWithPlayerAttached[i].player;
	...
	var allPlayersHaveFreePass = (numPlayersWithFreePass >= players.length);
...
function startTestCycle() {
	...
	var msg = 'Het is tijd voor de test! Typ je naam, zorg dat niemand meekijkt, en vul de vragen zo goed mogelijk in.';
...
function finishTestCycle() {
	...
	var p = players[curTestTakerIndex]
	p.timeTaken = new Date() - p.testStartTime // in milliseconds, should always be positive
...
function displayRandomTestStatistic() {
	var statistics = ['worstResult', 'bestResult', 'averageResult', 'doubleResults']
	...
	var randStatistic = statistics[Math.floor(Math.random() * statistics.length)]
	var res = testStatistics.allResults
	...
	switch(randStatistic) {
		...
		case 'averageResult':
			...
			var roundedAvg = Math.round((avg/res.length) * 10)/10
...
function startTestResultsPhase() {
	...
	var lastRound = (curRound == numRounds - 1)
	var eliminatedPlayerName = getPlayerWithWorstTestResult();

	var msg = '<p>De afvaller is ... <strong class="eliminatedPlayer">' + eliminatedPlayerName + '</strong></p><p>' + displayRandomTestStatistic() + '</p>';
	if(lastRound) {
	...
	var messages = ['door ons team van trolexperts ...', 'moeilijke berekeningen worden gedaan ... ', 'ah, eindelijk een antwoord: ']
	var messageRevealCounter = 0;				
	...
	// TO DO/IDEA: once in a while, give some other information, such as how BAD the worst score was exactly or how GOOD the best one was.
	// TO DO: Style invalid inputs better using the :invalid pseudo-selector
	// TO DO: Pick _fewer_ questions per task on larger player counts. (Otherwise the test phase will just take too long.)
	// TO DO: Show a new hint on EVERY question to the mole? (Ensure he knows ALL upcoming tasks?)
...
function startGameOverPhase() {
	...
	// (TO DO: the winner from the test was already found on the last screen - might want to change that order)
	var moleWasFound = false;
	for(var i = 0; i < players.length; i++) {
		if(i == moleIndex) { continue; }
	...
	var msg = '<p>Gefelicteerd ... trol!</p><p>Niemand heeft op jou gestemd in de laatste test, dus jij hebt gewonnen!</p><p>Je mag jezelf nu op spectaculaire wijze onthullen!</p>';
	if(moleWasFound) {
	...
	for(var i = 0; i < testStatistics.resultsWithPlayerAttached.length; i++) {
		var r = testStatistics.resultsWithPlayerAttached[i];
		var player = players[r.playerIndex]
		var score = r.score
