...
const tools = {
    ...
    getClaimResult(card, isReversed)
    {
    ...
    getClosestPile(card, piles, isReversed)
    {
...
function generate(o = o1, includeReverse = false)
{
    ...
    const types = ["blank"];
    const typesCached = [];
    if(includeReverse) { types.push("reverse"); typesCached.push("reverse"); }
    ...
    while(cardsWithoutDuplicates.length > 0)
    {
        ...
        const card = tools.getFirstCard(cardsWithoutDuplicates, isReversed);
