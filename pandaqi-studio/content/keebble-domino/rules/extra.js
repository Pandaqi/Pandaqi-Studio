...
class Domino {
    constructor(x = -1, y = -1, values = [], orient = 0)
    {
        this.x = x;
        this.y = y;
...
class Board {
    ...
    loadSpriteSheet()
    {
        return new Promise((resolve, reject) => {
            ...
            spriteSheet.src = "/keebble-domino/assets/ambigram_letters.webp"
        })
    }

    getNewWords(newCells = [])
    {            
        const params = {
            x: 0, 
            y: 0, 
    ...
    async visualize()
    {
        ...
        const ctx = canv.getContext("2d");
        const cellSize = Math.floor(this.widthPixel / this.width, this.heightPixel / this.height);
        const params = {
            ctx: ctx,
            cs: cellSize,
            sprites: spriteSheet,
...
async function generate() {
    ...
    if(play)
    {
        ...
        const newWords = board.getNewWords(cellsUsed.flat());
        let points = 0;
