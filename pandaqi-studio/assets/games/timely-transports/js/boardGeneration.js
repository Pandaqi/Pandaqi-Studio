...
class BoardGeneration extends Scene
{
	...
	create(config) 
	{
		...
		// so zoom in
		var planesAndTrainsDisabled = (DIFFICULTY_LEVELS[this.cfg.difficulty] < 2);
	...
	createTerrain() {
		...
		for(var x = 0; x < this.cfg.widthInCells; x++) {
			...
			for(var y = 0; y < this.cfg.heightInCells; y++) {
				...
				var terrainType = 'water'
				if(noiseVal > this.cfg.waterLine) {
					terrainType = 'land'
				}

				var isWater = (noiseVal <= this.cfg.waterLine)
		...
		for(var x = 1; x < this.cfg.widthInCells; x++) {
			for(var y = 1; y < this.cfg.heightInCells; y++) {
				...
				var noiseVal = noise.simplex2(x*z, y*z);
	...
	isInRulesMargin(x,y) {
		...
		var width = 3 + this.cfg.numPlayers;
		var height = 5 + this.cfg.numPlayers;
	...
	placeCities() {
		...
		for(var i = 0; i < numCities; i++) {
			...
			var possibleSpacesLeft = (this.dockLocations.length > 0 || this.landLocations.length > 0)
	...
	moveCity(ind, c) {
		...
		var cityTooClose = false;
		do {
			var pos = this.dockLocations.splice(0, 1)[0];
			...
			for(var i = 0; i < this.cities.length; i++) {
				...
				var dist = Math.abs(otherCity.x - x) + Math.abs(otherCity.y - y);
	...
	createConnections() {
		...
		var pathTypesCovered = { 'boat': 0, 'road': 0, 'rail': 0 };
		...
		for(var i = 0; i < this.cities.length; i++) {
			...
			var tempPathLength = maxPathLength;
			var tepmConnectionProbability = connectionProbability;
			...
			for(var j = (i+1); j < this.cities.length; j++) {
				var otherCity = this.cities[j];

				// first check if there is a path over water
				var pathOverWaterExists = false;
				var pathWater = this.getPath('water', null, curCity, otherCity);
				...
				var pathType = Math.random() < 0.5 ? 'road' : 'rail';
				if(planesAndTrainsDisabled) { pathType = 'road'; }

				var pathLand = this.getPath('land', pathType, curCity, otherCity);
			...
			for(var c = 0; c < possibleConnections.length; c++) {
				if(c > 0 && Math.random() > tepmConnectionProbability) {
					break;
				}

				var conn = possibleConnections[c];
				var otherCity = conn.city
		...
		// var airportProbability = 0.1
		this.numCitiesInGroup = {};
		var airportsPlaced = 0;

		var citiesCopy = this.cities.slice();
		...
		if(planesAndTrainsDisabled) {
			...
			if(airportsPlaced <= 1) {
				...
				var prevIndex = null;
				var numAirportsToPlace = 2 - airportsPlaced;
				for(var i = 0; i < numAirportsToPlace; i++) {
					var randIndex
					do {
		...
		for(pathType in pathTypesCovered) {
			while(pathTypesCovered[pathType] < this.cfg.minPathsRequiredOfEachType) {
				...
				for(var i = 0; i < this.cities.length; i++) {
					...
					for(var j = (i+1); j < this.cities.length; j++) {
						...
						var path = this.getPath(terrainType, pathType, cityA, cityB, true);
	...
	addPathToBoard(conn) {
		var halfwayPoint = Math.floor(0.5*conn.path.length);

		for(var p = 0; p < conn.path.length; p++) {
			var point = conn.path[p];
			var cell = this.terrain[point[0]][point[1]];
	...
	determineCityDesires() {
		...
		while(goodsList.length < desiredTotalNumberGoods) {
			var randGood = this.getRandomGood(totalGoodProbability);
	...
	determineCapitalValues() {
		...
		var highestValue = -1;
		for(var i = 0; i < this.cfg.numPlayers; i++) {
			var c = this.cities[i];
			var sum = 0;
	...
	cityHasRoomForGood(i, name) {
		...
		for(var i = 0; i < city.wantedGoods.length; i++) {
	...
	visualizeGame() {
		...
		for(var x = 0; x < this.cfg.widthInCells; x++) {
			for(var y = 0; y < this.cfg.heightInCells; y++) {
				var rect = new Geom.Rectangle(oX + x*cs, oY + y*cs, cs, cs);
				var noiseVal = this.terrain[x][y].val;
				...
				if(this.terrain[x][y].isForest) {
					var frame = this.checkNeighbourForestFrame(x, y);
					...
					var forest = this.add.sprite(rect.x + 0.5*cs, rect.y + 0.5*cs, imageKey, frame);

					var randSizeChange = Math.random()*8 - 4;
					var randOffsetChangeX = Math.random()*0.2-0.1, randOffsetChangeY = Math.random()*0.2-0.1;
				...
				if(this.cfg.printFriendly) {
					if(noiseVal <= waterLine) {
						var seaSprite = this.add.sprite(rect.x, rect.y, 'seaprintfriendly');
		...
		for(var x = this.cfg.widthInCells-1; x >= 0; x--) {
			for(var y = this.cfg.heightInCells-1; y >= 0; y--) {
				this.checkUglyBox(x,y);
			}
		}
		...
		for(var i = 0; i < this.waterEdges.length; i++) {
			var e = this.waterEdges[i];

			var line = new Geom.Line(oX + e[0]*cs, oY + e[1]*cs, oX + e[2]*cs, oY + e[3]*cs);
		...
		for(var x = 0; x < this.cfg.widthInCells; x++) {
			for(var y = 0; y < this.cfg.heightInCells; y++) {
				...
				for(var t = 0; t < curCell.pathTypes.length; t++) {
					// count connections to determine which sprite to display
					// (and how to rotate it)
					var pathType = curCell.pathTypes[t]
					var connectionInfo = this.countConnections(x, y, pathType);

					var spriteFrame = (connectionInfo.count - 1)
					...
					var spriteRotation = connectionInfo.rotation
					...
					if(curCell.halfwayPointOfPath) {
						...
						var shouldPlaceSprite = true;
						for(var xx = 0; xx < 3; xx++) {
							for(var yy = 0; yy < 3; yy++) {
								var pos = [x-1+xx, y-1+yy];
								if(xx == 1 && yy == 1) { continue; }
								if(this.outOfBounds(pos[0], pos[1])) { continue; }

								var cell = this.terrain[pos[0]][pos[1]];
						...
						if(shouldPlaceSprite) {
							var sprite = this.add.sprite(rect.x, rect.y, 'inbetween_space');
		...
		// cities
		var radius = this.cfg.cellSize*0.5*1.5;
		var textCfg = 
			{
				fontFamily: 'Rowdies', 
		...
		for(var i = 0; i < this.cities.length; i++) {
			...
			var xPos = oX + (c.x + 0.5)*cs, yPos = oY + (c.y + 0.5)*cs
			var circ
			...
			var yOffset = -6
			var txt = this.add.text(circ.x, circ.y + 2*radius + yOffset, CITY_NAMES[i], textCfg);
			txt.setOrigin(0.5, 0.5);
			txt.depth = 10000;

			// display wanted goods
			var wg = c.wantedGoods;
			var iconSize = 2*radius;
			for(var g = 0; g < wg.length; g++) {
				var good = wg[g];
				var spritesheetFrame = GOODS[good.name].frame
				var value = good.value;

				var sprite = this.add.sprite(circ.x + (-wg.length*0.5 + g + 0.5)*iconSize, circ.y - 2*radius, 'goods', spritesheetFrame);
				sprite.displayWidth = sprite.displayHeight = 2*radius;
				sprite.depth = 10000 + 1;

				var text = this.add.text(sprite.x, sprite.y, value, goodNumberCfg);
				text.depth = 10000 + 1;
			}

			// Airport is indicated by underline (with arrow pattern) to city name!
			if(c.airport) {
				var pos1 = txt.getBottomLeft()
				var pos2 = txt.getBottomRight()
				var thickness = Math.max(radius*0.5, 8)
				var yOffset = -5
				var rect = new Geom.Rectangle(pos1.x, pos1.y + yOffset, (pos2.x-pos1.x), thickness)
				...
				var tileSprite = this.add.tileSprite(rect.x, rect.y, rect.width, rect.height, 'airplane_tile_sprite');
			...
			if(this.cfg.cityBonus) {
				...
				if(b > 0) {
					var txt = this.add.text(circ.x, circ.y, '+' + b, bonusTxtCfg);
					txt.setOrigin(0.5, 0.5);
					txt.depth = 10000;
				}
			}
		...
		var txt1 = this.add.text(oX + margin, oY + margin, '"' + this.cfg.jungleName + '"', textCfg);
		txt1.setOrigin(0,0);
		txt1.depth = 20000;

		var playerText = this.cfg.numPlayers + ' Player'
		if(this.cfg.numPlayers > 1) { playerText += 's'; }

		var txt2 = this.add.text(oX + margin, oY + margin + fontSize, playerText, textCfg);
		txt2.setOrigin(0, 0);
		txt2.depth = 20000;

		var txt3 = this.add.text(oX + margin, oY + margin + fontSize*2, this.cfg.difficulty, textCfg);
		txt3.setOrigin(0,0);
		txt3.depth = 20000;	

		// a rectangle behind the text, to make it legible (and look a bit like a map legend)
		var maxWidth = Math.max(txt1.getRightCenter().x, txt3.getRightCenter().x) - margin;
		var maxHeight = txt3.getBottomRight().y - (txt1.getTopLeft().y - 0.5*margin) + 0.5*margin;
		var rect = new Geom.Rectangle(txt1.getLeftCenter().x - 0.5*margin, txt1.getTopLeft().y - 0.5*margin, maxWidth + margin, maxHeight + margin);

		var overlayGraphics = this.add.graphics();
		...
		if(this.cfg.rulesReminder) {
			var overview = this.add.sprite(rect.x, rect.y + maxHeight + margin, 'rules_overview');
			var ratio = (overview.displayWidth / overview.displayHeight);
	...
	countConnections(x, y, pathType = null) {
		...
		var hasBend = false;
		for(var a = 0; a < 4; a++) {
			var nbX = x + nbs[a][0], nbY = y + nbs[a][1];

			if(this.outOfBounds(nbX, nbY)) {
				continue;
			}

			var nbCell = this.terrain[nbX][nbY];
		...
		if(sum == 3) {
			var neighboursInARow = 0;
			var sequenceStarter = -1;
			var iterator = 0;
			while(true) {
				var nbX = x + nbs[iterator][0], nbY = y + nbs[iterator][1];

				var addNeighbour = false;
				if(!this.outOfBounds(nbX, nbY)) {
					var nbCell = this.terrain[nbX][nbY];
					if(this.validNeighbour(curCell, nbCell, pathType)) {
						addNeighbour = true;
	...
	{
		var matchingPathType = false;

		if(pathType == null) {
			for(var i = 0; i < a.pathTypes.length; i++) {
	...
	checkNeighbourForestFrame(x, y) {
		...
		for(var a = 0; a < dirs.length; a++) {
			var nbX = x + dirs[a][0], nbY = y + dirs[a][1]
	...
	distanceToBounds(x,y) {
		var left = x
		var right = this.cfg.widthInCells - 1 - x
		var top = y
		var bottom = this.cfg.heightInCells - 1 - y
