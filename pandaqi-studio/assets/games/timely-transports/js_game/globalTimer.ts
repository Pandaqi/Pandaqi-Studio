...
export default class GlobalTimer {
    ...
    globalTimerDuration: any;
    timer: Timer;
    soloTimer: Timer;
    visualTimer: HTMLDivElement;
    intervalHandler: any;
	...
	convertSecondsToString(s: number) {
        s = Math.ceil(s);

		let minutes:string|number = Math.floor(s / 60);
		if(minutes < 10) { minutes = "0"+minutes; }

		let seconds:string|number = Math.ceil(s) % 60
