import { REAL_LETTER_FREQS, LETTERS, CELLS, KEEBBLE_LETTER_VALUES } from "./gameDictionary"
...
export default class Generator {
    ...
    async start()
    {
        ...
        const fontURL = "/keebble/assets/ArbutusSlab-Regular.woff2";
    ...
    async createDominoes(userConfig)
    {
        ...
        const customPageLayoutDims = { x: 3, y: 8 }
        if(userConfig.bigSize) { 
            customPageLayoutDims.x = 2;
            customPageLayoutDims.y = 5;
        }

        const customCardDims = { x: 2, y: 1 }; // { x: 1380, y: 690 };
        ...
        resLoader.planLoad("letter", { path: baseAssetDir +  "ambigram_letters.webp", frames: { x: 8, y: 6 } });
        resLoader.planLoad("cell", { path: baseAssetDir +  "special_cells.webp", frames: { x: 8, y: 2 } });
        resLoader.planLoad("decorations", { path: baseAssetDir +  "domino_decorations.webp", frames: { x: 8, y: 1 } });
        ...
        const visualParams = {
            ...
            letterValues: {
                include: true,
                color: "dynamic",
                fontSize: 0.075*cardSize.height,
                width: 0.2*0.5*cardSize.width,
            ...
            outlineColor: "#000000",
            outlineWidth: 0.05*cardSize.height,

            innerSquare: {
                include: true,
                color: "dynamic",
                width: 0.01*cardSize.height,
                scale: 0.95
            },

            dominoPartSeparator: {
                include: false,
                margin: 0.1*cardSize.height,
                color: "#AAAAAA",
                width: 0.01*cardSize.width
            },
