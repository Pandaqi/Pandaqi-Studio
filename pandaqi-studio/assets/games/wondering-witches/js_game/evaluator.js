...
export default class PotionEvaluator {
    ...
    usePotion(fixedInput = null, earlyCutoff = false) 
    {
        ...
        data.totalEffectResult = this.config.shuffle(data.totalEffectResult).join("");

        const weWon = this.weWon(data);
        if(weWon) {
            data.growingResult = '';
            data.totalEffectResult = '<p class="winMessage">Congratulations! You won!</p>';
        }

        if(headerless) { return weWon; }

        this.config.classes.UI.onPotionResult({ text: data.growingResult + data.totalEffectResult });
        return weWon;
    }
    ...
    checkInvestigativeEffects(params)
    {
        const e = params.effect;
        if(e == "Liar") {
