...
export default class Option 
{
    ...
    determineDetails()
    {
        ...
        if(tp == "po_letter")
        {
            ...
            const num = possibleValues[Math.floor(Math.random() * possibleValues.length)];
            if(num > 0) { this.twoPartValue = "+" + this.twoPartValue; }
            else { this.twoPartValue = num.toString(); }
        }
    ...
    execute()
    {
        ...
        if(isPowerup)
        {
            this.game.score.update(this.playerName, CONFIG.pointsForPickingPowerup);
    ...
    filterLetters(list: string[], filter = [])
    {
        const arr = [];
