...
class BoardGeneration extends Scene
{
	...
	finalizePointTypes() {
		...
		if(this.cfg.firstGame) {
			...
			var MISSION_NODE_RNG = Random.seedRandom(this.cfg.seed + "-missionNodes")

			// remove any missions that are impossible (because there's no relevant node on the board)
			for(var name in totalMissionNodeList) {
				var relevantNodes = totalMissionNodeList[name].relevantNodes || [];
		...
		for(name in totalNodeList) {
			var howMany = totalNodeList[name].min || 0;
	...
	checkIfNodeAllowed(p, type) {
		var nodeData = this.lists.nodes[type]
	...
	createBetterNodeCollection() {
		var NODE_RNG = Random.seedRandom(this.cfg.seed + "-nodesCollection")
	...
	addNodeToCollection(list, name, curNodeData) {
		...
		var node = this.lists.nodes[name];
		list[name] = node;
	...
	getRandomNodeOfType(what = 'category', tp, nodesList, RNG) {
		...
		for(var name in this.lists.nodes) {
			if(nodesList[name] != undefined) { continue; }

			var n = this.lists.nodes[name]
	...
	addIntermediaryPoints() {
		...
		var NODE_RNG = Random.seedRandom(this.cfg.seed + "-tinyNodes")
	...
	addLandmarks() {
		var RNG = Random.seedRandom(this.cfg.seed + "-landmarks")
	...
	addNaturalResources() {
		var RNG = Random.seedRandom(this.cfg.seed + "-naturalResources")
	...
	visualizeGame() {
		...
		if(this.finalVisualization) {
			...
			/*
			var quadLineHoriz = new Geom.Line(0, this.centerNode.y * csY, w, this.centerNode.y * csY);
			var quadLineVert = new Geom.Line(this.centerNode.x * csX, 0, this.centerNode.x * csX, h);
		...
		for(var i = 0; i < this.connectionsToDraw.length; i++) {
			...
			var line = new Geom.Line(p[0] * csX, p[1] * csY, p[2] * csX, p[3] * csY);
		...
		if(this.naturalResources != undefined) {
			...
			for(var i = 0; i < this.naturalResources.length; i++) {
				...
				/*var circ = new Geom.Circle(nr.x * csX, nr.y * csY, 0.05 * cs);
		...
		for(var i = 0; i < this.points.length; i++) {
			...
			var color, lightColor, nodeCategory;
			if(p.nodeType == 'Mission') {
			...
			var colorObject = Display.Color.IntegerToColor(color);

			// Start nodes are rectangular, all other nodes are circular
			var obj = null, sprite = null, spriteOutline = null;
			var powerDotRadius = this.cfg.powerDotRadius;
			var lineWidth = this.cfg.lineWidth;
			...
			*/
			...
			if(p.nodeType == 'Mission') {
				...
				if(this.finalVisualization) {
					...
					if(p.type == 'Center') {
						...
						var txt = this.add.text(sprite.x, sprite.y + sprite.displayHeight * 0.5 + offset, this.cfg.seed, metadataTextConfig)
			...
			for(var pd = 0; pd < powerDots.length; pd++) {
				var PD = powerDots[pd];
				var circ = new Geom.Circle(objectCenter.x + PD.x * radius * cs, objectCenter.y + PD.y * radius * cs, powerDotRadius)
			...
			if(this.finalVisualization) {
				if(p.nodeType == 'Regular' && this.lists.nodes[p.type].needsNumber) {
					...
					var numNodesOfType = this.getNumNodesOfType(this.lists.nodes[p.type].typeNeeded) * Math.min(2.0 / this.cfg.numPlayers, 0.5);
				...
				if(p.type == 'Center') {
					...
					if(this.cfg.expansions.theElectricExpansion) {
						...
						var finalNum = Math.floor(0.66 * numEnergeticNodes / this.cfg.numPlayers);
		...
		if(this.expeditionNodes != undefined) {
			...
			for(var i = 0; i < this.expeditionNodes.length; i++) {
				...
				/*
				var circ = new Geom.Circle(c[0]*csX, c[1]*csY, expeditionNodeRadius);
				...
				*/
				...
				for(var a = 0; a < s; a++) {
					...
					var circ = new Geom.Circle(xPos + scatter[0], yPos + scatter[1], expeditionSlotRadius)
		...
		if(this.areas != undefined && false) {
			var color = new Display.Color(0, 0, 0);
			...
			for(var i = 0; i < this.areas.length; i++) {
				...
				var poly = new Geom.Polygon(pointsList);
