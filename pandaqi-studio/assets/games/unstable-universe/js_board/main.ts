...
class BoardGeneration extends Scene
{
	...
	visualizeGame() 
	{
		...
		//
		const resFaultLine = resLoader.getResource("fault_line");
		...
		{
			const resNatRes = resLoader.getResource("natural_resources");
		...
		for(const p of this.points) 
		{
			...
			const resMissionNodes = resLoader.getResource("mission_nodes");
			const resRegularNodes = resLoader.getResource("regular_nodes");
			const resOutlines = resLoader.getResource("node_outlines");
			...
			if(this.finalVisualization) 
			{
				const resDayNight = resLoader.getResource("daynight_icons");
		...
		if(this.intermediaryPoints) 
		{
			...
			const resTinyNodes = resLoader.getResource("tiny_nodes");
		...
		if(this.expeditionNodes) 
		{
			...
			const resExpeditionNodes = resLoader.getResource("expedition_nodes");
			const resOutlines = resLoader.getResource("node_outlines");
		...
		if(this.landmarks) 
		{
			...
			const resLandmarks = resLoader.getResource("landmarks");
	...
	async createSecretBoard() 
	{
		...
		const resExpedition = resLoader.getResource("expedition_nodes");
