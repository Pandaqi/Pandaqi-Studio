...
export default class AI 
{
    ...
    horizontalMarks: string;
    componentPointValues: { buildings: number; people: number; path: number; resource: number; };
    specificPointValues: { people: number; criminal: number; sick: number; educated: number; animal: number; };
    ...
    getLocationNode(comp:Component)
    {
    ...
    spacesMatch(pos1: { x: number; y: number; }, pos2: { x: number; y: number; })
    {
        return pos1.x == pos2.x && pos1.y == pos2.y;
    }
    
    removeBuildingAt(pos: any) {
    ...
    getRandomValidLocation(component:Component) 
    {
    ...
    convertLocationToString(pos: { x: number; y: number; }) 
    {
        return this.horizontalMarks.at(pos.x) + "" + (pos.y + 1)
    }
    
    registerAIFail(comp:Component) 
    { 
        this.removeBuildingAt(comp.getLocation());
    }
    
    calculateOptionScore(option:Option) 
    {
        let score = 0;
        for(const component of option.getComponents()) {
