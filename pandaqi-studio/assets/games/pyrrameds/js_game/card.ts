...
import { CARD_TEMPLATES, CardType, MEDICINE, PATIENTS } from "../js_shared/dict";
...
export default class Card
{
    ...
    drawPatient(vis:MaterialVisualizer, group:ResourceGroup)
    {
        ...
        if(reqsRows.length == 2) {
            anchorPositions = [
                anchor.clone().sub(new Point(0, 0.5*iconDims.y)),
                anchor.clone().add(new Point(0, 0.5*iconDims.y))
            ]
        }
