...
import { ALMOST_ACTIONS } from "./gameDictionary"
...
export default class Card {
    ...
    setupCanvas()
    {
        const size = this.config.cards.size;
        this.size = size;
        ...
        this.ctx.strokeStyle = this.config.cards.strokeStyle;
        this.ctx.lineWidth = this.config.cards.lineWidth * this.sizeSquare;

        const bgColor = this.config.inkFriendly ? this.config.cards.fillStyleInkfriendly : this.config.cards.fillStyle;
        this.ctx.fillStyle = bgColor;
        this.ctx.fillRect(0, 0, size.x, size.y);
        this.ctx.strokeRect(0, 0, size.x, size.y);

        this.gridSize = this.config.cards.grid;

        const margin = Math.min(size.x, size.y)*this.config.cards.innerMargin;
        const innerSize = new Point(size.x - margin, size.y - margin);

        const shp = this.config.tileShape
        ...
        this.innerCellSize = this.cellSize*(1.0 - this.config.cards.cells.margin);
    ...
    assignRoles()
    {
        ...
        const numTeams = this.config.numTeamsOnCodeCard;
        const numSecretTiles = this.config.numSecretTilesPerTeam;
        ...
        const almostTilesPerTeam = this.config.cards.almostTilesPerTeam;
        const actionProb = this.config.cards.actionProb;
        const addAlmostActions = this.config.addAlmostActions;
        for(let i = 0; i < numTeams; i++)
        {
            const teamKey = "team" + i;
        ...
        // add antsassins
        const numAssassins = this.config.cards.numAssassins;
        const minDistanceToAssassin = this.config.cards.minDistToAssassinTile;
    ...
    {
        const numBounds = this.config.cards.numUniqueAlmostActions;
    ...
    visualizeCells()
    {
        const ctx = this.ctx;
        const cells = this.getFlatGrid();
        const drawPattern = !this.config.inkFriendly;
        for(const cell of cells)
        {
            ctx.save();
            ...
            let strokeStyle = this.config.cards.cells.strokeStyle;
            let lineWidth = this.config.cards.cells.lineWidth * this.sizeSquare;

            let shadowColor = color;
            let shadowBlur = this.config.cards.cells.shadowBlur * this.sizeSquare;
            if(type == "secret") { 
                shadowBlur = this.config.cards.cells.shadowBlurSecretTile * this.sizeSquare; 
                shadowColor = "#000000";
                strokeStyle = "#FFFFFF";
                lineWidth *= 2;
            }
            ...
            const bgPattern = this.config.cards.bgPatterns[team];
            ...
            {
                let spriteSize = this.config.cards.actionSpriteSize*Math.min(boundingBox.width, boundingBox.height);
                if(this.config.tileShape == "triangle") { spriteSize *= 0.75; } // simply less space in triangles, so compensate
    ...
    {
        let color = this.config.cards.teamColors[team];
        if(this.config.inkFriendly) { color = this.config.cards.teamColorsInkfriendly[team]; }

        if(type == "almost")
        {
            color = this.config.cards.almostColors[team];
            if(this.config.inkFriendly) { color = this.config.cards.almostColorsInkFriendly[team]; }
        }
    ...
    {
        const ctx = this.ctx;
        const innerScale = this.config.cards.startingTeam.innerScale;
        ...
        ctx.lineWidth = this.config.cards.startingTeam.lineWidth * this.sizeSquare;
