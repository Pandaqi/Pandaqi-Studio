...
export default class Interface {
	...
	*/
	...
	showBotActionResults(actionName, squares)
	{
		var realSquares = [];
	...
	showBotActionInterface(actionName)
	{
		this.interface.innerHTML = '<p>You decided to <strong>' + this.capitalize(actionName) + '</strong>. Where?</p>';
		...
		btn.addEventListener("click", (ev) => {
			...
			var squares = [];
			for(let i = 0; i < numDropdowns; i++) {
				squares.push(document.getElementById("dropdown-" + i).value);
			}

			this.showBotActionResults(actionName, squares);
		})
		this.interface.appendChild(btn);
	}

	createBotActionButton(actionName)
	{
		var btn = document.createElement("button");
