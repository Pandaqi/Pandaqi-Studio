...
async function createWordCards(userConfig:Record<string,any>)
{
    ...
    const margin = { x: 0.05 * cardSize.x, y: 0 /*0.05 * cardSize.y*/ }
    const distanceBetweenWords = (cardSize.y - 2*margin.y) / wordsPerCard;
    ...
    for(let i = 0; i < actualNumCards; i++)
    {
        ...
        for(let a = 0; a < wordsToPutOnCard; a++)
        {
            ...
            const rectHeight = cardSize.y / wordsPerCard;
            const rectX = 0;
        ...
        const spriteSize = 0.066*cardSize.x;
