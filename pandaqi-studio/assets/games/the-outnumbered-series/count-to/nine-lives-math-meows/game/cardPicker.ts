...
{
    const cards = [];
    generateNumberCards(cards);
    generateLifeCards(cards);
    return cards;
}

const generateNumberCards = (cards) =>
{
    if(!CONFIG._settings.includeNumberCards.value) { return; }
...
const generateFullPowerDictionary = () =>
{
    const entriesToAdd = {};
...
const generateLifeCards = (cards) =>
{
    ...
    const powerDict = generateFullPowerDictionary();
