...
export default class Generator
{
    ...
    async drawCards(cards:Card[])
    {
        ...
        // cards handle drawing themselves (using any globals/cached stuff from visualizer)
        const promises = [];
        const cardsOfType = {};
        for(const card of cards)
        {
            if(!cardsOfType[card.suit]) { cardsOfType[card.suit] = []; }
            cardsOfType[card.suit].push(card);
            if(CONFIG.debugSingleCard && cardsOfType[card.suit].length > 1) { continue; }
