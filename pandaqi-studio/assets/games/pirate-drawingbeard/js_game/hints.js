...
export default {
	...
	build()
	{
		var alreadyHaveHints = (Object.keys(this.fullList).length > 0);
		...
		for(let i = 0; i < this.availableHints.length; i++) {
			var originalHint = this.availableHints[i];

			var categoryIsForbidden = !this.categories.includes(originalHint.category);
			if(categoryIsForbidden) { continue; }

			var expansionMissing = ("expansion" in originalHint) && !Config.expansions[originalHint.expansion] && !this.buildForDownload;
			if(expansionMissing) { continue; }

			var forbiddenBecauseAdvanced = ("advanced" in originalHint) && !Config.advancedHints && !this.buildForDownload;
			...
			var values = [Array(originalHint.params.length)];	
			...
			for(let p = 0; p < originalHint.params.length; p++) {
				...
				{ 
					var randValue = values[Math.floor(Config.rng.hints() * values.length)];
					values = [randValue]; 
				}
			}

			// And finally actually BUILD the hints according to these combinations
			var mustBeDifferent = ("different" in originalHint);
			for(let a = 0; a < values.length; a++)
			{
				...
				var strippedList = Map.mapList.slice();
				this.stripList(strippedList, params.hint);

				var hintIsTooPowerful = (strippedList.length <= Config.minTilesLeftPerHint);
				var hintIsTooWeak = (strippedList.length >= (Map.mapList.length - Config.minTilesRemovedPerHint));
				if(hintIsTooPowerful || hintIsTooWeak) { continue; }

				if(hint.category == "network")
				{
					var onlyNetworkTiles = strippedList.slice();
					this.stripNonNetworkTiles(onlyNetworkTiles);
					var badNetworkHint = (Math.abs(onlyNetworkTiles.length - Map.networkTiles.length) <= 2);
					if(badNetworkHint) { continue; }
				}

				var numericID = this.cachedStrippedLists.length;
	...
	reduce()
	{
		...
		var validLocations = Map.mapList.slice();

		var totalNumHints = 0;
		var categoryCounter = 0;

		var forbidGeneralHints = false;
		var forbidSwapResistantHints = false;
		...
		while(multipleSolutions)
		{
			...
			var category = this.categories[categoryCounter];
			var availableHints = this.fullListCopy[category];
			categoryCounter = (categoryCounter + 1) % this.categories.length;

			if(category == "general" && forbidGeneralHints) { continue; }

			var noHintsInCategory = (availableHints == undefined || availableHints.length <= 0);
			if(noHintsInCategory) { continue; }

			// var fewHintsInCategory = (availableHints.length <= 3);
			...
			var newHint = availableHints.pop();
			var isSwapResistant = ("swapResistant" in newHint);
			...
			if(validLocations.length > 1) {
				...
				var hintDidNothing = (prevNumSolutions == validLocations.length);
				var hintDidAlmostNothing = (Math.abs(prevNumSolutions - validLocations.length) < Config.minImpactPerHint);
				...
				var checkRedundancy = !Config.fastGeneration;
				if(checkRedundancy)
				{
					// we only check against the hints we've already decided on
					var strippedList = this.cachedStrippedLists[newHint.numericID];
					var isRedundant = this.checkIfSublist(strippedList, strippedLists);
			...
			var tooManyHintsNeeded = totalNumHints > Config.playerCount*Config.maxHintsPerPlayer;
	...
	distribute()
	{
		...
		var curPlayer = Math.floor(Config.rng.hints() * Config.playerCount);
		for(let i = 0; i < this.categories.length; i++)
		{
			var list = this.finalList[this.categories[i]];
			this.shuffle(list);
	...
	checkFairness()
	{
		...
		var minOptionsLeft = Math.round(0.4*Config.totalTileCount) - offset; // min options is the more important number
		var maxOptionsLeft = Math.round(0.8*Config.totalTileCount) + offset;

		// first we calculate how many tiles each player has left
		// and do an absolute check: must be between some min and max
		var failed = false;
		var tilesLeftPerPlayer = [];
		for(let i = 0; i < Config.playerCount; i++)
		{
			var hints = this.perPlayer[i];
			var list  = Map.mapList.slice();
			var prevNumSolutions;
		...
		var maxDifference = Math.round(0.4*Config.totalTileCount) + offset;
	...
	checkIfSublist(newList, existingLists)
	{
		...
		{
			var smallerList = newList;
			var biggerList = existingLists[i];
			...
			var curIndex = biggerList.indexOf(smallerList[0]);
			...
			for(let a = 1; a < smallerList.length; a++)
			{
				...
				var notEnoughSpaceToMakeSubset = (biggerList.length - curIndex) < (smallerList.length - a);
	...
	saveBotbeardInformation()
	{
		...
		var list = this.getBotbeardPositiveList();
		...
		if(Config.debugging)
		{
			...
			{
				var tileA = Map.getRandomTile();
				var tileB = Map.getRandomTile();
				var answerChanged = Map.testTileSwap(tileA, tileB);
	...
	{
		var list = Map.mapList.slice();
		var botHints = this.perPlayer[this.perPlayer.length-1];
