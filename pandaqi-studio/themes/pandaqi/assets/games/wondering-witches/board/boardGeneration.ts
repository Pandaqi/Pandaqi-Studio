...
import { MaterialVisualizer, shuffle, ResourceGroup, LayoutOperation, ResourceShape, Line, Vector2, Circle, Color, TextConfig, StrokeAlign, ResourceText } from "lib/pq-games"
...
export default class BoardGeneration
{
	...
	findGardenBorders(garden:Garden, borders: Line[])
	{
		...
		for(const cell of garden) {
			...
			for(let i = 0; i < nbs.length; i++)
			{
				...
				const line = new Line(
					new Vector2(myCenter.x + 0.5*dl[0]*w, 
					myCenter.y + 0.5*dl[1]*h), 
					new Vector2(myCenter.x + 0.5*dl[2]*w, 
	...
	visualizeIngredients(vis:MaterialVisualizer, group:ResourceGroup)
	{
		...
		for(const cell of this.gen.ingredients) {
			...
			const circleCenter = new Vector2(center.x + 0.35*spriteSize, center.y);
	...
	visualizeGrid(vis:MaterialVisualizer, group:ResourceGroup)
	{
		...
		for(let x = 0; x < this.cfg.xLines; x++) 
		{
			...
			const l = new Line(new Vector2(xPos, 0), new Vector2(xPos, this.cfg.fullHeight));
		...
		for(let y = 0; y < this.cfg.yLines; y++) 
		{
			...
			const l = new Line(new Vector2(0, yPos), new Vector2(this.cfg.fullWidth, yPos));
			group.add(new ResourceShape(l), opLine);
		}
		...
		for(const section of this.cfg.sections)
		{
			...
			rect.extents = new Vector2(rectSize.x - 4*edgeOffset, rectSize.y - 4*edgeOffset);
			...
			const opText = new LayoutOperation({
				pos: rect.center,
				fill: color,
				size: new Vector2(20 * fontSize, 2 * fontSize),
				pivot: Vector2.CENTER,
				stroke: "#FFFFFF",
