import { seedRandom, SettingType, Vector2 } from "lib/pq-games";
...
export const CONFIG =  
{
	...
	size: new Vector2(297*3.8, 210*3.8),

	_resources:
	{
		base: "/pirate-drawingbeard/assets/",
		files:
		{
			tile_types:
			{
			...
			// @TODO: the assets below are ALSO used by hint visualizer, but they grab it through the phaser instance---this will BREAK DOWN once I remove the Phaser dependency entirely!!!
			hint_base:
			{
				path: "hint_base.webp",
	...
	{
		this.rng = {};
		this.rng.general = seedRandom(finalSeed + "-general");
		this.rng.map = seedRandom(finalSeed + "-map");
		this.rng.hints = seedRandom(finalSeed + "-hints-" + randomSeed);
		this.rng.actions = seedRandom(finalSeed + "-actions");
	},
