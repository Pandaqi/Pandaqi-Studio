...
import { Vector2, MaterialVisualizer } from "lib/pq-games";
...
class HintVisualizer
{
	...
	async visualize(hint, vis:MaterialVisualizer)
	{
		...
		// @NOTE: This is a bit meh, but it's clean enough and I don't expect many changes to this API
	...
	// assumes all source spritesheets are also 8 wide + 256x256 pixels
	paintCanvas(hint, canvas, vis:MaterialVisualizer)
	{
		...
		let inkFriendly = vis.inkFriendly && (CONFIG._settings.createPremadeGame.value || CONFIG._settings.useRealMaterial.value)
		...
		for(let i = 0; i < layers.length; i++)
		{
			...
			{
				// @IMPROV: add possibility of NOT highlighting a specific thing
				// add custom colors to highlighting
				// add a background to row/column hints, just a different color, or merely an outline?
