...
import { TILE_DICT, DISCRETE_LISTS, BEARING_CONDITIONS, FIXED_MAP_TILES } from "./dict"
...
class MapClass
{
	...
	grid()
	{
		...
		for(const cell of this.mapList)
		{
			...
			{
				const newPos = new Vector2(cell.x + nbCoords[i].x, cell.y + nbCoords[i].y);
	...
	randomize()
	{
		...
		if(!CONFIG._settings.useRealMaterial.value) 
		{ 
	...
	network()
	{
		if(!CONFIG._settings.hints.networks.value) { return; }
	...
	{
		let dir = params.dir;
		let pos = new Vector2(params.cell.x, params.cell.y);
		let curCell;
	...
	getDirFromRotation(rot) : Vector2
	{
		return new Vector2(
	...
	getRotationFromDir(dir:Vector2) : number
	{
		if(dir.x == 1) { return 0; }
	...
	getRotationTowardsCell(target:Vector2, start:Vector2)
	{
		return this.getRotationFromDir(this.getDirTowardsCell(target, start));
	}

	getDirTowardsCell(target:Vector2, start:Vector2) : Vector2
	{
		return new Vector2(target.x - start.x, target.y - start.y);
	}
	...
	convertToRealPos(str:string) : Vector2
	{
		return new Vector2(
