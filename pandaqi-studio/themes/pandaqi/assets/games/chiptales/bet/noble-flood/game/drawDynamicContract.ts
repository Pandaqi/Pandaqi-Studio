...
import { MaterialVisualizer, ResourceImage, createContext, ResourceGroup, ResourceShape, Rectangle, Vector2, LayoutOperation, getPositionsCenteredAround, TextConfig, ColorLike, ResourceText, DropShadowEffect } from "lib/pq-games";
import { MISC, SUITS, NUMBERS_AS_STRINGS } from "../shared/dict";
...
{
    const canvSize = vis.get("cards.contractDraw.card.itemSize"); // new Vector2(750, 1050);
...
export default (vis: MaterialVisualizer, drawDetails:DrawDetails, dynDetails:DynamicDetails) : ResourceImage =>
{
    ...
    const groupDimsOuter = new Vector2(canvSize.x / numGroups);
    const groupDims = new Vector2((canvSize.x - (numGroups-1)*marginBetweenGroups) / numGroups);
    
    const positionsGlobal = getPositionsCenteredAround({ 
        pos: canvSize.clone().scale(0.5),
        num: numGroups,
        size: groupDimsOuter,
        dir: layoutDir == "horizontal" ? Vector2.RIGHT : Vector2.DOWN
    })
    ...
    let cardDims = new Vector2(maxCardSizeInGroup, maxCardSizeInGroup*1.4);
    if(layoutDir == "vertical") 
    {
        maxCardSizeInGroup = groupDims.y;
        cardDims = new Vector2(maxCardSizeInGroup / 1.4, maxCardSizeInGroup);
    }
    ...
    */
    ...
    for(let i = 0; i < numGroups; i++)
    {
        ...
        const betweenCardIconDims = new Vector2(betweenCardIconDimsRaw * cardDimsUnit);
        const overSetIconDims = new Vector2(overSetIconDimsRaw * cardDimsUnit);
        ...
        if(drawLengthFades)
        {
            const fadeDims = new Vector2(cardDims.y);
            const opFadeLeft = new LayoutOperation({
                pos: new Vector2(-0.5*groupDims.x + 0.225*fadeDims.x, 0),
                size: fadeDims,
        ...
        for(let i = 0; i < numCards; i++)
        {
            ...
            if(notFirstCard)
            {
                const shadowRectPos = pos.clone().sub(new Vector2(shadowRectOffset*cardDims.x, 0));
            ...
            if(drawGroup.discard)
            {
                const opDiscard = new LayoutOperation({
                    pos: new Vector2(0, -0.6*cardDims.y),
                    size: new Vector2(cardDims.x),
            ...
            if(notFinalCard)
            {
                const betweenPosAdjacent = pos.clone().add(new Vector2(0.5*cardDims.x - cardOverlap*cardDims.x, 0));
                const betweenPosNumeric = betweenPosAdjacent.clone();
                const hasBoth = drawGroup.adjacent && drawGroup.numeric; 
                const offset = new Vector2(0, 0.2*cardDims.y);
                if(hasBoth)
                {
        ...
        if(drawRowIcon)
        {
            const opRow = new LayoutOperation({
                pos: new Vector2(0, -0.5*cardDims.y),
