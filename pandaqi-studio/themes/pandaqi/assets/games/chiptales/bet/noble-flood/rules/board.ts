...
import { Vector2, fromArray, createContext, fillCanvas, ResourceShape, Rectangle, LayoutOperation, ResourceImage, convertCanvasToImage } from "lib/pq-games";
...
export default class Board
{
    ...
    addStartingCards(cards:Card[])
    {
        ...
        {
            const pos = new Vector2(offsetX + counter, offsetY);
    ...
    cardIsFacedown(pos:Vector2) { return this.cardsFlipped.includes(this.getPositionAsIndex(pos)); }
    ...
    getNeighborData(pos:Vector2) : AggregateCardData
    {
    ...
    getPositionsWith(prop:string, val:any) : Vector2[]
    {
    ...
    indexPointInList(list:Vector2[], pos:Vector2)
    {
    ...
    hasSequenceOfLength(callback:Function, maxLength:number, mustBeAdjacent = false)
    {
        ...
        while(optionsToExtend.length > 0)
        {
            ...
            // @NOTE: this is REALLY EXPENSIVE to recalculate all the time; but I see no cleaner/easier way to keep the original lists nice positions, but give cards to callback function?
            const optionAsCards = [];
    ...
    async draw(sim:InteractiveExampleSimulator)
    {
        ...
        const cardSizePossibleMove = cardSizeWithMargin.clone().sub(new Vector2(32));

        const mapBounds = new Vector2(sizeX * this.map.length, sizeY * this.map[0].length);
