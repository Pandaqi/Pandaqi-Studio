...
import { Vector2, shuffle, BalancedDictionaryPicker, fromArray, Bounds, assignGridNeighbors, GridNeighborType, initNumberRange, floodFill } from "lib/pq-games";
...
const determineIconDistribution = (size:Vector2) =>
{
    ...
    for(const [type, freq] of Object.entries(numPerType))
    {
        ...
        if(prePass == "floodfill") 
        {
            ...
            while(numPlaced < numWanted && numLeft > minGroupSize && possibleStartingCells.length > 3)
            {
                ...
                const group = floodFill(ffParams) as Cell[];
            ...
            const cols = shuffle(initNumberRange(0,gridDims-1));
            const rows = shuffle(initNumberRange(0,gridDims-1));
...
const createCells = (size:Vector2, list:number[]) =>
{
    const arr = [];
    ...
    for(let x = 0; x < size.x; x++)
    {
        ...
        for(let y = 0; y < size.y; y++)
        {
            ...
            const cell = new Cell(new Vector2(x,y), numIcons);
            arr[x].push(cell);
            counter++;
        }
    }
...
const getDistToEdge = (cell:Cell, size:Vector2) : number =>
{
...
const pickStartingPositions = (grid:Cell[][], size:Vector2) =>
{
    const cells = shuffle(grid.flat());
