...
import { Vector2, Line, MaterialVisualizer, shuffle, Rectangle, LayoutOperation, ResourceShape, TextConfig, StrokeAlign, ResourceText } from "lib/pq-games"
...
export default class BoardGeneration
{
	...
	terrain: CellData[][]
	dockLocations: Vector2[]
	landLocations: Vector2[]
	waterEdges: Line[]
	cities: CityData[]
	...
	createTerrain() 
	{
		...
		for(let x = 0; x < this.cfg.widthInCells; x++) 
		{
			for(let y = 0; y < this.cfg.heightInCells; y++) 
			{
				...
				for(let n = 0; n < 4; n++) {
					const newPos = new Vector2(x + neighbors[n][0], y + neighbors[n][1]);
					...
					if(!this.outOfBounds(newPos) && this.getCell(newPos).isWater) {
						...
						this.waterEdges.push(new Line(
							new Vector2(corner[0], corner[1]),
							new Vector2(corner[0] + orthoVec[0], corner[1] + orthoVec[1])
						));
					}
				}
	...
	disallowCitiesInRadius(pos:Vector2) 
	{
		...
		for(let a = -r; a <= r; a++) 
		{
			for(let b = -r; b <= r; b++) 
			{
				const newPos = new Vector2(pos.x + a, pos.y + b);
	...
	async visualizeGame(vis:MaterialVisualizer) 
	{
		...
		for(let x = 0; x < this.cfg.widthInCells; x++) 
		{
			...
			{
				const pos = new Vector2(x,y);
				const rect = new Rectangle().fromTopLeft(new Vector2(oX + x*cs, oY + y*cs), new Vector2(cs));
				...
				if(cell.isForest) 
				{
					...
					const op = new LayoutOperation({
						pos: new Vector2(rectPos.x + 0.5*cs, rectPos.y + 0.5*cs),
						size: new Vector2(forestWidth, forestWidth*2.0),
						pivot: new Vector2(0.5 + randOffsetChangeX, 1.0 + randOffsetChangeY),
		...
		for(let x = this.cfg.widthInCells-1; x >= 0; x--) 
		{
			...
			{
				this.checkUglyBox(new Vector2(x,y));
			}
		}
		...
		{
			const start = new Vector2(oX + e.start.x*cs, oY + e.start.y*cs);
			const end = new Vector2(oX + e.end.x*cs, oY + e.end.y*cs)
			...
			for(let y = 0; y < this.cfg.heightInCells; y++) 
			{
				...
				for(let t = 0; t < curCell.pathTypes.length; t++) 
				{
					...
					const opRoad = new LayoutOperation({
						pos: new Vector2(oX + (x+0.5)*cs, oY + (y+0.5)*cs),
						size: new Vector2(cs),
						frame: spriteFrame,
					...
					if(curCell.halfwayPointOfPath) 
					{
						...
						for(let xx = 0; xx < 3; xx++) 
						{
							...
							{
								const pos = new Vector2(x - 1 + xx, y - 1 + yy);
						...
						if(shouldPlaceSprite) 
						{
							...
							const op = new LayoutOperation({
								...
								size: new Vector2(this.cfg.cellSize*0.75),
								pivot: Vector2.CENTER,
								depth: y + 0.5
							});
							group.add(res, op);
		...
		for(let i = 0; i < this.cities.length; i++) 
		{
			...
			const opText = new LayoutOperation({
				...
				size: new Vector2(10,2).scale(textConfig.size),
				pivot: Vector2.CENTER
			});

			const opSprite = new LayoutOperation({
				pos: new Vector2(xPos, yPos), 
				size: new Vector2(radius*4),
				pivot: Vector2.CENTER,
				depth: yPos + 1.1
		    });
			...
			opTextCity.pos = new Vector2(xPos, yPos + 2*radius + yOffset);
			...
			for(let g = 0; g < wg.length; g++) 
			{
				...
				const pos = new Vector2(xPos + (-wg.length*0.5 + g + 0.5)*iconSize, yPos - 2*radius);
				const resGood = vis.getResource("goods");
				const opGood = new LayoutOperation({
					pos: pos,
					size: new Vector2(2*radius),
			...
			if(c.airport) 
			{
				const pos1 = new Vector2(opTextCity.pos.x - 0.5*textConfig.size, opTextCity.pos.y + 0.5*textConfig.size);
				const pos2 = new Vector2(opTextCity.pos.x + 0.5*textConfig.size, opTextCity.pos.y + 0.5*textConfig.size);
				const thickness = Math.max(radius*0.5, 8)
				const yOffset = -5;

				const rectPos = new Vector2(pos1.x, pos1.y + yOffset);
				const rectSize = new Vector2((pos2.x-pos1.x), thickness);

				let posStart = rectPos.clone();
				const numRepeats = 4;
				const sizeFinal = rectSize.clone();
				const sizePerRepeat = new Vector2(sizeFinal.x / numRepeats);
			...
			if(this.cfg.cityBonus) 
			{
				...
				if(b > 0) 
				{
					...
					const opText = new LayoutOperation({
						pos: new Vector2(xPos, yPos),
						size: new Vector2(2*textConfigBonus.size),
						pivot: Vector2.CENTER,
						depth: 10000,
		...
		const anchorPos = new Vector2(oX + margin, oY + margin);
		const opTextDetails1 = new LayoutOperation({
			pos: anchorPos,
			size: new Vector2(15*textConfigDetails.size, 2*textConfigDetails.size),
			fill: color,
			depth: 20000
		});
		...
		opTextDetails2.pos = anchorPos.clone().add(new Vector2(0, fontSize));
		...
		opTextDetails3.pos = anchorPos.clone().add(new Vector2(0, 2*fontSize));
		...
		const rectPos = new Vector2(margin);
		const rectSize = new Vector2(maxWidth + margin, maxHeight + margin);
		const rect = new Rectangle().fromTopLeft(rectPos, rectSize);
		...
		if(this.cfg.rulesReminder) 
		{
			...
			const op = new LayoutOperation({
				pos: new Vector2(rectPos.x, rectPos.y + maxHeight + margin),
				size: new Vector2(sizeX, sizeX / ratio),
				depth: 20000 - 1,
			});
			group.add(res, op);
		}
	...
	checkUglyBox(pos:Vector2) 
	{
		...
		const otherCorner = new Vector2(pos.x + 1, pos.y + 1);
		...
		for(let i = (curCell.pathTypes.length-1); i >= 0; i--) 
		{
			...
			for(let ooX = 0; ooX < 2; ooX++) 
			{
				...
				{
					const tempPos = new Vector2(pos.x + ooX, pos.y + ooY);
			...
			for(let rot = 0; rot < 4; rot++) 
			{
				...
				{
					const tempPos = new Vector2(cell.x + externalConns[ec % 4][0], cell.y + externalConns[ec % 4][1]);
				...
				cell = new Vector2(cell.x + orthoVec[0], cell.y + orthoVec[1]);
			}
		}
	}

	countConnections(pos:Vector2, pathType:PathType = null) : ConnectionQueryData
	{
		...
		for(let a = 0; a < 4; a++) 
		{
			const nbPos = new Vector2(pos.x + nbs[a].x, pos.y + nbs[a].y);
		...
		if(sum == 3) 
		{
			...
			while(true) 
			{
				const nbPos = new Vector2(pos.x + nbs[iterator].x, pos.y + nbs[iterator].y);
	...
	checkNeighbourForestFrame(pos:Vector2) 
	{
		// if any neighbour is a forest (which already has a frame), copy that
		let dirs = shuffle( [new Vector2(-1,0), new Vector2(0,-1), new Vector2(0,1), new Vector2(1,0)] );
		for(const dir of dirs) 
		{
			const nbPos = new Vector2(pos.x + dir.x, pos.y + dir.y);
			if(this.outOfBounds(nbPos)) { continue; }
