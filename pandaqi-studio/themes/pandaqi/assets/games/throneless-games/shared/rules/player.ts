import { fromArray, shuffle, ResourceGroup, ResourceImage, LayoutOperation, Vector2, ResourceText } from "lib/pq-games";
...
export default class Player
{
    ...
    async draw(sim:InteractiveExampleSimulator, params:Record<string,any> = {}) : Promise<ResourceGroup>
    {
        ...
        const opText = new LayoutOperation({
            pos: params.pos.clone().add(new Vector2(0, 0.6*params.size.y)),
            size: new Vector2(params.size.x*5, params.size.y),
            fill: "#000000",
            pivot: Vector2.CENTER
        })
        ...
        if(params.drawSeat)
        {
            const pos = opText.pos.clone().add(new Vector2(0.65*params.size.x, 0));
            const size = new Vector2(0.45*params.size.x);
