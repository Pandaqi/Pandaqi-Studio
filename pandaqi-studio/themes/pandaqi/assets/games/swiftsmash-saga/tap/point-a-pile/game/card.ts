...
import { shuffle, MaterialVisualizer, ResourceGroup, fillResourceGroup, LayoutOperation, Color, TextConfig, ResourceText, StrokeAlign, Vector2, ResourceShape, RectangleRounded, getRectangleCornersWithOffset, TextAlign, DropShadowEffect, colorLighten, colorDarken } from "lib/pq-games";
...
export default class Card
{
    ...
    drawBackground(vis:MaterialVisualizer, group:ResourceGroup)
    {
        ...
        let colLight = colorLighten(new Color(tempData.hex), 45);
        let colDark = colorDarken(new Color(tempData.hex), 45);
        if(vis.inkFriendly)
        {
    ...
    drawType(vis:MaterialVisualizer, group:ResourceGroup)
    {
        ...
        for(let i = 0; i < 2; i++)
        {
            const opText = new LayoutOperation({
                pos: anchors[i],
                rot: i * Math.PI,
                size: new Vector2(2.0 * textConfigType.size, 0.5*vis.size.y),
