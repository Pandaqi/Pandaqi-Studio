...
import { fromArray, MaterialVisualizer, createContext, fillCanvas, TextConfig, Vector2, LayoutOperation, ResourceText, convertCanvasToImage, fillResourceGroup, ResourceGroup, ResourceImage, GrayScaleEffect, DropShadowEffect, ResourceShape, Path, Line, TintEffect, movePath, calculateCenter, calculateBoundingBox, TextAlign, Rectangle } from "lib/pq-games";
import { COLORS, CardData, SETS, TYPES, Type } from "../shared/dict";
...
export default class Card
{
    ...
    async drawForRules(vis:MaterialVisualizer)
    {
        ...
        const textPos = new Vector2(0.5*vis.size.x, 0.5*vis.size.y + 0.33*fontSize);
        ...
        const positions = [
            new Vector2(numberOffset, numberOffset),
            new Vector2(vis.size.x-numberOffset, numberOffset)
        ]
        ...
        // > type
        const typePos = new Vector2(0.5*vis.size.x, numberOffset);
        const opType = new LayoutOperation({
            pos: new Vector2(typePos.x, typePos.y),
            size: vis.size,
            fill: "#000000",
            pivot: Vector2.CENTER,
        })
        ...
        const descPos = new Vector2(0.5*vis.size.x, 0.75*vis.size.y);
        const descDims = new Vector2(0.9*vis.size.x, 0.4*vis.size.y);
        const op = new LayoutOperation({
            pos: descPos,
            size: descDims,
            pivot: Vector2.CENTER,
    ...
    drawMetaDataSingle(vis:MaterialVisualizer, group: ResourceGroup, rot = 0)
    {
        ...
        const pos = new Vector2(anchorX, 0.5*vis.size.y);
        const mainIconSize = vis.get("cards.illustration.bgSize");
        const trapHeight = vis.get("cards.icons.trapeziumHeight");
        const trapSize = new Vector2(0.5 * (1.0 - mainIconSize.x - vis.get("cards.icons.gapToIllustration")), trapHeight).scaleFactor(vis.sizeUnit);
        const shrinkFactor = vis.get("cards.icons.trapeziumShortSideShrinkFactor");
        const path = [
            new Vector2(pos.x, pos.y - 0.5*trapSize.y),
            new Vector2(pos.x + dirX * trapSize.x, pos.y - 0.5*trapSize.y*shrinkFactor),
            new Vector2(pos.x + dirX * trapSize.x, pos.y + 0.5*trapSize.y*shrinkFactor),
            new Vector2(pos.x, pos.y + 0.5*trapSize.y)
        ]
        const trapezium = new ResourceShape({ shape: new Path(path) });
        const trapHalfLine = new Vector2(pos.x + 0.5 * dirX * trapSize.x, pos.y);

        const angle = new Line(
            path[0], 
            new Vector2(pos.x + trapSize.x, pos.y - 0.5*trapSize.y*shrinkFactor)
        ).angle();
        ...
        for(let i = 0; i < 2; i++)
        {
            ...
            let thirdPoint = new Vector2(points[1].x, points[1].y + thirdPointMove*pointyEndOvershoot);
            if(i == 1) { thirdPoint = new Vector2(points[0].x, points[0].y + thirdPointMove*pointyEndOvershoot); }
            
            let fourthPoint = new Vector2(points[0].x, thirdPoint.y - thirdPointMove*0.125);
            if(i == 1) { fourthPoint = new Vector2(points[1].x, thirdPoint.y - thirdPointMove*0.125); }
            ...
            points = movePath(points, new Vector2(0, offsetDir).scaleFactor(gapIconToTrap));
            ...
            const triangle = new ResourceShape({ shape: new Path(points)});
        ...
        let numberPos = new Vector2(trapHalfLine.x, trapHalfLine.y - dirX*0.25*trapSize.y);
        let namePos = new Vector2(trapHalfLine.x, trapHalfLine.y + dirX*0.1*trapSize.y);
        ...
        if(addUnderline)
        {
            ...
            if(rot == 0) { translate.add(new Vector2(0, 0.5*fontSizeNumber)); }
            else { translate.add(new Vector2(0, -0.5*fontSizeNumber)); }

            const rect = new Rectangle({ extents: new Vector2(0.8*fontSizeNumber, 0.12*fontSizeNumber) });
        ...
        const textDims = new Vector2(trapSize.y, trapSize.x);
    ...
    drawPowerToCanvas(vis:MaterialVisualizer, group: ResourceGroup, id = 0)
    {
        ...
        const positions = [
            new Vector2(vis.center.x, anchorOffset),
            new Vector2(vis.center.x, vis.size.y - anchorOffset)
        ];
        ...
        const iconOp = new LayoutOperation({
            pos: new Vector2(-0.66*vis.center.x, 0), // all of this is centered around (0,0) middle of subGroup
            size: new Vector2(imgHeight),
            frame: frame,
        ...
        const textOp = new LayoutOperation({
            pos: new Vector2(0.25*vis.center.x, 0),
            size: new Vector2(0.66*vis.size.x, 2*contHeight),
            fill: "#000000",
            pivot: Vector2.CENTER
        });
