...
import { MaterialVisualizer, PdfBuilder, PageOrientation, convertCanvasToImage, Vector2, seedRandom, ResourceGroup, Rectangle, LayoutOperation, ResourceShape, TextConfig, ResourceText, StrokeAlign, Line, TextAlign } from "lib/pq-games";
...
export default class BoardGeneration
{
	...
	calculateBotbeardInformation()
	{
		...
		for(let i = 0; i < numProposeChecks; i++)
		{
			...
			const otherPos = new Vector2(tileA.x, tileA.y);
	...
	*/
	...
	prepareGrid() 
	{
		...
		let nbCoords = [new Vector2(1,0), new Vector2(0,1), new Vector2(-1,0), new Vector2(0,-1)]
		for(let x = 0; x < this.cfg.width; x++) 
		{
			for(let y = 0; y < this.cfg.height; y++) 
			{
				...
				{
					const newPos = new Vector2( x + nbCoord.x, y + nbCoord.y);
	...
	*/
	visualizeTreasureOnly() 
	{
		...
		const rect = new Rectangle().fromTopLeft(new Vector2(fX, fY), new Vector2(this.cfg.cellSize));
	...
	visualizeHintCards()
	{
		...
		const cardMargin = new Vector2(20, 20);
		const margin = new Vector2(30, 80);
		const metadataMargin = new Vector2(150, 37)
		const scale = 0.35
		const cardSize = new Vector2(1038*scale, 1074*scale);
		...
		const extraGridMargin = new Vector2(3, 30); // just to center it nicely on the card
		...
		for(let i = 0; i < this.cfg.playerCount; i++)
		{
			...
			const posHintCard = new Vector2(cardMargin.x + row*cardSize.x, cardMargin.y + col*cardSize.y);
			...
			const opMetadataText = new LayoutOperation({
				pos: new Vector2(posHintCard.x + metadataMargin.x, posHintCard.y + metadataMargin.y),
				fill: "#111111",
				size: new Vector2(cardSize.x - margin.x*2, cardSize.y)
			})
			...
			const opText = new LayoutOperation({
				pos: new Vector2(posHintCard.x + margin.x, posHintCard.y + margin.y),
				fill: "#111111",
				stroke: "#FFFFFF",
				strokeWidth: 1,
			...
			const textHeight = textConfig.size * 5;
			...
			{
				const line = new Line(new Vector2(gridPos.x + x*cs, gridPos.y), new Vector2(gridPos.x + x * cs, gridPos.y + (this.cfg.height*cs)));
				group.add(new ResourceShape(line), opLine);
			}

			for(let y = 1; y < this.cfg.height; y++)
			{
				const line = new Line(new Vector2(gridPos.x, gridPos.y + y * cs), new Vector2(gridPos.x + (this.cfg.width*cs), gridPos.y + y*cs));
				group.add(new ResourceShape(line), opLine);
			}
	...
	visualizeGame() 
	{
		...
		for(let x = 0; x < this.cfg.width; x++) 
		{
			for(let y = 0; y < this.cfg.height; y++) 
			{
				...
				const rect = new Rectangle().fromTopLeft(new Vector2(fX, fY), new Vector2(cs));
				
				const cell = this.map[x][y];
				const spritePos = new Vector2(fX + 0.5*cs, fY + 0.5*cs);
				...
				if(hasNature) 
				{
					...
					const op = new LayoutOperation({
						pos: spritePos,
						frame: frame,
						size: new Vector2(cs * natureScale),
						pivot: Vector2.CENTER,
				...
				if(hasLandmark) 
				{
					...
					const op = new LayoutOperation({
						pos: spritePos,
						size: new Vector2(cs * landmarkScale),
						frame: frame,
						pivot: Vector2.CENTER,
						rot: Math.floor(this.cfg.rng.map()*4) * 0.5 * Math.PI
					});
		...
		const opText = new LayoutOperation({
			...
			strokeWidth: 1,
			strokeAlign: StrokeAlign.OUTSIDE,
			size: new Vector2(6*fontSize, 1.5*fontSize)
		});

		for(let x = 0; x <= this.cfg.width; x++) 
		{
			const line = new Line(new Vector2(oX + x*cs, oY + 0), new Vector2(oX + x*cs, oY + this.cfg.height*cs));
			group.add(new ResourceShape(line), opTop);

			if(x == this.cfg.width) { continue; }

			const opTextTemp = opText.clone();
			opTextTemp.pos = new Vector2(oX + (x+0.5)*cs, oY + margin);
			opTextTemp.pivot = new Vector2(0.5, 0);
		...
		for(let y = 0; y <= this.cfg.height; y++) 
		{
			const line = new Line(new Vector2(oX + 0, oY + y*cs), new Vector2(oX + this.cfg.width*cs, oY + y*cs));
			group.add(new ResourceShape(line), opTop);

			if(y == this.cfg.height) { continue; }

			const opTextTemp = opText.clone();
			opTextTemp.pos = new Vector2(oX + margin, oY + (y+0.5)*cs);
			opTextTemp.pivot = new Vector2(0, 0.5);
		...
		opText.pos = new Vector2(oX + 0.5*cs + margin, oY + margin + 12);
		opText.pivot = new Vector2(0.5, 0);
		...
		for(let i = 0; i < tiles.length; i++)
		{
			let pos = new Vector2(tiles[i].x, tiles[i].y);
			let posBelow = new Vector2(pos.x, pos.y + 1);
			let posRight = new Vector2(pos.x + 1, pos.y);

			if(!this.outOfBounds(posBelow) && tiles[i].terrain != this.map[posBelow.x][posBelow.y].terrain)
			{
				const line = new Line(new Vector2(oX + posBelow.x*cs, oY + posBelow.y*cs), new Vector2(oX + (posBelow.x+1)*cs, oY + posBelow.y*cs));
			...
			{
				const line = new Line(new Vector2(oX + posRight.x*cs, oY + posRight.y*cs), new Vector2(oX + posRight.x*cs, oY + (posRight.y+1)*cs));
				group.add(new ResourceShape(line), opTopDivider);
			}
		}
		...
		if(this.cfg.debugging) 
		{
			...
			for(const category in this.hints) 
			{
				for(let i = 0; i < this.hints[category].length; i++)
				{
					...
					const op = new LayoutOperation({
						pos: new Vector2(oX + margin, oY + margin + counter*lineHeight),
						fill: "#111111",
						stroke: "#FFFFFF",
						strokeWidth: 5,
						size: new Vector2(0.5*this.cfg.pixelWidth), // just a random big text box size
						depth: 1000,
					})		
