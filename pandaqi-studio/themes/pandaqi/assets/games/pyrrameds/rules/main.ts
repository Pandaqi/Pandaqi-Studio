import { Vector2, fromArray, Bounds, createContext, fillCanvas, ResourceShape, Rectangle, LayoutOperation, ResourceImage, convertCanvasToImage, convertCanvasToImageMultiple, getMaterialDataForRulebook, shuffle } from "lib/pq-games";
...
type BoardRow = Card[]
type CardPath = Vector2[] // might make this an interface with more data attached
...
class Board
{
    ...
    getCardAt(pos:Vector2) { if(this.outOfBounds(pos)) { return null; } return this.rows[pos.y][pos.x]; }
    hasCardAt(pos:Vector2) { return this.getCardAt(pos) != null; }
    setCardAt(pos:Vector2, c:Card) { if(this.outOfBounds(pos)) { return; } this.rows[pos.y][pos.x] = c; }
    ...
    listHasMatchingPoint(list:Vector2[], needle:Vector2)
    {
    ...
    getAllPositions(hasCard = false, exclude = []) : Vector2[]
    {
        const positions = [];
        for(let y = 0; y < this.rows.length; y++)
        {
            const rowBounds = this.getRowBounds(new Vector2(0,y));
    ...
    getRandomPosition(hasCard = false, exclude = []) : Vector2
    {
        return fromArray( this.getAllPositions(hasCard, exclude) );
    }

    swapCardsAt(pos1:Vector2, pos2:Vector2)
    {
    ...
    getAllNeighborsOf(pos:Vector2) : Vector2[]
    {
        const offsetLeft = Math.round(this.getRowOffsetFromLeft(pos.y));

        const offsets = [
            new Vector2(1,0), new Vector2(-1 + offsetLeft,1), new Vector2(0 + offsetLeft,1), 
            new Vector2(-1,0), new Vector2(-1 + offsetLeft,-1), new Vector2(0 + offsetLeft,-1)
        ];
    ...
    getLowerNeighborsOf(pos:Vector2)
    {
        const offsetLeft = Math.round(this.getRowOffsetFromLeft(pos.y));

        let offsets = [ new Vector2(-1+offsetLeft,1), new Vector2(0 + offsetLeft,1) ];
        if(this.isBelowCenter(pos))
        {
            offsets = [ new Vector2(-1+offsetLeft,-1), new Vector2(0 + offsetLeft,-1) ];
        }
    ...
    isValidMove(card:Card, pos:Vector2)
    {
        // A MEDICINE card ...
    ...
    getAllPathsFrom(pos:Vector2, maxLength:number, typesAllowed:string[]) : CardPath[]
    {
        const paths = [];
        ...
        while(pathsToExtend.length > 0)
        {
            ...
            {
                dirs.push(new Vector2(path[i].x - path[i-1].x, path[i].y - path[i-1].y));
            }
            ...
            for(const nb of nbs)
            {
                ...
                if(CONFIG.rulebook.forbidBackwardMovesOnPaths)
                {
                    ...
                    const newDir = new Vector2(nb.x - lastPos.x, nb.y - lastPos.y);
    ...
    async draw(sim:InteractiveExampleSimulator)
    {
        ...
        let cardSize = new Vector2(validSizeX, validSizeX * 1.4);
        if(validSizeX > validSizeY/1.4)
        {
            cardSize = new Vector2(validSizeY / 1.4, validSizeY);
        }

        // needed to neatly center the entire thing on the canvas
        const margin = 4;
        const cardSizeWithMargin = cardSize.clone().sub(new Vector2(margin));
        const cardSizePossibleMove = cardSizeWithMargin.clone().sub(new Vector2(2*margin));
        const pyramidBounds = new Vector2(
        ...
        for(let y = 0; y < this.rows.length; y++)
        {
            ...
            for(let x = 0; x < this.rows[y].length; x++)
            {
                ...
                const realPos = new Vector2((offsetX + x) * cardSize.x, y * cardSize.y);
