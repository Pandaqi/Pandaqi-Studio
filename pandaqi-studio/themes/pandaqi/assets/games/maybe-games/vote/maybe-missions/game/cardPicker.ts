...
import { Bounds, shuffle, getAllPossibleCombinations, fromArray, getWeighted, BalancedFrequencyPickerWithMargin } from "lib/pq-games";
...
export const cardPicker = () : Card[] =>
{
    ...
    generateIdentityCards(cards);
    generateShopCards(cards);

    return cards;
}

const drawRandomResources = (combos, combosBackup) =>
{
...
const generateBaseCards = (cards) =>
{
    ...
    for(let i = 0; i < num; i++)
    {
        ...
        newCard.setResources( drawRandomResources(combos, combosBackup) );
...
const fillDynamicEntry = (s:string, needles:Record<string, any[]>) =>
{
...
const drawRandomIdentityPowers = (dict:IdentityDictSorted) =>
{
    const d = { good: "", bad: "" };
    ...
    d.good = fillDynamicEntry(dict.good[optionGood].desc, replacementOptions);
    d.bad = fillDynamicEntry(dict.bad[optionBad].desc, replacementOptions);

    return d;
}

const generateIdentityCards = (cards) =>
{
    if(!CONFIG._settings.sets.identities.value) { return; }
    ...
    for(let i = 0; i < num; i++)
    {
        ...
        secretID.setPowers( drawRandomIdentityPowers(dictSecret) );
        cards.push(secretID);

        const publicID = new Card(CardType.IDENTITY, IdentityCardType.PUBLIC);
        publicID.setPowers( drawRandomIdentityPowers(dictPublic) );
        cards.push(publicID);
    }
}

const generateShopCards = (cards) =>
{
    if(!CONFIG._settings.sets.gadgets.value) { return; }
