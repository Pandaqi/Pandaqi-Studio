import { MaterialVisualizer, createContext, ResourceGroup, LayoutOperation, Vector2, DropShadowEffect, getPositionsCenteredAround, TextConfig, ResourceText, StrokeAlign, Rectangle, BlurEffect, ResourceShape } from "lib/pq-games";
import { CardType, CardSubType, CardResourceData, CardGadgetData, CardPowerData, MissionType, IdentityCardType, VoteType, CARD_TEMPLATES, RESOURCES, MISC, MASTER_CARDS } from "../shared/dict";
...
export default class Card
{
    ...
    drawResources(vis:MaterialVisualizer, group:ResourceGroup)
    {
        ...
        const iconOffsetCross = new Vector2(0, iconDims.y * vis.get("cards.resources.iconCrossOffsetY"));
        ...
        for(let i = 0; i < 2; i++)
        {
            ...
            const anchor = new Vector2(vis.center.x, offsetY);
    ...
    drawRandomText(vis:MaterialVisualizer, group:ResourceGroup)
    {
        ...
        const textBoxDims = new Vector2(vis.size.x*0.925, vis.size.y);
    ...
    drawBlurredRect(pos:Vector2, size:Vector2, blur:number, group:ResourceGroup)
    {
    ...
    drawPowers(vis:MaterialVisualizer, group:ResourceGroup)
    {
        ...
        for(let i = 0; i < 2; i++)
        {
            ...
            const textDims = new Vector2(rectDims.x*0.95, rectDims.y);
    ...
    drawGadgets(vis:MaterialVisualizer, group:ResourceGroup)
    {
        ...
        for(let i = 0; i < 2; i++)
        {
            ...
            const anchor = new Vector2(vis.center.x, offsetY);
            const anchorText = new Vector2(vis.center.x, offsetTextY);
            const anchorLabel = new Vector2(vis.center.x, offsetLabelY);
            if(type == "red") 
            { 
