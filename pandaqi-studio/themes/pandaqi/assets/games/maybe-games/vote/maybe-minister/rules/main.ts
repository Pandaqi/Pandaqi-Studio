import { shuffle, fromArray, convertCanvasToImageMultiple, getMaterialDataForRulebook, Bounds } from "lib/pq-games";
...
async function generate(sim:InteractiveExampleSimulator)
{
    ...
    // @IMPROV: for now, we just select 1 Support card with exactly 1 Good icon
    // I probably need to ENSURE these exist, or create a better rule for starting cards! 
    // => just ensure we have 6 Support-1 cards at least, and they all have the same VoteStorage number, and we're good right?
    const startingCards = [];
