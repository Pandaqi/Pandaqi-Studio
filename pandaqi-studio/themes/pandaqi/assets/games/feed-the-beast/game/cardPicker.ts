...
import { BalancedFrequencyPickerWithMargin, shuffle, fromArray, getWeighted } from "lib/pq-games";
...
export const cardPicker = () : Card[] =>
{
    ...
    generateRecipeCards(cards);
    generateVictimCards(cards);

    return cards;
}

const generateRecipeCards = (cards) =>
{
    if(!CONFIG._settings.sets.recipeCards.value) { return; }
    ...
    for(let i = 1; i < pickers.length; i++)
    {
        ...
        {
            card.addRecipe(createMultiFoodRecipe(pickers, i, recipeData));
        }
    }
    ...
    for(const card of arr)
    {
        ...
        {
            card.addRecipe(createMultiFoodRecipe(pickers, 0, recipeData));
        }
    }
    ...
    for(const card of arr)
    {
        ...
        {
            const recipeValue = countRecipeValue(recipe);
            const foodToExclude = recipe.cost[0];
            const suitableActions = getActionsOfValue(recipeValue, availableActions, [actionsAlreadyPicked, foodToExclude].flat());
...
const createMultiFoodRecipe = (pickers:BalancedFrequencyPickerWithMargin[], maxTier:number, recipeData:RecipeData, customProb:number = -1) : RecipeList =>
{
    recipeData.total++;
    ...
    while(recipe.length < targetRecipeLength)
    {
        ...
        const value = countRecipeValue({ cost: [recipe], reward: null });
...
const countRecipeValue = (recipe:Recipe) =>
{
...
const getActionsOfValue = (val:number, availableActions:Record<string, GeneralData>, exclude:string[] = []) =>
{
...
const generateVictimCards = (cards) =>
{
    if(!CONFIG._settings.sets.saveThePrincess.value) { return; }
