...
import { MaterialVisualizer, createContext, fillCanvas, ResourceGroup, DropShadowEffect, LayoutOperation, Vector2, TextConfig, StrokeAlign, ResourceText, TextAlign, getPositionsCenteredAround, AlignValue, TextStyle, toTextDrawerImageStrings, fromArray, drawBlurryRectangle } from "lib/pq-games";
import { ACTIONS, BEASTS, CARD_TEMPLATES, FOOD, MISC, MaterialType, Recipe, RecipeList, RecipeReward, RecipeRewardType, VICTIMS } from "../shared/dict";
...
export default class Card
{
    ...
    drawBeast(vis:MaterialVisualizer, group:ResourceGroup)
    {
        ...
        if(beastData.fury || beastData.fail)
        {
            ...
            const positions = [
                offset.clone(),
                new Vector2(vis.size.x - offset.x, offset.y)
            ];
        ...
        for(let i = 0; i < positions.length; i++)
        {
            ...
            textPos.add(new Vector2(0, vis.get("beasts.modal.textPosOffset")*modalDims.y));
            ...
            if(dataKey == "state")
            {
                ...
                const opTextDetails = new LayoutOperation({
                    pos: pos.clone().sub(new Vector2(0, vis.get("beasts.modal.stateTextOffsetFromCenter")*modalDims.y)),
                    size: modalDims,
                    pivot: Vector2.CENTER,
                    fill: "#000000",
                    stroke: "#FFFFFF",
            ...
            if(dataKey == "menu")
            {
                ...
                const op = new LayoutOperation({
                    pos: pos.clone().sub(new Vector2(0, menuYOffset)),
                });
                group.add(subGroup, op);

                textPos.add(new Vector2(0, menuYOffsetText)); // move text downward as well to make space
            }
    ...
    visualizeRecipe(vis:MaterialVisualizer, iconList:RecipeList, iconDims:Vector2, maxWidth:number = -1, align = AlignValue.MIDDLE) : ResourceGroup
    {
        const overlap = 0.3;
        let totalSizeX = 0;
        ...
        for(let i = 0; i < iconList.length; i++)
        {
            ...
            groupSizes.push(new Vector2((1.0 - overlap)*option.length*iconDims.x + extraMargin, iconDims.y));
        }
    ...
    drawVictimCard(vis:MaterialVisualizer, group:ResourceGroup)
    {
        ...
        const opTextDetails = new LayoutOperation({
            ...
            size: new Vector2(vis.size.x, 2*textConfigName.size),
            fill: "#000000",
            stroke: "#FFFFFF",
    ...
    drawMenuCard(vis:MaterialVisualizer, group:ResourceGroup)
    {
        ...
        for(let i = 0; i < numRecipes; i++)
        {
            ...
            const maxIconDims = new Vector2(vis.get("cards.menu.spaceYPerRecipe") * spaceYPerRecipe);
            ...
            // @IMPROV: hardcoded is ugly, but I didn't plan ahead and return the actual dimensions from visualizeRecipe, so meh
            ...
            const costGroupPos = new Vector2(-0.5*spaceXPerRecipe + leftOffset, 0);
            const maxSpaceForCost = 0.33 * spaceXPerRecipe;
            const connectorOffset = numIngs <= 1 ? new Vector2(0.66*maxIconDims.x, 0) : new Vector2(maxSpaceForCost, 0);
            ...
            const opConnector = new LayoutOperation({
                ...
                size: new Vector2(0.435*maxIconDims.x),
            ...
            const textBoxDims = new Vector2(0.5*spaceXPerRecipe, maxIconDims.y);
            const textBoxPos = new Vector2(0.475*spaceXPerRecipe - 0.5*textBoxDims.x, 0);
            ...
            const opRecipe = new LayoutOperation({
                pos: new Vector2(vis.center.x, listTop.y + spaceYPerRecipe * (i + 0.5))
            });
    ...
    drawFoodTokenV1(vis:MaterialVisualizer, group:ResourceGroup)
    {
        ...
        for(let i = 0; i < 4; i++)
        {
            ...
            const groupDotsOp = new LayoutOperation({
                pos: vis.center.clone().add(new Vector2(Math.cos(ang), Math.sin(ang)).scale(circleRadius)),
                rot: ang,
                pivot: Vector2.CENTER
            })
