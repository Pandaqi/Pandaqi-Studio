import { Vector2, fromArray, Bounds, createContext, ResourceGroup, ResourceShape, Circle, LayoutOperation, getPositionsCenteredAround, ResourceImage, convertCanvasToImage } from "lib/pq-games";
...
export default class Board
{
    ...
    setTile(pos:Vector2, tile:Tile, pawns:number[] = []) : TileData
    {
    ...
    movePawn(idx:number, pos:Vector2)
    {
    ...
    async executeTurn(g:GameState, silent = false)
    {
        ...
        for(let i = 0; i < numMoves; i++)
        {
            ...
            const moves : Vector2[] = this.getPossibleMoves(curPos, canLeapfrog, g);
            if(moves.length <= 0)
            {
            ...
            const stayOnBoard = Math.random() <= CONFIG.rulebook.preferStayingOnBoardProb;
    ...
    doMove(newPos:Vector2, pathTraveled:Tile[], leapfrog = false, g:GameState)
    {
    ...
    getPossibleMoves(pos:Vector2, leapfrog = false, g:GameState) : Vector2[]
    {
        if(pos == OUT_OF_BOARD_POS)
        {
        ...
        const posBelow : Vector2[] = this.getPositionsBelow(pos);
        const validMoves : Vector2[] = [];
    ...
    getPossibleAddLocations() : Vector2[]
    {
        const arr : Vector2[] = [];
    ...
    getPositionsAround(pos:Vector2, mustBeEmpty = false)
    {
        const allPositions = [
            new Vector2(pos.x, pos.y - 1), // above
            new Vector2(pos.x - 1, pos.y), new Vector2(pos.x + 1, pos.y), // sideways
            new Vector2(pos.x, pos.y + 1), new Vector2(pos.x + 1, pos.y + 1) // below
        ]
    ...
    getPositionsBelow(pos:Vector2) : Vector2[]
    {
        ...
        {
            arr.push(new Vector2(xPos, yPos));
        }
        return arr;
    }
    ...
    positionToID(pos:Vector2)
    {
        return pos.x * 10 + pos.y; // pyramid will never have width/height 10 or more, so safe to do this
    }

    boardWouldBeDisconnectedIfRemoved(pos:Vector2)
    {
        const toVisit : Vector2[] = [this.startingTileData.pos];
    ...
    async draw(sim:InteractiveExampleSimulator)
    {
        ...
        const tileSize = 256;
        const canvSize = new Vector2(tileSize * maxSize);

        const ctx = createContext({ size: canvSize });
        const group = new ResourceGroup();
        const pawnRadius = 0.25*tileSize;

        const drawPawn = (idx:number, pos:Vector2) => 
        {
        ...
        const drawPawns = (indices:number[], pos:Vector2) =>
        {
            const positions = getPositionsCenteredAround({ pos: pos, size: new Vector2(1.0*pawnRadius), num: indices.length });
        ...
        for(let x = 0; x < maxSize; x++)
        {
            for(let y = 0; y < maxSize; y++)
            {
                ...
                const posReal = new Vector2((x+xOffset)*tileSize,y*tileSize);
                const tileTop = new Vector2(posReal.x + 0.5*tileSize, posReal.y);
                const tileCenter = new Vector2(posReal.x + 0.5*tileSize, posReal.y + 0.5*tileSize);
        ...
        const groupOp = new LayoutOperation({
            pos: new Vector2(0.5*canvSize.x - 0.5*tileSize, 0)
        })
