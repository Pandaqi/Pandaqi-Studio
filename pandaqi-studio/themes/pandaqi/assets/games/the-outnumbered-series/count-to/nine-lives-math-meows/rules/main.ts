...
import { shuffle, fromArray, getAllPossibleCombinationsRaw, anyMatch, getMaterialDataForRulebook, rangeInteger } from "lib/pq-games";
import { InteractiveExampleSimulator, loadRulebook } from "lib/pq-rulebook";
import Card from "../game/card";
import { CONFIG } from "../shared/config";
import { SUITS, Type } from "../shared/dict";

const numberRange = (a:number, b:number) =>
{
    const arr = [];
    for(let i = a; i <= b; i++)
    {
...
class Hand
{
    ...
    constructor(amount:number, unique = false)
    {
        this.cards = [];
        const numbers : number[] = shuffle(numberRange(1,9));
