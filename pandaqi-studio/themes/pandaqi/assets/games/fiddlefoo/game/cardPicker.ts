...
import { lerp, shuffle, clamp } from "lib/pq-games";
    ...
    if(CONFIG._settings.sets.base.value)
    {
        ...
        for(const [color,num] of Object.entries(dist))
        {
            for(let i = 0; i < num; i++)
            {
                ...
                const numNotes = getNotesForNumber(finalNum);
                cards.push(new Card(color, finalNum, "", numNotes));
            }
        }
    }

    // the special expansion cards
    if(CONFIG._settings.sets.expansion.value)
    {
        ...
        for(const [key,data] of Object.entries(SPECIAL_CARDS))
        {
            ...
            const numNotes = getNotesForNumber(finalNum);
            cards.push(new Card(finalColors.pop(), finalNum, key, numNotes));
        }
    }
...
const getNotesForNumber = (num:number) =>
{
