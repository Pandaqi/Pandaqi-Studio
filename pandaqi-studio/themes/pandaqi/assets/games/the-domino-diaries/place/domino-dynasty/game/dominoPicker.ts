...
export const dominoPicker = () =>
{
    const dominoes = [];

    generateRoleTiles(dominoes);
    generateEventTiles(dominoes);
...
const generateRoleTiles = (dominoes) =>
{
    if(!CONFIG._settings.sets.roles.value) { return; }
...
const generateEventTiles = (dominoes) =>
{
    if(!CONFIG._settings.sets.proximity.value) { return; }
...
const generateDominoes = (dominoes, set:string) =>
{
    if(!CONFIG._settings.sets[set].value) { return; }

    generateRegularDominoes(dominoes, set);
    generateMissionDominoes(dominoes, set);
}

const generateMissionDominoes = (dominoes, set:string) =>
{
    ...
    const availableScalars = filterBySet(MISSION_SCALARS, set);
    const availableRewards = filterBySet(MISSION_REWARDS, set);
    const availablePenalties = filterBySet(MISSION_PENALTIES, set);
    ...
    const allScalars = assignDynamicallyWeighted(availableScalars, MISSION_SCALARS, totalReqsNeeded);
    const allMissionIcons = assignDynamicallyWeighted(availableMissionIcons, ICONS, totalReqsNeeded);
    ...
    const allRewards = assignDynamicallyWeighted(availableRewards, MISSION_REWARDS, numRewards);

    const numPenalties = Math.round(CONFIG.generation.percMissionPenalties * numDominoes);
    const allPenalties = assignDynamicallyWeighted(availablePenalties, MISSION_PENALTIES, numPenalties);
    ...
    for(let i = 0; i < numDominoes; i++)
    {
        ...
        d.cleanUpMission();
        d.setMissionText(getBestFlavorTextFor(d));
...
const getBestFlavorTextFor = (d:Domino) =>
{
...
const generateRegularDominoes = (dominoes: Domino[], set:string) =>
{
    ...
    const numDirectedPaths = CONFIG._settings.sets.direction.value ? Math.round(CONFIG.generation.pathPercentageDirected * allPaths.length) : 0;
    ...
    const allTerrains = assignDynamicallyWeighted(possibleTerrainKeys, TERRAINS, numSquares);

    const availableIcons = filterBySet(ICONS, set);
    const allIcons = assignDynamicallyWeighted(availableIcons, ICONS, numSquares);
...
const assignDynamicallyWeighted = (list:string[], dict:Record<string,GeneralData>, num:number) =>
{
    let total = 0.0;
