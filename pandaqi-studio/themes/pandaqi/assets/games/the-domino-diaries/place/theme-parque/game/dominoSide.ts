...
export default class DominoSide
{
    ...
    drawBackground(vis:MaterialVisualizer, group:ResourceGroup)
    {
        ...
        const bgColor = colorRandomize(baseColor, vis.get("dominoes.bg.randomizationBounds"));
    ...
    drawOverlay(vis:MaterialVisualizer, group:ResourceGroup)
    {
        ...
        if(this.isScorableItem())
        {
            ...
            const op = new LayoutOperation({
                pos: new Vector2(-0.5*vis.size.x, -0.5*partHeight).add(starIconOffset),
                frame: frame,
                size: starDims,
                pivot: Vector2.CENTER
            });
            ...
            const typeIconOffset = new Vector2(0.66*starIconOffset.x, starIconOffset.y);
            const opType = new LayoutOperation({
                pos: new Vector2(0.5*vis.size.x, -0.5*partHeight).add(new Vector2(-typeIconOffset.x, typeIconOffset.y)),
