...
export const dominoPicker = () : Domino[] =>
{
    ...
    generateZooperative(dominoes);
    calculateDebugStatistics(dominoes);

    return dominoes;
}

const calculateDebugStatistics = (dominoes:Domino[]) =>
{
...
const selectMissionForDifficulty = (diffTarget:CampDiff) =>
{
...
const generateZooperative = (dominoes:Domino[]) =>
{
    if(!CONFIG._settings.sets.zooperative.value) { return; }
    ...
    for(const [key,freq] of Object.entries(dist))
    {
        ...
        for(let i = 0; i < freq; i++)
        {
            ...
            const chosenMission = selectMissionForDifficulty(wantedDifficulty);
...
const generateDominoes = (dominoes:Domino[], set:string) =>
{
    ...
    const availableAnimals = filterBySet(ANIMALS, set);
    const availableStalls = filterBySet(STALLS, set);
    const availableObjects = filterBySet(OBJECTS, set);
    const availableTerrains = filterBySet(TERRAINS, set);
    ...
    const sidesWithoutTerrain = assignSidesFollowingDistribution(numSquaresWithoutTerrain, availableOptionsWithoutTerrain, customProbabilities1);
    ...
    const sidesWithTerrain = assignSidesFollowingDistribution(numSquaresWithTerrain, availableOptionsWithTerrain, customProbabilities2);
...
const assignSidesFollowingDistribution = (numSquares:number, options:string[], customProbs:Record<string,number> = {}) : DominoSide[] =>
{
