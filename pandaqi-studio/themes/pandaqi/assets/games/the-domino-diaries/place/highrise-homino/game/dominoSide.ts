...
export default class DominoSide
{
    ...
    drawBackground(vis:MaterialVisualizer, group:ResourceGroup)
    {
        ...
        // @IMPROV: this might be replaced by giving ALL tenants a floor requirement (by default), and making THAT the full background
        if(this.hasTenant())
        {
    ...
    drawOverlay(vis:MaterialVisualizer, group:ResourceGroup)
    {
        ...
        if(this.type == ItemType.TENANTPROP)
        {
            ...
            const opStar = new LayoutOperation({
                pos: new Vector2(0, tenantDetailsY),
                size: scoreDims,
            ...
            if(numPropsToShow > 0)
            {
                ...
                for(let i = 0; i < numPropsToShow; i++)
                {
                    const opProp = new LayoutOperation({
                        pos: new Vector2((positions[i] - 0.5) * vis.size.x, tenantDetailsY),
        ...
        if(this.type == ItemType.TENANTWISH)
        {
            ...
            const anchor = new Vector2(0, tenantDetailsY);
    ...
    drawWalls(vis:MaterialVisualizer) : ResourceGroup
    {
        ...
        const FENCE_OFFSETS = [Vector2.RIGHT, Vector2.DOWN, Vector2.LEFT, Vector2.UP];

        const res = vis.getResource("misc");
        const partSize = new Vector2(vis.size.x, 0.5*vis.size.y);
