
import { SettingType, TextConfig, TextStyle, TextWeight, Vector2, Bounds, MapperPreset, CVal } from "lib/pq-games"
import { cardPicker } from "../game/cardPicker"

export const CONFIG = 
{
    _settings:
    {
        sets:
        {
            type: SettingType.GROUP,

            base:
            {
                type: SettingType.CHECK,
                value: true,
                label: "Base Game"
            },

            superPowers:
            {
                type: SettingType.CHECK,
                label: "Superpowers",
                value: false,
                remark: "An expansion with more number cards with varied or complex powers."
            },
        }
    },

    _debug:
    {
        omitFile: false, // @DEBUGGING (should be false)
        singleDrawPerType: false, // @DEBUGGING (should be false)
        onlyGenerate: false, // @DEBUGGING (should be false)
    },

    _game:
    {
        fileName: "Folly & Fortune",
    },

    // assets
    _resources:
    {    
        base: "/the-luck-legends/roll/folly-and-fortune/assets/",
        files:
        {
            goodfish:
            {
                path: "fonts/Goodfish-Regular.woff2",
            },

            goodfish_italic:
            {
                key: "goodfish",
                path: "fonts/Goodfish-Italic.woff2",
                textConfig: new TextConfig({ style: TextStyle.ITALIC })
            },

            goodfish_bold:
            {
                key: "goodfish",
                path: "fonts/Goodfish-Bold.woff2",
                textConfig: new TextConfig({ weight: TextWeight.BOLD })
            },

            antique:
            {
                path: "fonts/AntiqueNo14-Regular.woff2",
            },

            card_templates:
            {
                path: "card_templates.webp",
                frames: new Vector2(1,1)
            },

            actions:
            {
                path: "actions.webp",
                frames: new Vector2(8,5)
            },

            misc:
            {
                path: "misc.webp",
                frames: new Vector2(4,2)
            },
        },
    },

    generation:
    {
        baseCardsNum: 50,
        baseNumbersPossible: [1,2,3,4,5,6], // 0 is not included in base game---too harsh/stagnant---but is in expansions

        superCardsNum: 25,
        superNumbersPossible: [-1,0,1,3,5,6,7],

        defHealth: 2,
        numHealthPerNumber:
        {
            "-1": 6,
            0: 6,
            1: 5,
            2: 4,
            3: 3,
            4: 3,
            5: 2,
            6: 1,
            7: 0
        },
        numHealthRandomness: new Bounds(-1,1),
        numHealthReductionUnseen: -2,
        numHealthMin: 1,
        numHealthMax: 6,
    },

    _material:
    {
        cards:
        {
            picker: cardPicker,
            mapper: MapperPreset.CARD,
        }
    },

    _drawing:
    {
        fonts:
        {
            heading: "antique",
            body: "goodfish"
        },

        cards:
        {
            shared:
            {
                shadowOffset: new CVal(new Vector2(0, 0.01), "sizeUnit")
            },

            bg:
            {
                alpha: 0.1
            },

            numbers:
            {
                boxOffset: new CVal(new Vector2(0.16, 0.1725), "sizeUnit"),
                boxDims: new CVal(new Vector2(0.325), "sizeUnit"),
                fontSize: new CVal(0.175, "sizeUnit"),
                strokeWidth: new CVal(0.05, "cards.numbers.fontSize"),
                textInBoxOffset: new CVal(new Vector2(0.03), "sizeUnit")
            },

            mainNumber:
            {
                fontSize: new CVal(0.6, "sizeUnit"),
                strokeWidth: new CVal(0.04, "cards.mainNumber.fontSize"),
                pos: new CVal(new Vector2(0.5, 0.35), "size"),
                addWrittenVersion: true,
                written:
                {
                    linePos: new CVal(new Vector2(0.5, 0.125), "size"),
                    lineDims: new CVal(new Vector2(0.2), "sizeUnit"),
                    lineAlpha: 0.5,
                    fontSize: new CVal(0.058, "sizeUnit"),
                    pos: new CVal(new Vector2(0.5, 0.09), "size")
                }
            },

            power:
            {
                textPos: new CVal(new Vector2(0.5, 0.77), "size"),
                fontSize: new CVal(0.0575, "sizeUnit"),
                textDims: new CVal(new Vector2(0.7, 0.33), "size"),
                textBoxDims: new CVal(new Vector2(1.0), "sizeUnit"),

                iconOffset: new CVal(new Vector2(0.1, 0.2975), "size"),
                iconDims: new CVal(new Vector2(0.125), "sizeUnit"),
                iconAlpha: 0.825,

                shieldPos: new CVal(new Vector2(0.9, 0.5), "size"),
                unseenPos: new CVal(new Vector2(0.1, 0.5), "size")
            },

            arrow:
            {
                size: new CVal(new Vector2(0.1), "sizeUnit"), // probably just the same as power.iconDims
                composite: "overlay",
                alpha: 0.5
            }
        },
    }
}