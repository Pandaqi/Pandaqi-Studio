...
import { MaterialVisualizer, createContext, fillCanvas, ResourceGroup, LayoutOperation, InvertEffect, ResourceShape, Rectangle, Vector2, DropShadowEffect, TextConfig, ResourceText, StrokeAlign, getPositionsCenteredAround, Circle } from "lib/pq-games";
...
export default class Card
{
    ...
    drawStrengths(vis:MaterialVisualizer, group:ResourceGroup)
    {
        ...
        {
            anchorPos = new Vector2(anchorOffset.x, vis.size.y - anchorOffset.y);
            textPos = new Vector2(textOffset.x, vis.size.y - textOffset.y);
        }
    ...
    drawAction(vis:MaterialVisualizer, group:ResourceGroup)
    {
        ...
        if(this.isRooster())
        {
            // @IMPROV: might need to shrink the rooster icon itself if it looks weird with overlap now
            ...
            const opDawnIcon = new LayoutOperation({
                pos: new Vector2(0.15*vis.size.x, posHeading.y),
                size: new Vector2(1.5*textConfigHeading.size),
                pivot: Vector2.CENTER,
