...
import { createContext, DropShadowEffect, fillCanvas, getRectangleCornersWithOffset, InvertEffect, LayoutOperation, MaterialVisualizer, Rectangle, ResourceGroup, ResourceShape, ResourceText, TextConfig, TintEffect, Vector2 } from "lib/pq-games";
...
export default class Card
{
        ...
        if(this.isWildCard())
        {
            ...
            for(const key of Object.keys(SUITS))
            {
                ...
                const opRect = new LayoutOperation({
                    pos: new Vector2(0, yOffset*counter),
        ...
        for(let i = 0; i < positions.length; i++)
        {
            ...
            for(let s = 0; s < suitsToDraw.length; s++)
            {
                const opMain = new LayoutOperation({
                    pos: anchorPos.clone().add(new Vector2(drawDir * suitDims.x * s * 1.1, 0)),
            ...
            if(!isSuit)
            {
                const opText = new LayoutOperation({
                    pos: anchorPos,
                    size: new Vector2(3.0 * textConfig.size),
                    rot: (i <= 1) ? 0 : Math.PI,
                    pivot: Vector2.CENTER,
    ...
    drawCenterNumber(vis:MaterialVisualizer, group:ResourceGroup)
    {
        ...
        for(let i = 0; i < 8; i++)
        {
            ...
            const circlePos = new Vector2(Math.cos(angle), Math.sin(angle)).scaleFactor(offset);
    ...
    drawSpecial(vis:MaterialVisualizer, group:ResourceGroup)
    {
        ...
        for(let i = 0; i < headingPositions.length; i++)
        {
            ...
            const shadowEffect = new DropShadowEffect({ color: "#000000", offset: new Vector2(0,0.05).scale(textConfigHeading.size) })
