import { MaterialVisualizer, createContext, fillCanvas, ResourceGroup, LayoutOperation, TintEffect, getRectangleCornersWithOffset, TextConfig, ResourceText, Vector2, DropShadowEffect, getPositionsCenteredAround } from "lib/pq-games";
import { MISC, SPECIAL_CARDS, TEMPLATES } from "../shared/dict";
...
export default class Card
{
    ...
    drawNumbers(vis:MaterialVisualizer, group:ResourceGroup)
    {
        ...
        const opTextWritten = new LayoutOperation({
            ...
            size: new Vector2(8.0 * textConfigWritten.size),
            pivot: Vector2.CENTER,
            fill: this.getTintColor(vis)
        })
    ...
    drawIcons(vis:MaterialVisualizer, group:ResourceGroup)
    {
        ...
        const anchorPositions = [
            new Vector2(edgeOffset.x, vis.size.y - edgeOffset.y),
            new Vector2(vis.size.x - edgeOffset.x, edgeOffset.y)
        ]
        ...
        for(let i = 0; i < anchorPositions.length; i++)
        {
            ...
            const positions = getPositionsCenteredAround({ pos: anchorPos, size: iconDims, num: this.numIcons, dir: Vector2.DOWN });
