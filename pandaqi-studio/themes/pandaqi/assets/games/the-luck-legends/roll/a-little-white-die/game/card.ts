...
import { MaterialVisualizer, createContext, fillCanvas, ResourceGroup, TextConfig, ResourceText, LayoutOperation, Vector2, Color, DropShadowEffect, StrokeAlign, TintEffect, Bounds, colorDarken } from "lib/pq-games";
import { DICE_ARRANGEMENTS, DICE_POSITIONS, MISC, POWER_CARDS, WILDCARD_DATA } from "../shared/dict";
...
export default class Card
{
    ...
    drawBackground(vis:MaterialVisualizer, group:ResourceGroup)
    {
        ...
        const op = new LayoutOperation({
            pos: vis.center,
            size: new Vector2(1.75*vis.sizeUnit),
    ...
    drawNumbers(vis:MaterialVisualizer, group:ResourceGroup)
    {
        ...
        const strokeColor = colorDarken(new Color(tintColor), vis.get("cards.mainNumber.strokeDarken"));
    ...
    drawPower(vis:MaterialVisualizer, group:ResourceGroup)
    {
        ...
        if(topLeftUsed)
        {
            positions[0].add(new Vector2(0, boxDims.y));
        }

        const topRightUsed = arrangement.includes(2);
        if(topRightUsed)
        {
            positions[1].add(new Vector2(0, boxDims.y));
        }
