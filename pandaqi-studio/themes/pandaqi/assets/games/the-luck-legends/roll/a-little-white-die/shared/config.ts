
import { SettingType, Vector2, Bounds, MapperPreset, CVal } from "lib/pq-games";
import { cardPicker } from "../game/cardPicker";

export const CONFIG = 
{
    _settings:
    {
        sets:
        {
            type: SettingType.GROUP,

            base:
            {
                type: SettingType.CHECK,
                value: true,
                label: "Base Game"
            },

            wildCards:
            {
                type: SettingType.CHECK,
                label: "Wildcards",
                value: false,
                remark: "A tiny expansion that adds wildcard numbers."
            },

            powerCards:
            {
                type: SettingType.CHECK,
                label: "Power Cards",
                value: false,
                remark: "An expansion that adds special powers, for more bluffing and wild reveals."
            },
        }
    },

    _debug:
    {
        omitFile: false, // @DEBUGGING (should be false)
        singleDrawPerType: false, // @DEBUGGING (should be false)
        onlyGenerate: false, // @DEBUGGING (should be false)
    },

    _game:
    {
        fileName: "A Little White Die",
    },

    // assets
    _resources:
    {    
        base: "/the-luck-legends/roll/a-little-white-die/assets/",
        files:
        {
            rumraisin:
            {
                path: "fonts/RumRaisin-Regular.woff2",
            },

            powers:
            {
                path: "powers.webp",
                frames: new Vector2(4,4),
                loadIf: ["sets.powerCards"]
            },

            misc:
            {
                path: "misc.webp",
                frames: new Vector2(8,2)
            },
        },
    },

    rulebook:
    {
        numPlayerBounds: new Bounds(2,5),
        numCardsPerPlayer: 6,
        itemSize: new Vector2(375, 525),

        startingCardsArePerfectDice: true,
        startingCardsAreRandom: false,

        keepGuessingProb: 0.735,
        minTurnsBeforeChallenge: 2,
        maxTurnsBeforeChallenge: 10
    },

    generation:
    {
        baseNumbers: new Bounds(1,6),
        baseCardsPerNumber: 6, // so you can play with 6 people at most

        wildCardsNum: 9, 

        powerCardFreqDefault: 1,
        maxPowerCards: 12,
        powerNumbers: new Bounds(7,9),
        powerCardsPerNumber: 4,
    },

    _material:
    {
        cards:
        {
            itemSize: new Vector2(375, 525),
            picker: () => cardPicker,
            mapper: MapperPreset.CARD
        }
    },

    _drawing:
    {
        fonts:
        {
            heading: "rumraisin",
            body: "rumraisin"
        },

        cards:
        {
            bg:
            {
                alpha: 1.0
            },

            mainNumber:
            {
                fontSize: new CVal(0.5, "sizeUnit"),
                fontSizeSmall: new CVal(0.35, "sizeUnit"),
                offset: new CVal(new Vector2(0, 0.275), "size"),
                offsetSmall: new CVal(new Vector2(0,0.2), "size"),
                shadowOffset: new CVal(new Vector2(0,0.03), "cards.mainNumber.fontSize"),
                strokeDarken: 60,
                strokeWidth: new CVal(0.0175, "cards.mainNumber.fontSize")
            },

            numbers:
            {
                wackyBoxDims: new CVal(new Vector2(0.2), "sizeUnit"),
                wackyBoxDotDims: new CVal(new Vector2(0.15), "sizeUnit"),
                offsetFromCenter: new CVal(new Vector2(0.35, 0.4), "size"),
            },

            power:
            {
                offset: new CVal(new Vector2(0, 0.2), "size"),
                fontSize: new CVal(0.06, "sizeUnit"),
                iconDims: new CVal(new Vector2(0.175), "sizeUnit"),
                shadowOffset: new CVal(0.06, "cards.power.iconDims"),
                textBoxDims: new CVal(new Vector2(0.72), "sizeUnit"),
                textDims: new CVal(new Vector2(0.65, 0.4), "size")
            }
        },
    }
}