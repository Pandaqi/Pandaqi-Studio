import { Color, MaterialVisualizer, Vector2, TextConfig, shuffle, getWeighted, Rectangle, ResourceGroup, LayoutOperation, ResourceShape, Line, ResourceText, StrokeAlign, DropShadowEffect } from "lib/pq-games"
...
export class BoardGeneration
{
	...
	async draw(vis:MaterialVisualizer) : Promise<HTMLCanvasElement>
	{
		...
		cfg.cellSize = new Vector2(cfg.cellSizeX, cfg.cellSizeY);
	...
	{
		const cfg = CONFIG;
		if(cfg._settings.expansions.scrabbleScoring.value)
		{
	...
	placeWalls()
	{
		...
		if(!CONFIG._settings.addWalls.value) { return; }
	...
	getAllEdges()
	{
		...
		for(let x = 1; x < CONFIG.numCellsX; x++)
		{
			for(let y = 1; y < CONFIG.numCellsY; y++)
			{
				...
				{
					const posDown = new Vector2(x, y + 1);
					edges.push(new Edge(pos, posDown, 0.0))
				}
				
				if(x < (CONFIG.numCellsX-1))
				{
					const posRight = new Vector2(x + 1, y);
	...
	{
		if(!CONFIG._settings.expansions.specialCells.value) { return; }
	...
	visualizeGrid(vis:MaterialVisualizer, group:ResourceGroup)
	{
		...
		for(let x = 1; x < CONFIG.numCellsX; x++)
		{
			...
			const line = new Line(new Vector2(xPixels, 0), new Vector2(xPixels, vis.size.y));
		...
		for(let y = 1; y < CONFIG.numCellsY; y++)
		{
			...
			const line = new Line(new Vector2(0, yPixels), new Vector2(vis.size.x, yPixels));
			group.add(new ResourceShape(line), op);
		}
	}

	toPixelPos(pos:Vector2) : Vector2
	{
		return new Vector2(CONFIG.cellSizeX * pos.x, CONFIG.cellSizeY * pos.y);
	}

	toCenteredPixelPos(pos:Vector2)
	{
		return this.toPixelPos(pos.clone().add(new Vector2(0.5)));
	}

	toOffsetPixelPos(startPos:Vector2, offsetPos:Vector2, cellSize:Vector2)
	{
		return new Vector2(
	...
	getWonkyOffset(maxOffset = 100, cutoff = 0.66) : Vector2
	{
	...
	visualizeLetter(vis:MaterialVisualizer, group:ResourceGroup, c:Cell)
	{
		...
		const textDims = new Vector2(3*CONFIG.letterTextConfig.size);
		const op = new LayoutOperation({
			pos: pixelPos,
			size: textDims,
		...
		if(CONFIG.showLetterValues)
		{
			...
			const posBottomRight = this.toPixelPos(c.getPos().add(new Vector2(0.85)));
	...
	visualizeHand(vis:MaterialVisualizer, group:ResourceGroup, c:Cell)
	{
		...
		const backpackCellSize = new Vector2(cellX, cellY);

		const displayAsGrid = CONFIG._settings.expansions.tinyBackpacks.value
		if(displayAsGrid)
		{
			...
			for(let x = 1; x < w; x ++)
			{
				...
				const line = new Line(new Vector2(pixelX, startPos.y), new Vector2(pixelX, startPos.y + CONFIG.cellSize.y));
				group.add(new ResourceShape(line), op);
			}
			...
			for(let y = 1; y < h; y ++)
			{
				...
				const line = new Line(new Vector2(startPos.x, pixelY), new Vector2(startPos.x + CONFIG.cellSizeX, pixelY));
				group.add(new ResourceShape(line), op);
			}
		}
		...
		const textDims = new Vector2(CONFIG.cellSizeX, 2*CONFIG.handTextConfig.size);
		if(displayLettersAsGrid)
		{
			...
			for(let i = 0; i < hand.length; i++)
			{
				...
				const offsetPos = new Vector2( (col+0.5), (row+0.5) );
	...
	visualizeType(vis:MaterialVisualizer, group:ResourceGroup, c:Cell)
	{
		...
		const op = new LayoutOperation({
			pos: pixelPos,
			size: new Vector2(CONFIG.spriteSize),
			pivot: Vector2.CENTER,
