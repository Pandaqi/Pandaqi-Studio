...
import { Vector2, rangeInteger, createContext, convertCanvasToImage, getMaterialDataForRulebook, shuffle } from "lib/pq-games";
...
class Board
{
    ...
    fillRandom(options:Tile[])
    {
        ...
        for(let x = 0; x < this.size.x; x++)
        {
            ...
            for(let y = 0; y < this.size.y; y++)
            {
                ...
                let tile = null;
                if(shouldFill) { tile = options.pop(); tile.customData.pos = new Vector2(x,y); }
                this.grid[x][y] = tile;
    ...
    getTopLayerTiles() : Tile[]
    {
        ...
        const checkVector = Vector2.DOWN.clone();
        checkVector.rotate(flooredRotation * 0.5 * Math.PI).round();

        const rowVector = new Vector2(1,-1);
        rowVector.rotate(this.arrowIndex * 0.5 * Math.PI).round();

        const startCells = 
        [
            new Vector2(),
            new Vector2(this.size.x-1,0),
            new Vector2(this.size.x-1, this.size.y-1),
            new Vector2(0, this.size.y-1),
        ]

        let curCell : Vector2 = startCells[flooredRotation];
    ...
    getTilesInRow(pos:Vector2, vec:Vector2) : Tile[]
    {
    ...
    outOfBounds(pos:Vector2) : boolean
    {
    ...
    async draw(sim:InteractiveExampleSimulator, highlightedTiles:Tile[] = []) : Promise<HTMLImageElement>
    {
        ...
        const fullSize = this.size.clone().add(new Vector2(extraXSpaceNeeded,extraYSpaceNeeded)).scale(tileSize); // add extra margin for arrow tile
        const ctx = createContext({ size: fullSize });
        const csize = new Vector2(this.size.x-1, this.size.y-1).scale(0.5);
        ...
        [
            new Vector2(-1,-1),
            new Vector2(csize.x, -1 - off),
            new Vector2(this.size.x,-1),
            new Vector2(this.size.x + off, csize.y),
            new Vector2(this.size.x, this.size.y),
            new Vector2(csize.x, this.size.y + off),
            new Vector2(-1, this.size.y),
            new Vector2(-1 - off, csize.y)
        ]
    ...
    tables:
    {
        base:
        {
            icons:
            {
                config:
                {
                    sheetURL: CONFIG._resources.files.tiles.path,
                    sheetWidth: 8,
