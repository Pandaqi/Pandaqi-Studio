...
import { MaterialVisualizer, ResourceGroup, LayoutOperation, TextConfig, ResourceText, Vector2, StrokeAlign, Rectangle, Line, shuffle, fromArray, ResourceShape, Circle } from "lib/pq-games";
...
export default class Tile
{
        ...
        if(this.keyAction)
        {
            ...
            if(drawIcon)
            {
                ...
                const opIcon = new LayoutOperation({
                    pos: pos,
                    size: new Vector2(0.9*Math.min(size.x, size.y)),
    ...
    {
        const size = new Vector2(CONFIG.generation.innerRectangleSize * vis.sizeUnit);
    ...
    {
        const size = new Vector2(CONFIG.generation.gemstoneRectangleSize * vis.sizeUnit);
        const pos = vis.size.clone().sub(size).scale(0.5);
        return new Rectangle().fromTopLeft(pos, size);
    }

    getRandomPosOnLine(line:Line, forbiddenPos:Vector2[], size:Vector2)
    {
        const lineShrunk = line.clone(true);
        const minDist = 2 * size.length();
        lineShrunk.scale(new Vector2(0.8)); // to prevent overlapping with any diagonals
    ...
    drawWater(vis:MaterialVisualizer, group:ResourceGroup)
    {
        ...
        if(this.water.topRight)
        {
            ...
            subGroup.add(resMisc, opDiag);
            lines.push(new Line(new Vector2(vis.size.x, 0), topRight));
        ...
        if(this.water.bottomLeft)
        {
            ...
            subGroup.add(resMisc, opDiag);
            lines.push(new Line(bottomLeft, new Vector2(0, vis.size.y)));
        }
        ...
        if(placeDecorations)
        {
            for(const line of lines)
            {
                ...
                {
                    const op = new LayoutOperation({
                        pos: line.getRandomPositionInside(boulderLineDownscale).add(new Vector2(0, -boulderOffset)),
                        size: sizeBoulder,
        ...
        if(placeDecorations)
        {
            for(const line of lines)
            {
                ...
                if(isVert)
                {
                    const leftPos = line.getRandomPositionInside(boulderLineDownscale).add(new Vector2(-boulderOffset, 0));
                    const rightPos = line.getRandomPositionInside(boulderLineDownscale).add(new Vector2(boulderOffset, 0));
