import { shuffle, distributeDiscrete, range, rangeInteger, floodFill } from "lib/pq-games";
...
export default class BoardState
{
    ...
    getDimensions() { return CONFIG.board.sizePerSize[CONFIG._settings.boardSize.value]; }
    ...
    growRivers()
    {
        ...
        const numRiverPercentage = range(riverConfig.percentageBounds.min, riverConfig.percentageBounds.max);
    ...
    assignTypes()
    {
        ...
        const desiredAvailableScore = rangeInteger(scoreBounds.min, scoreBounds.max);
    ...
    getConnectedReachableGroups() : Cell[][]
    {
        const cells = this.getGridFlat();
        ...
        for(const cell of cells)
        {
            ...
            const group = floodFill(ffParams);

            for(const cellHandled of group)
            {
    ...
    getHoleClumps() : Cell[][]
    {
        ...
        for(const cell of cells)
        {
            ...
            const clump = floodFill(ffParams);
            for(const cellHandled of clump)
            {
                cellsHandled.push(cellHandled as Cell);
            }
