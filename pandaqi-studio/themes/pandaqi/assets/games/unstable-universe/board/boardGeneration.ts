...
import { Vector2, Line, MaterialVisualizer, seedRandom, getWeighted, shuffle, ResourceGroup, LayoutOperation, Color, ResourceShape, TextConfig, Rectangle, Circle, ResourceText, StrokeAlign, Path, colorRandomize } from "lib/pq-games"
...
type Landmark = { center:Vector2, type:string }
type IntermediaryPoint = { x: number, y: number, type:string, angle: number }
type Area = GraphNode[]
type AreaObject = { tiles: Area, center: Vector2, dist: number }
type ExpeditionNode = { area: Area, center: Vector2, type: string, slots: number }
...
export default class BoardGeneration
{
	...
	setup(vis:MaterialVisualizer)
	{
		...
		const baseRes = new Vector2(10,7);
	...
	placePoints() 
	{
		...
		const centerNodePosition = new Vector2(Math.floor(0.5*rX), Math.floor(0.5*rY));
		...
		for(let x = 0; x <= rX; x++) 
		{
			...
			for(let y = 0; y <= rY; y++) 
			{
				...
				node.setPosition( new Vector2(x - 0.5 + RNG3(), y - 0.5 + RNG3()) );
	...
	getCell(pos:Vector2) : GraphElement[]
	{
	...
	{
		const safePos = new Vector2(Math.floor(node.x), Math.floor(node.y));
		...
		for(let i = 0; i < numIterations; i++) 
		{
			...
			for(const p of this.points) 
			{
				...
				for(let xx = -range; xx <= range; xx++) 
				{
					...
					{
						const pos = new Vector2(cX + xx, cY + yy);
				...
				for(let c = 0; c < connections.length; c++) 
				{
					...
					const vec = new Vector2(p.x - p2.x, p.y - p2.y);
					...
					if(force != 0) {
						vel = new Vector2(vel.x + dir*vec.x*force, vel.y + dir*vec.y*force);
					}
				}
			...
			for(const p of this.points) 
			{
				...
				this.updateCell(new Vector2(oldCellX, oldCellY), p);
			}
		}
	}
	...
	pushPointsToEdge() 
	{
		...
		for(const p of this.points) 
		{
			...
			this.updateCell(new Vector2(oldCenterX, oldCenterY), p);
		}
	}

	updateCell(pos:Vector2, p:GraphNode) 
	{
		// add to new cell
		let cX = Math.floor(p.x), cY = Math.floor(p.y)
		...
		if(pos.x != cX || pos.y != cY) 
		{
			...
			cell = this.getCell(new Vector2(cX, cY));
			cell.push(p);
		}
	}
	...
	createPowerDot(angle:number, p:GraphNode, allAngles:number[], checkBoundaries = true) 
	{
		...
		let realPos = new Vector2(p.x + dx, p.y + dy);
	...
	relaxExpeditionNode(c:Vector2, area:Area) 
	{
		const numSteps = 100;
	...
	closestDistToResource(point:Vector2, list:Vector2[]) 
	{
	...
	closestDistToPolygonNode(point:Vector2, poly:Vector2[]|GraphNode[]) 
	{
		let minDist = Infinity;
	...
	pointInsidePolygon(point:Vector2, vs:Vector2[]) 
	{
	...
	async visualizeGame(vis:MaterialVisualizer) 
	{
		...
		const opHoriz = new LayoutOperation({
			pos: new Vector2(0, this.centerNode.y * csY),
			size: new Vector2(w, 50*3),
			alpha: 0.5,
		...
		const opVert = new LayoutOperation({
			pos: new Vector2(this.centerNode.x * csX, 0),
			size: new Vector2(h, 50*3),
			alpha: 0.5,
			pivot: new Vector2(0, 0.5),
			rot: 0.5*Math.PI
		})
		...
		{
			const line = new Line(conn.start.clone().scale(new Vector2(csX,csY)), conn.end.clone().scale(new Vector2(csX,csY)));
		...
		if(this.naturalResources) 
		{
			...
			for(const nr of this.naturalResources) 
			{
				const op = new LayoutOperation({
					...
					size: new Vector2(2*this.cfg.naturalResourceRadius*cs),
		...
		for(const p of this.points) 
		{
			...
			if(p.nodeType == 'Mission') 
			{
				...
				const rectPos = new Vector2(p.x * csX - halfSize, p.y * csY - halfSize);
				obj = new Rectangle().fromTopLeft(rectPos, new Vector2(halfSize * 2));

				const opMissionNode = new LayoutOperation({
					pos: this.getRealPos(p),
					size: new Vector2(halfSize),
					pivot: Vector2.CENTER,
				...
				const opOutline = new LayoutOperation({
					...
					size: new Vector2(2*halfSize*outlineMarginFactor),
					pivot: Vector2.CENTER,
				...
				const opOutline = new LayoutOperation({
					...
					size: new Vector2(2*radius*cs*outlineMarginFactor),
					pivot: Vector2.CENTER,
					depth: 3,
					rot: this.getRandomRotation(),
				})
				...
				if(p.type == 'Center') 
				{
					...
					const opTextMetadata = new LayoutOperation({
						pos: new Vector2(opRegular.pos.x, opRegular.pos.y + opRegular.size.y * 0.5 + offset),
						pivot: Vector2.CENTER,
						fill: "#555555",
						size: new Vector2(15,2).scale(textConfigMetadata.size),
						depth: 4
					})
			...
			{
				const center = new Vector2(objectCenter.x + pd.x * radius * cs, objectCenter.y + pd.y * radius * cs);
			...
			if(p.type == 'Center') 
			{
				...
				if(this.cfg.expansions.nodesOfKnowledge) 
				{
					...
					const pos1 = new Vector2(obj.center.x - (radius+margin)*cs, obj.center.y - (radius+margin)*cs);
					const pos2 = new Vector2(obj.center.x + (radius+margin)*cs, obj.center.y + (radius+margin)*cs);
					const size = new Vector2(0.6*radius*cs);
		...
		if(this.intermediaryPoints) 
		{
			...
			const iPointSize = new Vector2(radius * cs * 0.25 * 2);
		...
		if(this.expeditionNodes) 
		{
			...
			for(const n of this.expeditionNodes) 
			{
				...
				const op = new LayoutOperation({
					...
					frame: 0,
					size: new Vector2(expeditionNodeRadius * 2.0),
					pivot: Vector2.CENTER,
					rot: this.getRandomRotation()
				});
				...
				for(let a = 0; a < s; a++) 
				{
					...
					const scatter = new Vector2(
						Math.cos(scatterAngle)*scatterRadius, 
						Math.sin(scatterAngle)*scatterRadius
					);

					const circ = new Circle({ center: new Vector2(xPos + scatter.x, yPos + scatter.y), radius: expeditionSlotRadius });
					graphicsGroup.add(new ResourceShape(circ), opCirc);

					const op = new LayoutOperation({
						pos: circ.center,
						size: new Vector2(2*expeditionSlotRadius),
		...
		if(this.landmarks) 
		{
			const landmarkRadius = new Vector2(0.5 * cs);
	...
	{
		p.clampPositionTo(new Vector2(this.cfg.resolutionX, this.cfg.resolutionY));
	}
	...
	getRealPos(pos:{ x: number, y: number }) : Vector2
	{
		const csX = this.cfg.cellSizeX, csY = this.cfg.cellSizeY
		return new Vector2(pos.x * csX, pos.y * csY);
	}
