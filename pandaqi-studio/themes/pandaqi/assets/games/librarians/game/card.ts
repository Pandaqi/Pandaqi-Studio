...
import { MaterialVisualizer, Vector2, Rectangle, ResourceGroup, fillResourceGroup, LayoutOperation, TintEffect, BlurEffect, ResourceShape, TextConfig, ResourceText, getRectangleCornersWithOffset, InvertEffect, TextWeight, TextAlign, StrokeAlign } from "lib/pq-games";
import { ACTIONS, ACTIONS_THRILL, AGE_RANGES, AUTHORS, BOOK_TITLES, COLORS, CardType, GENRES, MISC } from "../shared/dict";
...
export default class Card
{
    ...
    getBookDimensions(vis:MaterialVisualizer)
    {
        ...
        const offsetBook = new Vector2(0.5 * (vis.size.x - backgroundDims.x), 0.5 * (vis.size.y - backgroundDims.y) + 0.045*backgroundDims.y);
        ...
        const rectSpine = new Rectangle().fromTopLeft(offsetBook, new Vector2(spineWidth, sizeBook.y));

        const offsetInner = offsetBook.clone().add(new Vector2(spineWidth, 0));
        const rectInner = new Rectangle().fromTopLeft(offsetInner, new Vector2(sizeBook.x - spineWidth, sizeBook.y));
    ...
    drawBlurredRect(vis:MaterialVisualizer, group:ResourceGroup, pos:Vector2, size:Vector2, color:string = "#FFFFFF", composite:GlobalCompositeOperation = "source-over", alpha:number = 1.0)
    {
    ...
    drawShelfContent(vis:MaterialVisualizer, group:ResourceGroup)
    {
        ...
        const positions = [
            new Vector2(textPos.x - 0.55*textDims.x, textPos.y + 0.5*textDims.y - 0.5*arrowVertDims.y),
            new Vector2(textPos.x + 0.55*textDims.x, textPos.y + 0.5*textDims.y - 0.5*arrowVertDims.y)
        ]
    ...
    drawGenre(vis:MaterialVisualizer, group:ResourceGroup)
    {
        ...
        const positions = [
            new Vector2(bookCoverOffset.x + 0.5*bookCoverSize.x, bookCoverOffset.y + textOffsetY),
            new Vector2(bookCoverOffset.x + 0.5*bookCoverSize.x, bookCoverOffset.y + bookCoverSize.y - textOffsetY)
        ]
        ...
        for(const pos of positions)
        {
            const opText = new LayoutOperation({
                pos: pos,
                size: new Vector2(bookCoverSize.x, 2*fontSize),
                pivot: Vector2.CENTER,
    ...
    drawMetadata(vis:MaterialVisualizer, group:ResourceGroup)
    {
        ...
        const posIcon = new Vector2(rectSpine.getCenter().x, rectSpine.getTopLeft().y + edgeMargin);
        ...
        const textBoxSize = new Vector2(rectSpine.getSize().y, rectSpine.getSize().x);
        const strokeWidth = 0.1*textConfigAuthor.size;
        const opTextAuthor = new LayoutOperation({
            pos: posIcon.clone().add(new Vector2(0, 0.5*sizeIcon.x)),
            size: textBoxSize,
            rot: rotation,
        ...
        const posIconBottom = new Vector2(rectSpine.center.x, rectSpine.getBottomLeft().y - edgeMargin); 
        ...
        const opTextAge = new LayoutOperation({
            pos: posIconBottom.clone().add(new Vector2(0, -0.5*sizeIcon.x)),
            size: textBoxSize,
            rot: rotation,
    ...
    drawTitle(vis:MaterialVisualizer, group:ResourceGroup)
    {
        ...
        const dropCapPos = new Vector2(rectInner.center.x, rectInner.getTopLeft().y + dropCapOffsetY)
        this.drawBlurredRect(vis, group, dropCapPos, new Vector2(dropCapSize), "#FFFFFF");
        ...
        const opDropCap = new LayoutOperation({
            pos: dropCapPos,
            size: new Vector2(2.0 * dropCapSize),
            pivot: Vector2.CENTER,
            fill: colorData.dark
        });
        ...
        {
            const positions = [
                new Vector2(dropCapPos.x - 0.6 * dropCapSize, dropCapPos.y),
                new Vector2(dropCapPos.x + 0.6 * dropCapSize, dropCapPos.y)
            ]
            ...
            for(const pos of positions)
            {
                const opNumber = new LayoutOperation({
                    pos: pos,
                    size: new Vector2(1.5*textConfig.size),
                    pivot: Vector2.CENTER,
        ...
        const titlePos = new Vector2(rectInner.center.x, rectInner.getTopLeft().y + titleOffsetY);
    ...
    drawAction(vis:MaterialVisualizer, group:ResourceGroup)
    {
        ...
        const actionPos = new Vector2(rectInner.center.x, rectInner.getTopLeft().y + offsetY);
        const actionDims = new Vector2(rectInner.extents.x, sizeY);
        const fontSizeTitle = vis.get("cards.action.fontSize");
        this.drawBlurredRect(vis, group, actionPos.clone().add(new Vector2(0, 0.33*fontSizeTitle)), actionDims, "#FFFFFF", "source-over", 0.7);

        // "header" of action
        const sizeIcon = vis.get("cards.action.sizeIcon");
        const posHeader = actionPos.clone().add(new Vector2(-0.5*actionDims.x + 0.5*sizeIcon.x, -0.5*actionDims.y + 0.5*sizeIcon.y));  
        ...
        const posTitle = posHeader.clone().add(new Vector2(0.5 * (actionDims.x - sizeIcon.x) , 0));
        ...
        const opTextTitle = new LayoutOperation({
            pos: posTitle,
            size: new Vector2(actionDims.x, sizeIcon.y),
            pivot: Vector2.CENTER,
            fill: colorData.dark,
        });
        ...
        if(addActionExplanation)
        {
            ...
            const opTextAction = new LayoutOperation({
                pos: actionPos.clone().add(new Vector2(0, 0.5 * sizeIcon.y)),
                size: actionDims,
                pivot: Vector2.CENTER,
                fill: colorData.dark
            });
