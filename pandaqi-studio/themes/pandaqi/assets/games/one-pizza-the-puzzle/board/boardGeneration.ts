...
import { Vector2, MaterialVisualizer, seedRandom, shuffle, Line, ResourceGroup, LayoutOperation, ResourceShape, TextConfig, Rectangle, StrokeAlign, ResourceText, Color, getWeighted } from "lib/pq-games";

type RandomizerFunction = () => number;
type TrafficSign = { pos: Vector2, type: string, side: number, ind: number };

const NBS = [new Vector2(1,0), new Vector2(0,1), new Vector2(-1,0), new Vector2(0,-1)];
const NBS_BOX = [new Vector2(), new Vector2(1,0), new Vector2(1,1), new Vector2(0,1)];
const CELL_OFFSETS = [new Vector2(1,-1), new Vector2(1,1), new Vector2(-1, 1), new Vector2(-1,-1)];
...
export default class BoardGeneration
{
	...
	policeCells: Cell[]
	possiblePoliceVector2s: Vector2[]
	allEntrances: Cell[]
	fullIngredientList: number[]
	crossingCells: Vector2[]
	crossingSets: Vector2[][]
	pathsToExtend: Vector2[]
	...
	reserveShapeSquares() 
	{
		...
		const possibleConfigurations = [
			new Vector2(4,1), new Vector2(3,1), new Vector2(2,2), 
			new Vector2(3,1), new Vector2(1,4)
		];
		...
		{	
			if(this.VARIATION_RNG() <= this.cfg.crossingDiagonalVariationProbability) 
			{
				...
				if(this.VARIATION_RNG() <= 0.5) {
					this.addRoadCell(new Vector2(pos.x + 1, pos.y + 1), new Vector2(1, 0), crossing);
					this.addRoadCell(new Vector2(pos.x - 1, pos.y - 1), new Vector2(-1, 0), crossing)
				} else {
					this.addRoadCell(new Vector2(pos.x + 1, pos.y - 1), new Vector2(0,-1), crossing);
					this.addRoadCell(new Vector2(pos.x - 1, pos.y + 1), new Vector2(0, 1), crossing);
				}
			}
		}
	...
	addRoadCell(pos:Vector2, dir:Vector2, originCrossing:number) 
	{
	...
	getValidExtensionDir(pos:Vector2) 
	{
		// find a free direction into which to extend
		const validExtensionDirections = [];
		for(const nb of NBS) 
		{
			const tPos = new Vector2(pos.x + nb.x, pos.y + nb.y);
	...
	extendRoad(pos:Vector2, dir:Vector2) 
	{
		let tPos = pos.clone();
		...
		{
			if(this.neighbourCellFilled(new Vector2(pos.x+dir.x, pos.y+dir.y), dir) && this.VARIATION_RNG() <= this.cfg.forbidFirstStepProbability) 
			{ 
				canContinue = false; 
			}
		}
	...
	prepareLists() 
	{
		...
		this.RAND_POINT_RNG = seedRandom(seed + "-randVector2s");
		...
		this.possiblePoliceVector2s = []; // police is preferably placed on crossings (maximum visibility), so keep track of those during generation
		...
		for(let x = 0; x < this.cfg.resX; x++) 
		{
			for(let y = 0; y < this.cfg.resY; y++)
			{
				this.randomCreationOrder.push(new Vector2(x,y));
			}
		}
	...
	generateBordersToPos(pos:Vector2, buildingIndex:number) 
	{
		const borders : Border[] = [];
		let corner = new Vector2(pos.x + 1, pos.y);

		for(let i = 0; i < NBS.length; i++) 
		{
			const nb = NBS[i];
			const nextDir = NBS[(i + 1) % 4];
			const nextCorner = new Vector2(corner.x + nextDir.x, corner.y + nextDir.y);
			const tPos = new Vector2(pos.x + nb.x, pos.y + nb.y);
	...
	generateValidNeighbours(pos:Vector2) 
	{
	...
	cellFilled(pos:Vector2) 
	{
		return !this.getCell(pos).isEmpty();
	}

	sameOriginCrossing(pos1:Vector2, pos2:Vector2) 
	{
		return (this.getCell(pos1).originCrossing == this.getCell(pos2).originCrossing)
	}

	neighbourCellFilled(pos:Vector2, dir:Vector2) 
	{
		for(let i = 0; i < 4; i++) 
		{
			const nb = NBS[i];
	...
	getOrientationFromNeighbours(pos:Vector2) 
	{
		...
		for(let i = 0; i < 4; i++) 
		{
			...
			let nb = NBS[i];
			let tPos = new Vector2(pos.x + nb.x, pos.y + nb.y);
	...
	calculateDirFromSurroundings(pos:Vector2) : Vector2 
	{
		for(const nb of NBS) 
		{
			let tPos = new Vector2(pos.x + nb.x, pos.y + nb.y);
	...
	{
		// now we have our list of buildings, each of which is an ARRAY of Vector2s
	...
	determineDecorationBorders() 
	{
		...
		for(const o of this.obstacles) 
		{
			...
			// obstacles are saved with their CENTER point, so startVector2 + vector2(1,1)
			// now we just look at its neighbours and check if they have the property "roundabout" set to true
			const borders = [new Vector2(-1,1), new Vector2(-1,-1), new Vector2(1,-1), new Vector2(1,1)];
			const NBS_CUSTOM = [new Vector2(-1,0), new Vector2(0,-1), new Vector2(1,0), new Vector2(0,1)]; // @UPDATE: we need a custom order for NBs here to make this correct, because old me is stupid
			for(let a = 0; a < NBS_CUSTOM.length; a++) 
			{
				const nb = NBS_CUSTOM[a];
				const tPos = new Vector2(o.center.x + nb.x, o.center.y + nb.y);
				...
				if(shouldAddBorder) 
				{
					const border = new Line(
						new Vector2(o.center.x + borders[a].x*0.5, o.center.y + borders[a].y*0.5),
						new Vector2(o.center.x + borders[(a+1)%4].x*0.5, o.center.y + borders[(a+1)%4].y*0.5)
					)
	...
	obstacleWithCenter(pos:Vector2) 
	{
	...
	removeUglyBoxes() 
	{
		const dirVectors = [new Vector2(1,0), new Vector2(2,1), new Vector2(1,2), new Vector2(0,1)];
		...
		for(let i = 0; i < this.randomCreationOrder.length; i++) 
		{
			...
			if(randSolution == 'hedge') 
			{
				...
				this.getCell(new Vector2(pos.x + start.x, pos.y + start.y)).closedSides[randDir] = true;
				this.getCell(new Vector2(pos.x + end.x, pos.y + end.y)).closedSides[(randDir + 2) % 4] = true;
				...
				h.setLine( new Line(new Vector2(pos.x+1, pos.y+1), new Vector2(pos.x+vec.x, pos.y+vec.y)) );
				...
				for(let a = 0; a < 4; a++) 
				{
					let tPos = new Vector2(pos.x + NBS_BOX[a].x, pos.y + NBS_BOX[a].y);
				...
				r.setCenter(new Vector2(pos.x + 1, pos.y + 1));
		...
		{ 
			...
			for(const o of this.obstacles) 
			{
				...
				for(let a = 0; a < 4; a++) 
				{
					let tPos = new Vector2(o.center.x + NBS[a].x, o.center.y + NBS[a].y);
	...
	mergeUglyBox(pos:Vector2, RNG:RandomizerFunction) 
	{
		let success = false;
		for(let i = 0; i < NBS_BOX.length; i++) 
		{
			let tPos = new Vector2(pos.x + NBS_BOX[i].x, pos.y + NBS_BOX[i].y);
	...
	getUglyBoxData(pos:Vector2) 
	{
		let hasRoundabout = false;
		for(let i = 0; i < NBS_BOX.length; i++) 
		{
			const tPos = new Vector2(pos.x + NBS_BOX[i].x, pos.y + NBS_BOX[i].y);
	...
	buildingAtSide(pos:Vector2, num:number) 
	{
		let dir = this.convertIndexToDir(num);
		let tPos = new Vector2(pos.x + dir.x, pos.y + dir.y);
	...
	sideClosed(pos:Vector2, num:number) 
	{
		return (this.getCell(pos).closedSides[num]);
	}

	isRoundabout(pos:Vector2) 
	{
	...
	addToBuilding(b:Building, pos:Vector2) 
	{
		b.tiles.push(pos);
	...
	getNeighborBuildingTile(pos:Vector2, maxEntrances = -1) 
	{
		const nbs = NBS.slice();
		this.shuffle(nbs);
	...
	getNeighborBuilding(type = 'connected', pos:Vector2, buildingIndex:number) 
	{
		const nbs = NBS.slice();
		this.shuffle(nbs);
	...
	placeSingleEntrance(pos:Vector2, obj:Cell, maxEntrances = -1) 
	{
	...
	getSquarePositionOnCell(pos:Vector2, ind:number, center = true) 
	{
		...
		return new Vector2( newPos.x - 0.25, newPos.y - 0.25 );
	}

	pickSuitableIngredient(pos:Vector2) {
	...
	distanceToSameEntrance(pos:Vector2, ing:number) 
	{
	...
	getRandomPoliceVector2() : Vector2
	{
		let x,y
		let invalidVector2 = false;
		...
		do {
			...
			invalidVector2 = notARoad || isPolice || tooCloseToPolice;
			tries++;
		} while(invalidVector2 && tries < maxTries);
	...
	distanceToClosestPolice(posA:Vector2) 
	{
		let dist = Infinity;
	...
	getFreeSquareIndex(pos:Vector2) 
	{
	...
	{
		this.policeCells = [];
		this.shuffle(this.possiblePoliceVector2s);

		const numPolice = Math.max(Math.round(this.possiblePoliceVector2s.length*this.cfg.policeFactor), 1.0);
		let pos
		for(let i = 0; i < numPolice; i++) 
		{
			if(this.possiblePoliceVector2s.length > 0) {
				pos = this.possiblePoliceVector2s.pop();
			} else {
				pos = this.getRandomPoliceVector2();
			}
	...
	distanceToClosestTrafficSign(posA:Vector2) 
	{
		let dist = Infinity;
	...
	getRandomTrafficVector2(type:string) : Vector2
	{
		...
		do {
			...
			invalidVector2 = notARoad || isTrafficSign || tooCloseToSign;
			tries++;
			...
			if(!invalidVector2) 
			{
				if(isGate) {
					...
					invalidVector2 = cell.roundabout || (this.getEmptySide(pos) == null)
				} else {
					invalidVector2 = (this.getFreeSquareIndex(pos) == null)
				}
			}
		} while(invalidVector2 && tries < maxTries);
	...
	placeTrafficSigns() {
		...
		for(let i = 0; i < numSigns; i++) {
			...
			let pos = this.getRandomTrafficVector2(randType);
	...
	async visualizeGame(vis:MaterialVisualizer) 
	{
		...
		{
			const line = new ResourceShape(new Line(new Vector2(x*cs,0), new Vector2(x*cs, this.cfg.resY*cs)));
			gridGroup.add(line, opLine);
		}

		for(let y = 0; y < this.cfg.resY; y += 0.5) 
		{
			const line = new ResourceShape(new Line(new Vector2(0, y*cs), new Vector2(this.cfg.resX*cs, y*cs)));
			gridGroup.add(line, opLine);
		}
		...
		for(let x = 0; x < this.cfg.resX; x++) 
		{
			for(let y = 0; y < this.cfg.resY; y++) 
			{
				const pos = new Vector2(x,y);
				const obj = this.getCell(pos);
				const posRect = new Vector2(x*cs,y*cs);
				const rect = new Rectangle().fromTopLeft(posRect, new Vector2(cs));
				...
				if(obj.type == 'road') 
				{
					...
					const opRoad = new LayoutOperation({
						pos: new Vector2(posRect.x+0.5*cs, posRect.y+0.5*cs),
						size: new Vector2(cs),
					...
					if(obj.hasSubway()) 
					{
						...
						const subwayPosReal = new Vector2(subwayPos.x*cs, subwayPos.y*cs);

						const resDec = this.cfg.visualizer.getResource("decorations");
						const opDec = new LayoutOperation({
							pos: subwayPosReal,
							size: new Vector2((0.5-margin)*cs),
							frame: 1,
							pivot: Vector2.CENTER,
							depth: 5
						})
					...
					if(obj.hasPolice()) 
					{
						...
						const policePos = this.getSquarePositionOnCell(new Vector2(x,y), obj.police.positionIndex, true);
						const resPolice = this.cfg.visualizer.getResource("general_icons");
						const opPolice = new LayoutOperation({
							pos: new Vector2(policePos.x*cs, policePos.y*cs),
							size: new Vector2((0.5-margin)*cs),
							pivot: Vector2.CENTER,
							frame: 1,
							depth: 5
					...
					if(buildingObj.type == 'ingredient' || buildingObj.type == 'special') 
					{
						...
						if(!this.cfg.inkFriendly || isCenterCell) 
						{
							const opSprite = new LayoutOperation({
								pos: new Vector2(posRect.x+0.5*cs, posRect.y+0.5*cs),
								size: new Vector2(cs * this.cfg.ingredientSpriteScale),
								pivot: Vector2.CENTER,
								depth: 15,
					...
					if(!this.cfg.inkFriendly) 
					{
						...
						const shadowOffset = new Vector2(this.cfg.borderWidth*this.cfg.cellSize);
		...
		for(const building of this.buildings) 
		{
			...
			{
				// b is an ARRAY [startCorner, endCorner], where each corner is a Vector2
				const line = new Line(new Vector2(b[0].x * cs, b[0].y * cs), new Vector2(b[1].x * cs, b[1].y * cs));
		...
		const opBank = new LayoutOperation({
			pos: new Vector2((randTile.x+0.5)*cs, (randTile.y+0.5)*cs),
			frame: 0,
			size: new Vector2(cs * this.cfg.ingredientSpriteScale),
			pivot: Vector2.CENTER,
			depth: 15
		...
		const opTextSeed = new LayoutOperation({
			pos: new Vector2((randNeighbour.x + 0.5)*cs, (randNeighbour.y + 0.5)*cs),
			size: new Vector2(cs),
			pivot: Vector2.CENTER,
			fill: "#AAAAAA",
			depth: 15
		...
		//
		const rectStart = new Vector2(
			this.cfg.resX-this.cfg.shapeRectSize.x,
			this.cfg.resY-this.cfg.shapeRectSize.y
		);

		// Big background rectangle
		const rectStartReal = new Vector2(rectStart.x*cs, rectStart.y*cs);
		const rect = new Rectangle().fromTopLeft(rectStartReal, new Vector2(this.cfg.shapeRectSize.x*cs, this.cfg.shapeRectSize.y*cs));
		...
		let moveShapeSize = new Vector2(0.8*cs);
		...
		for(let i = 0; i < this.shapes.length; i++) 
		{
			...
			const pos = new Vector2((rectStart.x+0.5+col)*cs, (rectStart.y + 0.5+row)*cs);
		...
		const hintMargin = 4;
		const posHint = new Vector2(rectStartReal.x + 0.5*rect.getSize().x, rectStartReal.y + rect.getSize().y - hintMargin);
		const hintX = 0.6*moveShapeSize.x;
		const opHint = new LayoutOperation({
			pos: posHint,
			size: new Vector2(hintX, (160.0/400.0) * hintX),
	...
	visualizeEntrance(group:ResourceGroup, obj:Cell, roadGroup:ResourceGroup) 
	{
		...
		const rectPos = new Vector2(entData.pos.x*cs, entData.pos.y*cs);
		const rectSize = new Vector2(0.5*cs);
		...
		const opSprite = new LayoutOperation({
			pos: new Vector2(rectPos.x + 0.5*rectSize.x, rectPos.y + 0.5*rectSize.y),
			size: new Vector2(rectSize.x * this.cfg.ingredientSpriteScale),
			pivot: Vector2.CENTER,
			alpha: 0.75
		})
	...
	visualizeTrafficSigns(group:ResourceGroup) 
	{
		...
		for(let i = 0; i < this.trafficSigns.length; i++) 
		{
			...
			if(dictObj.gate) 
			{
				...
				const posReal = new Vector2(pos.x*cs, pos.y*cs);
				...
				if(obj.type == 'Line Gate') {
					...
					const opText = new LayoutOperation({
						pos: posReal,
						size: new Vector2(3*textConfigGate.size),
						pivot: Vector2.CENTER,
						depth: 6,
						fill: "#2D0B37"
					})
					...
					if(obj.type == 'Smuggler Gate') 
					{
						const resGeneral = this.cfg.visualizer.getResource("general_icons");
						const opGate = new LayoutOperation({
							pos: new Vector2(posReal.x + 0.075*cs, posReal.y - 0.075*cs),
							size: new Vector2(0.2*cs),
							frame: 2,
							pivot: Vector2.CENTER,
			...
			// NOT A GATE; just a regular sign, display inside the designated square
				...
				const posReal = new Vector2(pos.x*cs, pos.y*cs); // @TODO: I should just replace AAAALL of these calls with one .toRealPosition() function
	...
	visualizeDecorations(group:ResourceGroup, buildingGroup:ResourceGroup, overlayGroup:ResourceGroup) 
	{
		...
		for(let i = 0; i < this.obstacles.length; i++) 
		{
			...
			if(o.type == 'hedge') 
			{
				...
				let h = o.line;
				const line = new Line(new Vector2(h.start.x*cs, h.start.y*cs), new Vector2(h.end.x*cs, h.end.y*cs));
				...
				const rectSize = new Vector2(cs);
				const rectPos = new Vector2((o.center.x - 0.5) * cs, (o.center.y-0.5) * cs);
				const rect = new Rectangle().fromTopLeft(rectPos, rectSize);

				const opRoundabout = new LayoutOperation({
					pos: new Vector2(o.center.x*cs, o.center.y*cs),
					pivot: Vector2.CENTER,
					depth: 20
				});
				...
				if(o.specialBuilding != null) {
					...
					opRoundabout.size = new Vector2(this.cfg.ingredientSpriteScale * cs);
					...
					opRoundabout.size = new Vector2(cs);
					color = "#9EFB7B";
					strokeColor = "#0E5E00";
				}
	...
	visualizeOrders(group:ResourceGroup) 
	{
		...
		for(let i = 0; i < this.buildings.length; i++) 
		{
			...
			const midVector2 = new Vector2((cell.x + 0.5) * cs, (cell.y + 0.5) * cs);
			...
			for(const ingNumber of order) 
			{
				const op = new LayoutOperation({
					pos: midVector2,
					size: new Vector2(this.cfg.ingredientSpriteScale * cs),
					pivot: Vector2.CENTER,
					depth: 15,
					frame: ingNumber + 1
				})
			...
			for(let sd = 0; sd < b.sideDishes.length; sd++) 
			{
				...
				const op = new LayoutOperation({
					pos: midVector2.clone().add(data.iconOffset.clone().scale(cs)),
					frame: data.iconFrame,
					size: new Vector2(0.4*cs),
					pivot: Vector2.CENTER,
					depth: 15
				})
				group.add(resIng, op);
			...
			const posText = new Vector2((randNeighbourCell.x + 0.5) * cs, (randNeighbourCell.y + 0.5) * cs);
			...
			const opCourier = new LayoutOperation({
				pos: new Vector2(posText.x, posText.y + 8 - 0.13*cs),
				size: new Vector2(0.5*cs),
	...
	getAllBuildingCellsAround(pos:Vector2, ind:number) 
	{
