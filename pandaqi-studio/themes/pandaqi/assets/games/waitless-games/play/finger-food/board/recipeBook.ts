...
import { getWeighted, MaterialVisualizer, ResourceGroup, Vector2, LayoutOperation, TextConfig, TextAlign, ResourceText, StrokeAlign, rangeInteger, shuffle } from "lib/pq-games";
...
class Recipe
{
    ...
    display(vis:MaterialVisualizer, group:ResourceGroup, boardDisplay:BoardDisplay, pos:Vector2, size:number)
    {
        ...
        const padding = vis.get("recipes.paddingWithinRecipe") * boardDisplay.cellSizeUnit;
        ...
        const txcfg = vis.get("recipes.textConfig");
        ...
        const opText = new LayoutOperation({
            pos: pos,
            size: new Vector2(5.5*spriteSize, 2*spriteSize),
            fill: txcfg.color,
            stroke: txcfg.stroke,
...
export default class RecipeBook
{
    ...
    getAvailableDimensions(boardDisplay : BoardDisplay)
    {
        ...
        const bottomRight = new Vector2(-Infinity, -Infinity);
        for(const cell of this.reservedCells)
        {
            const pos = boardDisplay.convertGridPointToRealPoint(cell.getPosition());
            const pos2 = boardDisplay.convertGridPointToRealPoint(cell.getPosition().add(new Vector2(1,1)));
    ...
    display(vis: MaterialVisualizer, group: ResourceGroup, boardDisplay : BoardDisplay)
    {
        ...
        const opBG = new LayoutOperation({
            ...
            size: new Vector2(sizeTotal.size.x),
        ...
        const margin = vis.get("recipes.marginAroundRecipeBook")*boardDisplay.cellSizeUnit;
        const padding = vis.get("recipes.paddingBetweenRecipes")*boardDisplay.cellSizeUnit;
        ...
        let anchor = sizeTotal.center.clone().sub(new Vector2(offsetX, offsetY));
