...
import { getWeighted, rangeInteger } from "lib/pq-games";

export const prepareCorrectCellTypes = () =>
{
    const maxTries = 100;
    let numTries = 0;
    do {
        setup();
        numTries++;
    } while(hasInvalidTypeDictionary() && numTries < maxTries)
}

const hasInvalidTypeDictionary = () =>
{
    const maxTypes = CONFIG._drawing.types.maxPerDifficulty[CONFIG._settings.difficulty.value];
...
const getAllWithProperty = (dict, prop, val) =>
{
    const list = [];
...
const removeKeysFromDict = (dict, list) =>
{
    ...
    for(const elem of list)
    {
        ...
        if(data.hasMultiple) 
        { 
            const newValues = getAllWithId(dict, data.id);
...
const checkIfPropertyInData = (dict, types, prop) =>
{
    let exists = false;
...
const setup = () =>
{
    ...
    const gameDiffNum = difficulties.indexOf(CONFIG._settings.difficulty.value);
    ...
    let types = [];
    const setTemplate = CONFIG._drawing.types.setTemplate;
    for(const type of setTemplate)
    {
        const options = getAllOfType(typesDict, type);
        ...
        if(addAll) { 
            ...
            removeKeysFromDict(typesDict, options);
        } else { 
            const option = getWeighted(tempOptionDict, "prob");
            types.push(option);
            removeKeysFromDict(typesDict, [option]);
        }
    }

    // EXCEPTION: make sure we have something that uses blank squares
    const typeUsesBlankSquares = checkIfPropertyInData(CELL_TYPES, types, "usesBlankSquares");
    if(!typeUsesBlankSquares)
    {
        const options = getAllWithProperty(CELL_TYPES, "usesBlankSquares", true);
        const tempOptionDict = {};
        for(const option of options)
        {
            tempOptionDict[option] = fullTypesDict[option];
        ...
        const randOption = getWeighted(tempOptionDict, "prob");
        types.push(randOption);
        removeKeysFromDict(typesDict, [randOption]);
    }

    // fill up the list of types as needed
    const setBounds = CONFIG._drawing.types.maxSetSize;
    const setMax = rangeInteger(setBounds.min, setBounds.max);
    while(types.length < setMax)
    {
        const randOption = getWeighted(typesDict, "prob");
        types.push(randOption);
        removeKeysFromDict(typesDict, [randOption]);
    }

    // EXCEPTION: some types specifically need a spy in the game
    const needsSpy = checkIfPropertyInData(CELL_TYPES, types, "needsSpy");
    ...
    if(needsSpy && !alreadyHasSpy)
    {
        ...
        types.push("spy");
        removeKeysFromDict(typesDict, ["spy"]);

    }

    // this should always be LAST, as the debug types override the rest
    const debugTypes = CONFIG._drawing.types.debug;
