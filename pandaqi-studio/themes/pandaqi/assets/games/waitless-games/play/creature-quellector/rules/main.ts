...
class Card
{
    ...
    draw(ctx:CanvasRenderingContext2D, pos: Vector2, inverted = false)
    {
        ...
        const offsetPerType = Vector2.RIGHT.clone().scaleFactor(typeSize + padding);
...
class Squad
{
    ...
    draw(ctx:CanvasRenderingContext2D, pos:Vector2, inverted = false)
    {
        const offsetPerCard = Vector2.RIGHT.clone().scaleFactor(CONFIG.itemSize.x + CONFIG.paddingBetweenCards);
...
export const generateForRulebook = async (sim:InteractiveExampleSimulator) =>
{
    ...
    const positions = [
        new Vector2(0.5*canv.width, 0.25*canv.height),
        new Vector2(0.5*canv.width, 0.75*canv.height)
    ]
