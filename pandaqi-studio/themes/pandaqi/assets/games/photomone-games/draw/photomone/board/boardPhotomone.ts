...
import { MaterialVisualizer, LayoutOperation, ResourceShape, StrokeAlign, TextConfig, TextAlign, ResourceText, Circle, Line, Rectangle, Vector2 } from "lib/pq-games"
...
export class BoardPhotomone
{
    ...
    visualize(vis:MaterialVisualizer)
    {
        ...
        {
            const rectObj = new Rectangle({ center: new Vector2(rect.p.x, rect.p.y), extents: new Vector2(rect.size.x, rect.size.y) });
        ...
        for(const sprite of visRes.sprites)
        {
            ...
            const opSprite = new LayoutOperation({
                pos: new Vector2(sprite.p),
                rot: sprite.rot ?? 0,
                size: new Vector2(sprite.size),
        ...
        for(const text of visRes.text)
        {
            ...
            const pivot = new Vector2(originX, originY);

            const op = new LayoutOperation({
                pos: new Vector2(text.p),
                size: new Vector2(0.5*vis.size.y, 2*text.fontSize),
        ...
        const rectAnchor = new Vector2((xOffset + 1.5*xOffset)*0.5, yOffset);
        const rectSize = new Vector2(spriteSize*3, (1 + yHeightFactor)*spriteSize);
        ...
        const opText = new LayoutOperation({
            pos: new Vector2(xOffset + 2*textConfig.size, yOffset),
            size: new Vector2(4*textConfig.size),
            fill: "#000000",
            pivot: Vector2.CENTER
        })
