import { shuffle, rangeInteger, range } from "lib/pq-games";
...
export default class CloudsGenerator
{
    visualType: string;
    gridSize: Vector2;
    gridSizeSquare: number;
    amountFilled: number;
    amountFilledTotal: number;
    grid: any[];
    backgroundColor: any;
    ...
    setupGrid(config)
    {
        ...
        this.gridSize = new Vector2(size.x / scaleDownFactor, size.y / scaleDownFactor);
    ...
    stampCloud(params:StampParams = {})
    {
        const pos = params.pos || new Vector2(0,0);
