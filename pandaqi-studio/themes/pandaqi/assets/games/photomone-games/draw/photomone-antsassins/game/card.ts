...
import { createContext, shuffle, rangeInteger, getWeighted, MaterialVisualizer, LayoutOperation, Vector2 } from "lib/pq-games"
...
export default class Card 
{
    ...
    setupCanvas()
    {
        ...
        // @ts-ignore
        this.inkFriendly = CONFIG._settings.defaults.inkFriendly.value;
        const bgColor = this.inkFriendly ? CONFIG.cards.fillStyleInkfriendly : CONFIG.cards.fillStyle;
        ...
        const shp = CONFIG._settings.tileShape.value
    ...
    assignRoles()
    {
        ...
        const numTeams = CONFIG._settings.material.numTeamsOnCodeCard.value;
        const numSecretTiles = CONFIG._settings.material.numSecretTilesPerTeam.value;
        ...
        const addAlmostActions = CONFIG._settings.material.addAlmostActions.value;
        for(let i = 0; i < numTeams; i++)
        {
            const teamKey = "team" + i;
    ...
    {
        await this.visualizeCells(vis);
        this.visualizeStartingTeam();
        return this.getCanvas();
    }

    async visualizeCells(vis:MaterialVisualizer)
    {
        const ctx = this.ctx;
        const cells = this.getFlatGrid();
        const drawPattern = !vis.inkFriendly;
        for(const cell of cells)
        {
            ctx.save();
            ...
            if(type == "almost" && cell.getAction())
            {
                ...
                if(CONFIG._settings.tileShape.value == "triangle") { spriteSize *= 0.75; } // simply less space in triangles, so compensate

                const res = vis.getResource("almostActions");
                const canvOp = new LayoutOperation({
                    frame: ALMOST_ACTIONS[cell.getAction()].frame,
                    pos: new Vector2(centerPos.x, centerPos.y),
    ...
    getTeamColor(team = "team0", type = "secret")
    {
        ...
        if(this.inkFriendly) { color = CONFIG.cards.teamColorsInkFriendly[team]; }

        if(type == "almost")
        {
            color = CONFIG.cards.almostColors[team];
            if(this.inkFriendly) { color = CONFIG.cards.almostColorsInkFriendly[team]; }
        }
