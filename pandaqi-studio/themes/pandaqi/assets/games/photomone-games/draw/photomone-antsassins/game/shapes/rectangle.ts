...
export default class Rectangle 
{
    ...
    {
        const numPoints = new Vector2(1 + resolution, 1 + resolution);
        const offset = new Vector2(this.extents.x / (numPoints.x - 1), this.extents.y / (numPoints.y - 1));
    ...
    getGridPoints(resolution)
    {
        const numPoints = new Vector2(1 + resolution, 1 + resolution);
        const offset = new Vector2(this.extents.x / (numPoints.x - 1), this.extents.y / (numPoints.y - 1));
        const start = new Vector2(this.pos.x - 0.5*this.extents.x, this.pos.y - 0.5*this.extents.y);
