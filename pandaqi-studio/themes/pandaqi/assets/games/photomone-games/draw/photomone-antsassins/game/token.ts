...
import { createContext, LayoutOperation, MaterialVisualizer, Vector2 } from "lib/pq-games";
...
export default class Token {
    ...
    {
        await this.visualizeSprite(vis);
        this.visualizeGrid();
        return this.getCanvas();
    }

    async visualizeSprite(vis:MaterialVisualizer)
    {
        const ctx = this.ctx;
        const res = vis.getResource("tokens");
        const canvOp = new LayoutOperation({
            pos: new Vector2(this.centerPos.x, this.centerPos.y),
            size: new Vector2(this.radius*2),
