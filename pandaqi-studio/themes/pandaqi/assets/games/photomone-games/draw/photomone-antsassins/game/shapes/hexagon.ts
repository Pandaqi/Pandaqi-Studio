...
export default class Hexagon 
{
    ...
    getPoints()
    {
        ...
        const arr = [];
        for (let i = 0; i < 6; i++) {
            const p = new Vector2(this.pos.x + r * Math.cos(a * i), this.pos.y + r * Math.sin(a * i))
            arr.push(p);
        }
        return arr;
    }
    ...
    {
        const offset = new Vector2(this.radius / resolution, this.hexRatio * this.radius / resolution);
        return offset.x;
    }
    ...
    getGridPoints(resolution)
    {
        ...
        const offset = new Vector2(this.radius / resolution, hexRatio * this.radius / resolution);
        const fullSize = this.radius*2;
        const numPoints = new Vector2(Math.round(fullSize/offset.x) + 1, Math.round( (fullSize/hexRatio)/offset.y ) - 1);

        const radiusY = hexRatio * this.radius;
        const start = new Vector2(this.pos.x - this.radius, this.pos.y - radiusY);
        ...
        for(let x = 0; x < numPoints.x; x++)
        {
            for(let y = 0; y < numPoints.y; y++)
            {
                ...
                const p = new Vector2(realX, realY);
                const gridPoint = new Vector2(x,y);
                p.gridPos = gridPoint;
