...
import { TextConfig, LayoutOperation, Vector2, ResourceText } from "lib/pq-games";
...
export default class CanvasDrawable 
{
    ...
    getPosFromEvent(ev:any)
    {
        ...
        p.move(new Vector2(-offset.left, -offset.top));
        ...
        if(ev.type == 'touchstart' || ev.type == 'touchmove' || ev.type == 'touchend' || ev.type == 'touchcancel')
        {
            ...
            p.move(new Vector2(touch.clientX, touch.clientY));
            ...
            p.move(new Vector2(ev.clientX, ev.clientY));
        } 
        p.scale(res);
        return p;
    }
