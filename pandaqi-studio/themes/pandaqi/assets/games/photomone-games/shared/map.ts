...
import { rangeInteger, shuffle, getWeighted, range } from "lib/pq-games";
export default class Map 
{
    ...
    edgeMargin: number;
    mapBounds: { min: Vector2; max: Vector2; };
    ...
    constructor(params:Record<string,any> = {})
    {
        ...
        this.mapBounds = {
            min: new Vector2({ x: this.edgeMargin, y: this.edgeMargin }),
            max: new Vector2({ x: this.width - this.edgeMargin - spaceReservedForWords, y: this.height - this.edgeMargin })
        }
    ...
    applyRandomChanges()
    {
        ...
        for(const p of this.getPointsAsList())
        {
            ...
            const vec = new Vector2({ x: randOffsetX, y: randOffsetY });
            p.move(vec);
    ...
    // @IMPROV: some way to ensure a type isn't added too often?
    ...
    getNeighborsInRange(p1:Vector2, rangeSquared = null)
    {
        const arr = [];
    ...
    calculateMinRangeBetweenPointsSquared(a:Vector2,b:Vector2)
    {
    ...
    sumPushAwayVectors(anchor:Vector2, nbs:Vector2[], iteration:number)
    {
        const change = new Vector2({ x: 0, y: 0});
    ...
    getEllipsePointAtAngle(angle)
    {
        ...
        return new Vector2({ x: ellipseX, y: ellipseY });
    }
    ...
    isPointInsideMapPoints(p:Vector2, exclude = [], params:Record<string,any> = {})
    {
    ...
    snapToClosestPoint(pos:Vector2, repel = false)
    {
        const points = this.getPointsAsList();
    ...
    getBiggestDistanceToExisting(anchorPoint:Vector2, linesToConsider:Line[])
    {
        const points = this.getPointsAsList();
        let biggestDist = 0;
