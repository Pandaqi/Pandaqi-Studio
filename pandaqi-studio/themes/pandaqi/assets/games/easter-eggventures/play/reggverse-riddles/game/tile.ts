...
import { MATERIAL, MISC_UNIQUE, TYPE_DATA, TileType } from "../shared/dict";
import { createContext, ResourceGroup, MaterialVisualizer, LayoutOperation, Vector2, TextConfig, TextWeight, ResourceText, ResourceImage, Rectangle, Color, ResourceShape, DropShadowEffect, colorLighten } from "lib/pq-games";
...
export default class Tile extends MaterialEaster
{
    ...
    drawBackground(vis:MaterialVisualizer, group:ResourceGroup)
    {
        ...
        let positions = [new Vector2(vis.center.x, vis.center.y - yOffset), new Vector2(vis.center.x, vis.center.y + yOffset)];
        if(this.type == TileType.ACTION) { positions = [positions[1]]; }
        if(this.type == TileType.OBJECTIVE) { positions = [new Vector2(vis.center.x, vis.size.y - 1*yOffset)]; }
    ...
    drawMovementGrid(vis:MaterialVisualizer) : ResourceImage
    {
        ...
        const rectInnerSize = new Vector2(0.9 * rectOuterSize);
        ...
        const yCentering = new Vector2(0.5 * rectOuterSize, 0.5 * (vis.size.y - gridDims.y*rectOuterSize) + 0.5*rectOuterSize);
        ...
        for(let x = 0; x < gridDims.x; x++)
        {
            ...
            {
                const pos = new Vector2(x,y).scale(rectOuterSize).add(yCentering);
                ...
                const op = new LayoutOperation({
                    fill: color,
                    stroke: colorLighten(color, darkenVal),
