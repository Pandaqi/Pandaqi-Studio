...
{
    const cards = [];
    generateClueCards(cards);
    generateScoreCards(cards);
    return cards;
}

const generateClueCards = (cards) =>
{        
    if(CONFIG._settings.sets.base.value)
    {
        ...
        {
            cards.push(new Card(CardType.CLUE, { num: i }));
        }
    }

    if(CONFIG._settings.sets.cluesRooms.value)
    {
...
const generateScoreCards = (cards) =>
{
    ...
    {
        const scoringRules = generateRandomScoringRules();
        cards.push(new Card(CardType.SCORE, { scoringValues: scoringRules }));
    }
}

const generateRandomScoringRules = () =>
{
