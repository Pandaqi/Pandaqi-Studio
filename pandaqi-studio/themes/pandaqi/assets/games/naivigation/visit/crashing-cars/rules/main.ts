...
import { MaterialVisualizer, Vector2, fromArray } from "lib/pq-games";
...
const visualizerTiles = new MaterialVisualizer(CONFIG, new Vector2(256,256));
...
const movementCallback = (card:MaterialNaivigation, setup:RandomNaivigationSetupGenerator, turn:RandomNaivigationTurnGenerator) =>
{
    ...
    if(key == "drive")
    {
        ...
        const invalidMove = setup.isPlayerOutOfBounds();
        if(invalidMove)
        {
    ...
    if(onParkingLot)
    {
        ...
        if(orientedCorrectly)
        {
            ...
            // @IMPROV: actually do that collecting + keep the unique shop number in mind for taking damage?
        } else {
...
const visualizerCards = new MaterialVisualizer(CONFIG, new Vector2(256,360));
