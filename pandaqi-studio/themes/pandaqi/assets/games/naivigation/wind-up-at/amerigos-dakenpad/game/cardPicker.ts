...
export const cardPicker = () : Card[] =>
{
    ...
    generateGladdeDaken(cards);
    generatePaardenSprongen(cards);
...
const generateBaseCards = (cards) =>
{
    ...
    // create one pawn
    cards.push(new Card(CardType.PAWN));
    ...
    for(let i = 0; i < numPakjeCards; i++)
    {
        ...
        cards.push(new Card(CardType.PAKJE, key, getFirstNumber(numbers, "odd")));
    }
    ...
    {
        cards.push(new Card(CardType.VAREN, vaarTypes.pop(), getFirstNumber(numbers, "even")));
    }
}

const getFirstNumber = (list:number[], type = "odd") =>
{
...
const generateGladdeDaken = (cards) =>
{
    if(!CONFIG._settings.sets.gladdeDaken.value) { return; }
    ...
    for(const type of types)
    {
        ...
        {
            cards.push(new Card(CardType.ROUTE, type));
        }
    }

}

const generatePaardenSprongen = (cards) =>
{
    if(!CONFIG._settings.sets.paardenSprongen.value) { return; }
