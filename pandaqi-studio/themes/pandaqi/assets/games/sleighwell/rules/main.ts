...
import { Vector2, range, createContext, convertCanvasToImage, getMaterialDataForRulebook, shuffle, fromArray } from "lib/pq-games";
...
class Board
{
    ...
    placeSleigh(tile:Tile, maxDistCenter:number)
    {
        ...
        const pos = new Vector2(posX, posY).round();

        this.placeTile(pos, tile);
        this.sleighPos = pos;
    }

    placeTile(pos:Vector2, tile:Tile)
    {
        this.grid[pos.x][pos.y] = tile;
    }

    removeTile(pos:Vector2, tile:Tile)
    {
    ...
    undoSleigh(from:Vector2, tilePositions:Vector2[], tiles:Tile[])
    {
    ...
    isFullyConnected()
    {
        ...
        const posToVisit : Vector2[] = [this.sleighPos];
    ...
    getValidSleighMoves() : Vector2[]
    {
        ...
        for(let x = 0; x < this.size.x; x++) 
        {
            const pos = new Vector2(x, this.sleighPos.y);
        ...
        for(let y = 0; y < this.size.y; y++)
        {
            const pos = new Vector2(this.sleighPos.x, y);
            if(pos.matches(this.sleighPos)) { continue; }
    ...
    getValidPlacements(tile:Tile) : Vector2[]
    {
        const emptyAdjacentCells = this.getEmptyAdjacentCells();
        const validCells = [];
    ...
    getRowAndColumnAround(pos:Vector2)
    {
        const tile = this.getCell(pos);
        const left = this.getTilesInDir(pos, Vector2.LEFT);
        const right = this.getTilesInDir(pos, Vector2.RIGHT);
        const row = [left.reverse(), tile, right].flat();

        const up = this.getTilesInDir(pos, Vector2.UP);
        const down = this.getTilesInDir(pos, Vector2.DOWN);
    ...
    getPositionsInDir(pos:Vector2, dir:Vector2, cutoff:Vector2 = null)
    {
    ...
    getTilesInDir(pos:Vector2, dir:Vector2, cutoff:Vector2 = null)
    {
    ...
    getNeighbors(pos:Vector2) : Vector2[]
    {
